...
An \textit{alphabet}, denoted \Alphabet{}, is set of \textit{characters}.
...
\SemanticsOf{\IdentityLens} &=& \SetOf{(\String,\String)\SuchThat\String\in\LanguageOf{\Regex}}\\
...
of its type.  The \textit{restriction} of $\SemanticsOf{\Lens}$ to \Relation{},
denoted \SemanticsOf{\Lens}\RestrictedTo{\Relation},
equals $\SemanticsOf{\Lens}\Intersect\Relation$.
...
A DNF regular esxpression is, intuitively, a regular expression with
...
The syntax of this language is formalized in Figure~\ref{fig:dnf-regex-syntax}.
...
To do this, Some functions need to be defined on DNF regular expressions,
which we define in Figure~\ref{fig:dnf-regex-functions}.
From this, \ToDNFRegex{}, a function which converts a regular expression into
...
$\SemanticsOf{\DNFLens}\Intersect(\LanguageOf{\Regex})=\SemanticsOf{\Lens}\Intersect(\LanguageOf{\Regex})$.
\end{restatable}
\input{proof-dnfls}
