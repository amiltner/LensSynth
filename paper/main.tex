...
\begin{proof}
  ...
  \item[\LeftUnrollRewriteRule{}]
    ...
    Because of this, by Lemma~\ref{?} and Lemma~\ref{?}, there exists the typing
    for the lens
...
\begin{lemma}[Confluence of Parallel Rewriting Without Reordering]
  \label{lem:parallel_confluence_noswap}
...
\begin{lemma}[Confluence of Parallel Rewriting With Reordering]
  \begin{enumerate}\leavevmode
  \item For all lenses $\Lens$,
...
\begin{proof}
  ...
  \item[\LeftUnrollRewriteRule{}]
    ...
    Lemma~\ref{lem:parallel_confluence_noswap}.
  \item[\RightUnrollRewriteRule{}]
    All cases are handled in the same way as in
    Lemma~\ref{lem:parallel_confluence_noswap}.
  \item[\LeftQuotientRewriteRuleOf{n}]
...
\begin{lemma}
  If $\DNFRegex \StarOf{\ParallelRewrite} \DNFRegex'$, and $\DNFRegexAlt
  \StarOf{\ParallelRewrite} \DNFRegexAlt'$, and there exists a rewriteless DNF lens
  ...
  $\DNFRegex \StarOf{\ParallelRewriteSwap} \DNFRegex''$, $\DNFRegexAlt \StarOf{\ParallelRewriteSwap}
  \DNFRegexAlt''$, and
...
\begin{proof}
  ...
  $\DNFOf{\SequenceOf{\StarOf{\DNFRegex}}} \StarOf{\ParallelRewrite}
  \DNFOf{\SequenceOf{\StarOf{\DNFRegexAlt}}}$ by Lemma~\ref{?}, and
  $\StarOf{\ParallelRewrite} = \StarOf{\Rewrite}$.
\end{proof}
...
\end{corollary}
\begin{proof}
  If $\DNFRegex_1 \StarOf{\ParallelRewrite} \DNFRegexAlt_1$, and $\DNFRegex_2
  \StarOf{\ParallelRewrite} \DNFRegexAlt_2$, then
  $\DNFRegex_1 \ConcatDNF \DNFRegex_2 \StarOf{\ParallelRewrite} \DNFRegexAlt_1
  \ConcatDNF \DNFRegexAlt_2$ by Lemma~\ref{?}, and
  $\StarOf{\ParallelRewrite} = \StarOf{\Rewrite}$
\end{proof}
...
\begin{proof}
  If $\DNFRegex_1 \StarOf{\ParallelRewrite} \DNFRegexAlt_1$, and $\DNFRegex_2
  \StarOf{\ParallelRewrite} \DNFRegexAlt_2$, then
  $\DNFRegex_1 \OrDNF \DNFRegex_2 \StarOf{\ParallelRewrite} \DNFRegexAlt_1
  \OrDNF \DNFRegexAlt_2$ by Lemma~\ref{?}, and
  $\StarOf{\ParallelRewrite} = \StarOf{\Rewrite}$
\end{proof}
...
Let the last typing rule be an instance of \OrLensRule{}.
  ...
  \equiv_{\StarOf{\Rewrite}} \ToDNFRegexOf{\Regex'}$.  By Lemma~\ref{?}, there exists
  a $\DNFRegex_1$, $\DNFRegex_2$, and $\DNFLens_{\Regex}'$ such that $\ToDNFRegexOf{\Regex} \StarOf{\Rewrite}
  \DNFRegex_1$, and $\ToDNFRegexOf{\Regex'} \StarOf{\Rewrite} \DNFRegex_2$,
  ...
  By Lemma~\ref{?}, as $\ToDNFRegexOf{\Regex}\StarOf{\Rewrite}\DNFRegex$ and
  $\ToDNFRegexOf{\Regex}\StarOf{\Rewrite}\DNFRegex_1$, and there exists a DNF Lens
  ...
  $\DNFRegex \StarOf{\Rewrite} \DNFRegex'$, $\DNFRegex_1 \StarOf{\Rewrite} \DNFRegex_1'$,
  ...
  $\DNFRegex_2'$, $\DNFLens_{\Regex}$ such that $\DNFRegex_2 \StarOf{\Rewrite}
  ...
  $\DNFRegex \StarOf{\Rewrite} \DNFRegex'$ and $\DNFRegexAlt \StarOf{\Rewrite}
