...
Specifically,
we present \Optician{}, an algorithm for {\em type-directed synthesis} of bijective
string transformers in Boomerang, a bidirectional language based on the
theory of {\em lenses}.  The inputs to \Optician{} are a pair of ordinary
...
internal regular expression.  This unambiguity allows the identity lens to
merely be syntactic sugar for 
...
composition (we describe sufficient properties for this closure shortly).
...
%
\begin{lstlisting}
$\DNFLegacyTitle$ = $\DNFLeft \SequenceOf{\StringCF{"<Field Id=2></Field>"}}$
  $\DNFSep
...
%
\begin{lstlisting}
$\DNFModernTitle$ = $\DNFLeft \SequenceOf{\StringCF{"Title:"} \SeqSep \TextChar
    \SeqSep \StringCF{""}
    \SeqSep \StarOf{\DNFOf{\SequenceOf{\TextChar}}}
    \SeqSep \StringCF{","}}$
                    $\DNFSep
...
This directed rewrite system help limit
...
This required expansion is apparent
...
expansions. These situations typically appear in data formats with large amounts
...
The function, \RigidSynth{}, implements the portion of \Optician{} that
  ...
  \begin{algorithmic}[1]
    ...
    \CaseTwo {\SomeOf{\ListOf{\AtomLens_1 \SeqSep \ldots \SeqSep
          \AtomLens_n}}}{$\ReturnVal{\SomeOf{(\SequenceLensOf{(\String_0,\StringAlt_0)
            \SeqSep \AtomLens_1 \SeqSep \ldots \SeqSep \AtomLens_n \SeqSep (\String_n,\StringAlt_n)},\sigma)}}$}
    \EndCaseTwo
    \CaseTwo {\None}{$\ReturnVal{\None}$}
    \EndCaseTwo
    \EndSwitch
    ...
    \CaseTwo {\SomeOf{\ListOf{\SequenceLens_1 \DNFSep \ldots \DNFSep
          \SequenceLens_n}}}{$\ReturnVal{\SomeOf{(\DNFLensOf{\SequenceLens_1
            \DNFSep \ldots \DNFSep \SequenceLens_n},\sigma)}}$}
    \EndCaseTwo
    \CaseTwo {\None}{$\ReturnVal{\None}$}
    \EndCaseTwo
    \EndSwitch
...
orderings,
\Optician{} guarantees not only that there will be a lens between the regular
...
The proofs of these, and the previous theorems, are provided in the extended
version, and will be published in a tech report.
...
\afm{211} AST nodes, for complex tasks like transforming arbitrary XML to a
...
defined data types) succeeds on \afm{7} of the benchmarks.
...
\afm{Ideas for ways to expand: more in depth on similar type-directed synthesis
  things; more discussion on similar proofs}
...
\begin{proof}
  ...
  \SetOf{(\String_{1,0}\Concat\String_1\Concat\ldots
    ...
    \Concat\StringAlt_{\Permutation(n)}\Concat\String_{1,n})\\
    \hspace*{3em}\in\SemanticsOf{(\SequenceLensOf{(\String_{2,0},\String_{3,0})\SeqLSep
        \AtomLens_{2,1}\SeqLSep\ldots\SeqLSep\AtomLens_{2,n}
        \SeqLSep(\String_{2,n},\String_{3,n})},\Permutation_2)}}\\
  \SetOf{(\String_1,\String_3)\SuchThat\exists\String_2\\
    \hspace*{2em}
...
Thus, if we have confluence and bisimilarity on rewrites for regular
expressions, then we also do for the DNF rewrites they induce.  As such, because
