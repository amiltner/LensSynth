...
composition (we describe sufficient properties for this closure
shortly). \ksf{are these the properties in the next paragraph?  If so,
you can delete the parenthetical.}

In particular, it is sufficient \ksf{For what purpose?} for the equivalences used in type equivalence
...
of \emph{expansions} $\ExpCount$ (\IE, uses of a rewrite rule or the substitution
of a user-defined
definition for its name \ksf{Anders, confirm that we are replacing a
name with its definition.})  required to obtain these regular
...
\Expand{}, shown in Algorithm~\ref{alg:synthlens2},
 has three major components: \ExpandRequired{}, \FixProblemElts{}, and
\ExpandOnce{}.   \Expand{} makes critical use of the 
locations of user-defined data types, measured 
