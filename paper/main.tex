...
\begin{abstract}
  ...
  with respect to a standard language of bijective lenses\bcp{I don't know
...
unrolling of Kleene Star.\footnote{\BCP{Why not?  More generally, why
...
other hand, there is a lot of data that must be managed carefully by
...
rewrite relation that creates equivalent regular expressions.  \BCP{This is
...
Conway's axioms consist of the semiring axioms\bcp{summarize them in words},
and the following 
...
to be confluent\bcp{say why}.  It is also important that rewrites can be \emph{mirrored}.
...
$\Regex'$ and $\RegexAlt'$.  While Conway's axioms are confluent\BCP{I
  thought we didn't know this}, they don't
satisfy the second property\bcp{say why not}.
Furthermore, removing the rewrites that cannot be mirrored makes\bcp{say why}
the rewrites no longer confluent!\bcp{...Or complete, I suppose.}  
...
\begin{lemma}[Expressibility of Adjacency Swapping Permutation of Separated Concat List]
  ...
  \DNFRegex_1 \ConcatDNF
  \ldots \ConcatDNF \DNFRegex_n \Leftrightarrow
  \DNFRegexAlt_{\sigma_i(1)} \ConcatDNF \ldots \ConcatDNF
  \DNFRegexAlt_{\sigma_i(n)}$, where $\SemanticsOf{\DNFRegex} =
  \SetOf{(\String_1\Concat\Sep\Concat\ldots\Concat\Sep\String_n,
    \StringAlt_{\sigma(1)}\Concat\Sep\Concat\ldots\Concat\Sep\Concat\StringAlt_{\sigma(n)})
    \SuchThat
