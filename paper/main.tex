...
\documentclass[numbers,10pt,preprint\ifanon ,nocopyrightspace\fi]{acmart}
...
\begin{figure}
  ...
  \begin{tikzpicture}
    \begin{groupplot}[
      ...
      \nextgroupplot[ymin=120,ymax=125,
      ytick={125},
      yticklabels={$>120$},
      axis x line=top, 
  ...
  \end{tikzpicture}
  \label{fig:time-plot}
  \caption{This will be used for time plot.}
\end{figure}


We evaluated our synthesis algorithm by applying it to 26
benchmark programs derived from practice.  We investigate our benchmark suite,
the impact of our optimizations, how many examples are required, and a
comparison against other tools.
The tests were run on a 2.5 GHz Intel Core i7 processor with 16 GB of 1600 MHz DDR3
...
algorithm was finalized, demonstrating that our optimizations are not build
purely for the testing data.
...
those lenses.  We generate lenses of sizes between 5 AST nodes, for simple
...
scale as expected, with more complex tasks taking longer.
...
\begin{figure}
  ...
  \caption{Impact of optimizations and comparison against other tools.}
...
%
Figure~\ref{fig:synthesis-times} shows the runtimes of various algorithms on our
test suite.
Our algorithm took a number of optimizations to become tractable.

Conversion to DNF form was a very important optimization we performed.  Trying
naive type-directed synthesis on these languages doesn't work at all, none of
the examples, even the very simple ones, are able to be synthesized within the 2
minute timeframe.
...
caused the search space, only \afm{12} could be solved simply with DNF
conversion, the rest required additional optimizations.

Inferring expansions gives the fanout less of an impact.  Inferring the forced
expansions alone makes almost all the synthesis tasks tractable.  This is
because inferring forced expansions, in many cases, infers \emph{all} the
expansions.  In \afm{15} of the \afm{26} examples, all expansions were forced.
This allows most of the examples to be tractable, but rather slow.  This is able
to be sped up by directing the search using more than just blind breadth-first
search, when more information can be used to direct it.
...
infer:  \CF{aug/xml} and \CF{cust/2-cap-prob}.

The example \CF{aug/xml} was slowest because it was both a large data format,
a large number of expansions were required, and not all expansions were
inferred.  While a fairly large number of expansions were forced (15),
there were 3 expansions that were inferred, but not forced, and 1 expansion that
had to be found through random search.  This is not always a problem, however,
as there were other formats that required even more search, both random search
and directed search.  What differentiated \CF{aug/xml} was that it also was a
very large data format: it had the \afm{second} largest lens and the
\afm{seventh} largest specification.  The amount of random search it required
had a larger impact because, with an increased specification size, there is also
an increase in the fanout, with additional iterations that can be unfolded, and
additional user defined data types that can be replaced by their definitions.
This large fanout, alongside the depth that needed to be searched through, made
the search very difficult, requiring \afm{164} types to be proposed before the
correct one was found.\footnote{Since xml syntax is regular,
