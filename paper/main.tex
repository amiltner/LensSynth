...
However, we elide such user-defined regular expression information
...
is compatible with our lens synthesis strategy, namely:

\begin{itemize}
\item The rewrite system should be \textit{confluent}.  
\item Whenever $\Lens \OfType{} \Regex \Leftrightarrow \RegexAlt$ and 
$\Regex \to \Regex'$ then there exist $\RegexAlt'$ and  $\Lens'$ such
...
For our purposes, we take the \textit{definitional equivalence  rules} $\DefinitionalEquiv$ to
 be the semiring axioms, plus these two ``unrolling'' rules that
...
This choice of definitional equivalence rules is the coarsest subset
of the regular expression equivalence rules we could find that still
is confluent and can be mirrored.  There may be coarser equivalence
relations,~\footnote{We have not been able to prove that it is the
  coarsest such relation.} but it was sufficient to cover all of the
test cases that appeared in our experiments (see \S\ref{evaluation}).
