...
While we do not know of any previous efforts to synthesize bi-directional
transformations from examples, there is a lot of other recent research on
synthesizing uni-directional string transformations~\cite{?}.  Much of that
work assumes the synthesis engine is provided with a collection of
examples.  Our work differs in that we assume the programmer supplies
both examples and format descriptions in the form of regular expressions.
...
and, of course, it requires some work to do so.
...
allows the system to handle complex large, complex formats.  
On the
latter point, FlashFill~\cite{gulwani-popl-2014}, for instance, 
...
We applied FlashExtract to the problem
...
extract the desired fields.  Our system succeeds without any examples
because all the necessary information is encoded in the format specifications.

There are also many other recent efforts to synthesize functions
from type-based specifications~\cite{?,?,?}.  While our work
was certainly inspired by others working in this space, we
needed to develop new kinds of techniques to manage
the multi-dimensional search space generated by 
regular expression equivalences and other aspects of 
lens combinator languages like Augeas and Boomerang.
Hence, there is little, deep, technical overlap with past type-directed
