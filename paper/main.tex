...
\begin{abstract}
  ...
  search by defining a new language of lenses governed by normalized
  regular expression types. We prove this language sound and complete
  with respect to \afm{bijective lenses derived from the combinators used
  in Boomerang}\sout{a standard language of bijective lenses}\bcp{I don't know
    what this means.}.  We then
  show how to search the reduced space for well-typed lenses that are
...
data are found everywhere in software systems---in serializers and
...
languages based on the theory of {\em lenses} tend to come with restrictive
typing constraints such as unambiguity that are tricky for programmers to
work with.
...
\item Specifically, we present an algorithm for synthesizing bijective lenses
between pairs of string data formats.  Inputs to the algorithm are the
schemas for the two formats, expressed as regular expressions, plus a few
concrete examples to guide synthesis.  The output is a well-typed lens that
...
nor do they have type systems strong enough to ensure invertibility.
Example uses of lenses in practice include Augeas~\cite{augeas}, a
configuration editing system for linux that uses lens combinators to
provide editable views of configuration files; the biXid~\cite{bixid}
language that expresses bidirectional XML transformations, and
XSugar~\cite{xsugar} that converts bidirectionally between XML and
non-XML data and between EpiDoc~\cite{epidoc} and
Leiden+~\cite{leidenplus} formats for encoding ancient
documents~\cite{epidocleidenplus}.

Unfortunately, writing lenses presents a number of challenges.  First,
users must learn a lens language and its type system.  Second, they
must adopt its unorthodox programming style that requires thinking in
two directions at once.  Finally, they must learn how to work within
the restrictive type system that ensures invertibility.  These
difficulties can disincentivize programmers from adopting the
technology, despite its advantages.

To overcome these difficulties, we have developed a system for
synthesizing bijective string lenses from the types of the source and
target data formats and a collection of representative examples.
The language we use for lenses has
typing judgments of the form
$\Lens \OfType \Regex \Leftrightarrow \RegexAlt$, which may be read
``lens $\Lens$ converts between data in the language of
regular expression $\Regex$ and data in the language of regular
expression $\RegexAlt$.''  Given a type
$\Regex \Leftrightarrow \RegexAlt$ and a small number of
representative examples (sometimes no examples at all!), we synthesize
$\Lens$, a well typed lens that implements a bijection between the
languages of $\Regex$ and $\RegexAlt$ while matching the provided
examples.

Our work is related to recent efforts that synthesize various types of
...
expression with type $A \rightarrow B$, such algorithms assume that
the expression in question is a function of the form $\lambda
x{:}A. e$.  These algorithms then search for an expression $e$ with
...
To explain the components of our synthesis algorithm,
we use a running example that converts between citations given
in simplified BibTex and EndNote formats.  The BibTex format
...
In our system, users write lens synthesis specifications by giving
regular expressions that define the source and target formats
% \footnote{Lenses
...
%  the ``target.''}
and by giving a suite of input/output examples.
Figure~\ref{fig:bibend-spec} shows an example specification for a
lens \CF{BibEnd} that converts between the
\CF{Bibtex} and \CF{EndNote} formats.
We allow users to name intermediate regular expressions
(\CF{Name}, \CF{Names}) to facilitate  
the construction of complex formats.
...
and $\Lens_2$ to the second to yield $t_1$ and $t_2$, respectively.
...
and \emph{put} functions form a bijection via the typing rule:
\[
  \inferrule
  {
...
\]
The notation $\UnambigConcatOf{\Regex_1}{\Regex_2}$ asserts that $\Regex_1$ and $\Regex_2$ are
\textit{unambiguously concatenable}, meaning there is
...
and $\String_2$ is in the language of $\Regex_2$.  These 
contatentation constraints guarantee that \emph{get} and \emph{put}
are bijective functions if 
the lenses $\Lens_1$ and $\Lens_2$ are bijective.

Now consider searching for
a lens with the type $(A B) C \Leftrightarrow A' (B' C')$ 
given lenses with types 
...
we are stuck---we have no lenses with those types.  
...
\]
With such a rule, we could 
convert the type $(A B) C \Leftrightarrow A' (B' C')$
to the better-aligned $(A B) C \Leftrightarrow (A' B') C'$.
With this type, structure decomposition using the
concatenation rule generates sub-problems that require
finding lenses for 
$A \Leftrightarrow A'$, $B \Leftrightarrow B'$, and 
$C \Leftrightarrow C'$,  exactly what we have.

Because regular expressions equivalence is decideable, 
...
difficult because
there are an infinite number of regular
expressions that are equivalent to the source and target regular
expressions. 

A second challenge arises from composition lenses.  The typing of such
lenses requires the target type of the first lens to be the source
type of the second.  To synthesize a composition lens between
...
regular expression $\Regex_3$ is difficult because
the search space is infinite.  We cannot simply omit composition
lenses from our langauge because their removal reduces the expressivity
of the language.
...
In essence, naively applying type-directed synthesis techniques
involves searching \emph{in three infinite dimensions}:  we must
search for
(1) a regular expression \emph{type} with an appropriate ``syntactic shape,'' 
(2) a lens \emph{expression} that has the given type and is consistent
with the user's examples, and 
(3) intermediary regular expressions for composing lenses.

\paragraph*{Solution 1: Canonization}
Our solution is to define a new language of lenses that
reduces the synthesis search space \emph{in all dimensions}.
We designed this language so that its regular expression types are limited 
to those in a normalized form.  This normalized form
demands that regular expressions be fully distributed and removes all
...
With this approach, many, but not all, regular expression equivalences that
prevent type-directed lens synthesis disappear.
...
unrolling of Kleene Star.  \afm{The source and target types of lenses are not
...
\footnote{\BCP{Why not?  More generally, why
...
The specialized lenses between our normalized 
forms are expessive enough that they are closed under composition
\emph{without adding a composition order}.  Obviating the need for a
composition operator reduces the search space by an entire dimension
because the synthesis algorithm no longer needs to search through
possible intermediary regular expressions for compositions.

\paragraph*{Challenge 2: Large Types}
The types describing data formats may be very large --- they are
regular grammars describing complex data formats.  On the one hand,
the size of these types is very useful as such types provide a wealth
of information that constrains the search process effectively.  On the
other hand, \afm{the algorithm must be able to guarantee that the generated lens
  satisfies the many constraints introduced by these large, complicated types}
\sout{there is a lot of data that must be managed carefully by
the synthesis system}.\footnote{\ksf{Having to manage a lot of data
carefully is not a compelling difficulty.  Is there another way of
expressing this challenge?}}

\paragraph*{Solution 2: Compositional Types}
The key to solving this challenge is to observe that users naturally
construct large types compositionally, using named abbreviations for
subcomponents.  We exploit the structure inherent in these named
abbreviations to speed up the search dramatically.  For example, if
the name $A$ appears once in both the source and the target, the
system hypothesizes that the identity lens can be used to convert
between occurrences of $A$.  On the other hand, if the name $A$
appears in the source, but not in the target, the system replaces the
name $A$ with its definition.  Furthermore, differing numbers of
occurrences of names serve as a guide for applying other regular
expression equivalence axioms in the synthesis algorithm.  To capture
this intuiton, we define a pseudometric based on the occurrence of
names to guide the transformation of regular expression types.  This
pseudometric provides a heuristic to guide the search, which we have
found to perform better than a simple breadth-first search.
...
a system for the synthesis of bijective string lenses.  In addition,
we make the following contributions: 

\begin{itemize}
\item We define a new domain-specific lens language designed specifically
  for synthesis.  This language uses pseudo-canonical regular
  expressions to type the source and target data formats. Its term
  language maps between the languages of these pseudo-canonical
  regular expressions.
\item We formalize the semantics of this language and prove it sound and complete
  with respect to an existing language for bijective string lenses.
\item We present a type-directed algorithm for synthesizing lenses in our new
  domain-specific language, which can efficiently search through lens terms and
  type equivalences.
\item We evaluate the algorithm on 23 benchmarks, including
  examples from FlashFill and from the Augeas system for transforming
  linux 
  file formats.  Our synthesis algorithm succeeds on all of our
  benchmarks, with an average synthesis time of 0.134 seconds.
\end{itemize}
% end introduction
...
\BCP{When we remove stuff from the short version, we're not removing it
  ...
  all the stuff elided from the short version...}
\afm{for this I just removed it, for a lot of things I moved it into the
  appendix.  In future I will do the iffull strategy.}

We assume given an alphabet $\Sigma$; variables $s$ and $t$ denote 
...
\end{tabular}
\end{center}
The language of $R$, written $\LanguageOf{R}\subseteq \StarOf{\Sigma}$, is defined as usual.
...
A regular expression $\Regex$ is \textit{strongly unambiguous} if
\begin{enumerate*}[label=(\alph*)]
\item $\Regex = \String$, 
\item $\Regex = \SetOf{}$, 
\item $\Regex = \Regex_1 \Concat \Regex_2$ with $\Regex_1 \UnambigConcat
  \Regex_2$ and $\Regex_1$ and $\Regex_2$ strongly unambiguous,
\item $\Regex = \Regex_1 \Or \Regex_2$ with $\Regex_1 \Intersect \Regex_2 =
  \emptyset$, and $\Regex_1$ and $\Regex_2$ strongly unambiguous,
and
\item $\Regex = \StarOf{(\Regex')}$ with $\UnambigItOf{(\Regex')}$ and
  $\Regex'$ strongly unambiguous.
\end{enumerate*}
...
\bcp{This section confusingly blurs the distinction between equational
  theories and systems of rewrite rules.}

Conway's axioms consist of the semiring axioms \afm{of associativity of
\OrRegexType{} and \ConcatRegexType{}, commutativity of \OrRegexType{},
distributivity, and the existance of \OrRegexType{} and \ConcatRegexType{}
identities.  They also consist of}\bcp{summarize them in words},
\sout{and} the following 
...
to be confluent\bcp{say why}.  \afm{If two lenses were composed together, and
  ...
  there would be no way to combine the derivations.}
...
$\Regex'$ and $\RegexAlt'$.  While \afm{the rewrite rules corresponding to both
  the forward and the backward directions of each equivalence of }Conway's
axioms are confluent \afm{as the reverse rewrite can always be taken}
\BCP{I thought we didn't know this}, they don't
...
\begin{figure}
  ...
  \caption{Lens Typing\bcp{We can save quite a few lines by moving the rule
      names}\afm{What do you mean by this?}}
...
\begin{center}
 ...
 &&                                           $\hspace*{2em}(\String_i,\StringAlt_i)\in\SemanticsOf{\Lens}}$\\
  ...
  \end{tabular}
\end{center}
\bcp{We should either write composition with a ; or else define it's
  semantics the other way round.}
...
BibTex':
  $\EmptyString$ 
  | ($\mathit{author=}$ Name $\mathit{,}$ Name Names
    ($\mathit{\_and\_}$ (Name $\mathit{,\_}$ Name Names))* $\mathit{\},}$)

EndNote':
  ($\mathit{\%A\_}$ (Name Names $\mathit{\_}$ Name) $\mathit{\\n}$)*

DNFBib':
  $\EmptyString$
  | $\mathit{author=\{}$ Name $\mathit{,\_}$ Name $\EmptyString$ Names $\EmptyString$
    ($\mathit{\_and\_}$ Name $\mathit{,\_}$ Name $\EmptyString$ Names $\EmptyString$)* $\mathit{\},}$

DNFEnd':
  $\EmptyString$ ($\mathit{\%A\_}$ Name $\EmptyString$ Names $\mathit{\_}$ Name $\mathit{\\n}$)* $\EmptyString$

DNFEndUnroll:
  $\EmptyString$
  | $\mathit{\%A\_}$ Name $\EmptyString$ Names $\mathit{\_}$ Name $\mathit{\\n}$
    ($\mathit{\%A\_}$ Name $\EmptyString$ Names $\mathit{\_}$ Name $\mathit{\\n}$)* $\EmptyString$

DNFEndReplacement:
  ($\mathit{\%A\_}$ Name $\EmptyString$ ($\mathit{\_}$ Name)* $\mathit{\_}$ Name $\mathit{\\n}$)* $\EmptyString$
\end{lstlisting}
...
\bcp{All the double quotes make the examples quite hard to read.  Could we
  simplify the formats to eliminate them, for purposes of this example?}
  }
\end{figure}
...
\begin{center}
  \begin{tabular}{l@{}l@{}c@{}l@{\ }>{\itshape\/}r}
    ...
    (DNF Reg-& \DNFRegex{},\DNFRegexAlt{} & \GEq{} & $\DNFOf{\Sequence_1\DNFSep\ldots\DNFSep\Sequence_n}$ & \MultiOrDNFRegexType{} \\
    exs)
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{R@{}L}
    ...
    \LanguageOf{\SequenceOf{\String_0\SequenceSep\Atom_1\SequenceSep\ldots\SequenceSep\Atom_n\SequenceSep\String_n}}= &
                                                                                                                        \{\String_0\Concat\StringAlt_1\Concat\ldots\Concat\StringAlt_n\Concat\String_n \\
    & \hspace*{2em}\SuchThat \StringAlt_i\in\LanguageOf{\Atom_i}\}\\
    \LanguageOf{\DNFOf{\Sequence_1\DNFSep\ldots\DNFSep\Sequence_n}}= &
                                                                       \{\String \SuchThat \String \in \LanguageOf{\Sequence_i}\\
    & \hspace*{2em}\text{ for some $i\in\RangeIncInc{1}{n}$}\}
  \end{tabular}
\end{center}
...
\begin{figure*}
  ...
  \end{mathpar}
  \caption{DNF Regex Rewrite Rules\bcp{Figure belongs on previous page.  And
    the last three rules are not needed: just say that the star relation is
    the reflexive and transitive closure of the unstarred one.}}
...
We can now define a syntax for lenses between DNF regular
expressions, called {\em DNF lenses}.  
\begin{center}
  \begin{tabular}{@{}l@{\ }l@{\ }c@{}l@{\ }>{\itshape\/}r@{}}
    % REGEX
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \EndFunction
  \end{algorithmic}
\bcp{Minor comment: we can save several lines by removing ``end if'' and
  such.  Larger comment: Is this seriously using {\em null} to
  represent a failure result from a subroutine??  The reviewers are going to
  laugh at us.}\afm{is $\bot$ better?}
\end{algorithm}
