...
This approach 
ensures that the resulting functions are correct
while
reducing programmer tedium. 
Specifically,
...
regular expressions representing the two data formats
...
%
The main technical challenges our algorithm addresses are 
(1) the combinatorial explosion caused by the uniqueness conditions
imposed by lens typing rules, and
...
long as possible. 
%only replacing  user-defined regular expressions by their definitions when non-identity  transformations must be applied to them.
...
source format.  
A benefit of lens-based languages is that they use a single expression
to express both 
...
\ksf{Should we give our system a name?  It
would make some of the subsequent text less awkward and allow others
to talk about our work more easily.}
%
...
instead of asking programmers to write lenses explicitly.
%
...
Figure~\ref{fig:schematic-diagram-synthesis} shows our synthesis
algorithm, which makes essential use of DNF regular expressions and DNF lenses.
...
\paragraph*{Contributions.}  Our synthesis algorithm makes
...
After we briefly review some background on regular expressions and core
...
(\S\ref{sec:dnf}) based on pseudo-canonical DNF regular expressions  
(\S\ref{sec:dnfre}).  
We show how to convert ordinary regular expressions and lenses
into the corresponding DNF forms. 
We prove that DNF lenses are sound and complete with
respect to our high-level lens syntax.

\item We present an efficient, type-directed synthesis algorithm for
    synthesizing DNF lenses (\S\ref{algorithm}).  We prove that this
    algorithm gives rise to a sound and complete synthesis algorithm
    for lenses.  \ksf{are we stating this soundness/completeness
    result twice or is there a difference between the result in this
    bullet and the one in the previous bullet?}

  \item We evaluate our core algorithm, its optimizations, and
    existing synthesis tools on \afm{26} benchmarks, including
    examples derived from Flash Fill~\cite{gulwani-popl-2014} and the
    Augeas~\cite{augeas} system (\S \ref{evaluation}).  We show our
    algorithm significantly outperforms previous systems.
    We show our optimizations are critical for synthesizing many of
...
Figure~\ref{subfig:vsts-arch}.  To avoid errors and expedite the
process, we would like synthesize this lens.
\bcp{Will our system really make it all that much better?
...
functionality proivded by \OrLens{} lenses.  The typing
...
However, the rule above does not suffice to type check lenses that map 
...
by representing types as {\emph regular languages}, not regular expressions.
We formalize this behavior by allowing conversions between equivalent
...
This algorithm is complete, \ksf{What algorithm?  The one in Fig.3?
That algorithm doesn't include the conversion to DNF, does it?  Is
Fig. 3 the ``SynthLEns'' box in Fig. 1?  If so, say so and say the
algorithm is the one given in Fig 1.}
but the conversion to DNF incurs an exponential blowup.  In practical
...
Before formally defining DNF regular expressions and DNF
lenses, we first breifly review the formal definitions of regular
expressions and lenses.
...
\ksf{It is confusing to have R and T range over regular expressions
but s and t range over strings.  In particular, the statement
...
$\String_1=\StringAlt_1$ and $\String_2=\StringAlt_2$'' is hard to
parse.
It would be easier if it read
``for all strings $s, s'\in\LanguageOf{S}$ and
$t,t'\in\LanguageOf{T}$, whenever
$s\Concat t=s' \Concat t'$ we have
$s=s'$ and $t=t'$''
The difference is that the string meta-variables s and t are easily
linked to the regular expression meta-variables S and T.  Currently,
we have string meta variables s1 and t1 coming from regular expression
R, but s2 and t2 coming from regular expression T. 
}
...
$\String_2$.
\ksf{In the iterate form, should this be unioned over all values of n?}.
\begin{center}
  \begin{tabular}{@{}rcl}
...
\fi
  ...
  \caption{Lens Typing Rules \iffull\else (except $\mathit{or}$ and
 $;$)
      \ksf{why are we omitting or and ;?  Should the caption say
 ``Selected typing rules''?} \fi}
...
\ksf{The rest of this section doesn't fit with a preliminary
section focused on ``formal definitions of regular expressions and
lenses.''  These sections are confusing because the motivation is all
about the synthesis algorithm that we haven't introduced yet.}
\ksf{I think we should introduce another section entitled ``algorithm
overview'' at this piont that describes the high-level view of the
algorithm.  In describing it, the issue of equivalence and rewriting
would come up naturally and we would have been able to introduce the
retyping rule informally.  }

\SUBSECTION{Definitional Equivalence and Rewriting}

Type-directed synthesis seeks to construct a lens by searching for a
well-formed typing derivation using the rules in
Figure~\ref{fig:lens-typing}.
\ksf{Do we not use or and ;?  or do we search over all the rules?  If
we are omitting or and ; from synthesis, we should explain why.}
...
composition (we describe the necessary properties shortly).
...
\Leftrightarrow \RegexAlt$).  \ksf{The reader doesn't know what the
retyping rule is.  }
...
large fan-out.  While unable to infer all the examples, it is able to
infer a large number: a maximum of 26 expansions, and a median of 11
expansions.  \ksf{I don't understand what the previous sentence is
saying about a maximum and minimum number of expansions.}
...
\ksf{``lenses in the context'' seems like a complex way of saying we
can leverage a library of existing lenses. Why not say these lenses
belong to a library of existing lenses instead of saying they are in
the context? } 
