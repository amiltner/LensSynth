...
problem'' \cite{chen2010comparative}.\BCP{We persistently mix
  ...
  {\em bijective} lenses are notm really.}
...
types and examples, rather than 
...
We wrote \ksf{3} custom examples to test various features of the
algorithm.
In Figure~\ref{fig:evaluation-data}, examples derived from Augeas are
prefixed with ``aug'', those from FlashFill with ``ff'', and those
written by hand by ``cust.''
...
right-hand sides to strings that corresponded to serialized versions
of the tree formats.  
...
right-hand side to make the transfomations bijective and thus suitable
for our study.
...
\ksf{Anders, we need to have a paragraph describing the synthetic
benchmarks and what we created them to evaluate.}

\ksf{Anders, we need to have a paragraph describing the sizes of these
benchmark programs.  The paragraph needs to be concrete, talking about
the sizes of the source/target regular expressions and/or the size of
the synthesized lenses.}
...
seconds}; the slowest example (aug/xml) completes in just under four 
seconds. 
\ksf{Say something about how the running time relates to the
complexity of the benchmark program once we have that data available.}

The example aug/xml takes as long as it does primarily because of the
...
We constructed the pathological example \ksf{cust/double-capital} to
...
hence the name ``double.''  In this example, the conversion to DNF
