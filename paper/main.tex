...
To determine if one expression is less than another, the algorithm sorts the
subterms of the expressions, and applies the lexicographic ordering to these
sorted subterms. 
...
\ksf{Is the code publicly available?  (or will it be once we don't
have to preserve anonymity?  We should say something about it here.}
...
dictionary representation. \ksf{Arbitrary XML of a fixed depth?}
...
\item[\textbf{DNF:}]  The synthesis algorithm performs type-directed synthesis
  ...
  derivation for base lenses. \ksf{What are base lenses?  Do you mean
  unrestricted lenses where equalities can be applied anywhere?}
...
were forced.  However, the remaining \afm{11} \ksf{these numbers don't
add up!} still require enumerative search.
...
\footnote{Since xml syntax is regular \ksf{context-free?}, the source and target regular
...
\begin{figure}
  ...
  have any data to tell us whether we picked the right one or not?}}
  \label{fig:exs-reqd}
\end{figure}
...
than either existing tool.  Unlike these two tools, which must spend  
effort figuring out the shape of the data, Optician{} avoids
this problem by having the user supply the data formats.
