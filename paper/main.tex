...
Specifically,
we present \Optician{}, an tool for {\em type-directed synthesis} of bijective
...
abbreviations to speed up the search through dramatically.  For example, if
...
\TextChar{} must be substituted by its
definition.  In this way, the positions of names
can serve as a guide for applying both substitutions and rewrite in the
...
two-way conversions between the formats.  Of course, 
this focus on bidirectional transformations limits the set of
functions we will synthesize to bijective ones. 
...
search procedures to process only terms that are
well-typed and match the specifications.
Our system is distinctive in that it supports many type equivalences.
The subsumption rule can always be used, and finding the
correct supertype introduces another infinite search.
Only \InSynth{}~\cite{gvero-pldi-2013} works on a system with many equivalences
on the types.  \InSynth{} takes a similar approach of creating an alternative
language which identifies equivalent types.
Our types are much more complex; our system handles the equivalences of a star
semiring instead of those of cartesian closed category.
Furthermore, the relationship between types and terms in Boomerang does not
permit us to identify all equivalent terms. Instead \Optician{} must
process some equivalences on the types themselves.

\Morpheus{}~\cite{morpheus} is a synthesis system which also uses two
communicating synthesizers to generate programs.  In \Morpheus{} and \Optician{}, 
one synthesizer provides an 
outline for the program (as a sketch in \Morpheus{}, as a DNF regular expression
pair in \Optician{}), and the other fills in that outline with program
details which satisfy the user's specifications.
This approach works well in large search spaces, which
require some enumerative search.  Using two synthesizers allows the first
synthesizer to perform this enumerative search, and the second to fill in the
details ensuring the specifications are satisfied.
However, to na\"ively search through Boomerang lenses 
...
language closed under composition, all applications of subsumption can be pushed
to the end of the typing derivation.
...
language for synthesis -- DNF lenses.  However, we provide the capabilities to
...
Furthermore, we also provide completeness results stating exactly which terms of
Boomerang we are able to synthesize.
...
We use \StringIntListSet{} to denote a set of string and int list pairs.
...
parse trees for a number of strings which match it embedded.  We build the
...
$\ExampledAtom$.
  ...
  DNF regular expressions, exampled sequences, and exampled atoms.

  Let $\leq$ be an ordering on $A$.  Let $\ListOf{x_1,\ldots,\x_n}$ be a list of
  $A$s.  Define $\SortingOf{\ListOf{x_1,\ldots,x_n}}$ as a permutation $\sigma
  \in \PermutationSetOf{n}$ such that $\sigma(i) \leq \sigma(j) \BooleanImplies
  x_{\sigma(i)} \leq x_{\sigma(j)}$.

  Let $\leq$ be an ordering on $A$.  Define 

  Let $\leq$ be an ordering on $A$.  Define $\DictionaryOrderOf{\leq}$ as the
  ordering on $\ListTypeOf{A}$ as:
  $\ListOf{x_1;\ldots;x_n} \DictionaryOrderOf{\leq} \ListOf{y_1;\ldots;y_m}$ if
  $n < m$, or if $n = m = 0$, or if $n = m > 0$ and $x_n \leq y_n$ and
  $\ListOf{x_1;\ldots;x_{n-1}} \DictionaryOrderOf{\leq}
  \ListOf{y_1;\ldots;y_{n-1}}$.
  ...
  Let $\leq$ be an orderin on $A$.  Define
