...
\usetikzlibrary{er,matrix,shapes,arrows,positioning,fit,calc,pgfplots.groupplots}
...
iteration that other synthesis algorithms preclude), and constrains the
...
the second phase of the typing derivation aligns the terms with
the types.
We have then proven this language equivalent to our bijective fragment of
Boomerang.
...
lens combinators (\S\ref{background}),
...
\begin{figure}
  ...
  \begin{tikzpicture}[auto,node distance=1.5cm]
    ...
    \draw[<->] (old-server-api.south)
    -- ($(old-server-api.south)+(0,-.15)$)
    -| ($(old-server-api.south)+(1.13,-.15)$) node[below] {Lens} 
    -| ($(new-server-api.north)+(-1.63,.15)$)
    -| ($(new-server-api.north)+(-.50,.15)$)
    -- ($(new-server-api.north)+(-.50,0)$);
    \node[fit=(old-server-api) (new-server-api) (server-stack) ,draw, inner sep=.8em] (server) {};
    %\node (synth-name) [below=0cm of surrounding] (name) {\SynthLens{}};
  \end{tikzpicture}
...
<WorkItem>
 ...
 <Field Id=2>Return 400 on bad PUT request</Field>
 <Field Id=5>J Smith</Field>
</WorkItem>
\end{lstlisting}
\end{minipage}
}
...
{
  Title: Return 400 on bad PUT request,
  Type: Bug,
  AssignedTo: J Smith,
}
\end{lstlisting}
...
\savestack{\listingB}{\box0}
  ...
  \resizebox{\columnwidth}{!}{%
    \begin{tabular}{|>{\columncolor{lightyellow}}c|>{\columncolor{lightgreen}}c|}
      \hline
      Legacy Task Representation & Modern Task Representation \\
      \hline
      \listingA & \listingB \\
      \hline
    \end{tabular}
  }
  \caption{Idealized Task representations from legacy and modern web service endpoints}
  \label{fig:idealized-representations}
\end{figure}

Visual Studio Team Services is a web service for
...
new, modernized APIs, the old, legacy APIs must be maintained for continued
support of legacy
...
Figure~\ref{fig:vsts-arch}.  Because of the large number of existing service
endpoints, writing these converters will be a pain, so we would like to be able
to synthesize them.

Consider a ``Task'', a resource that represents a piece of work given to a
team.  Idealized versions of the representations of tasks from the new and
old apis are given in Figure~\ref{fig:idealized-representations}.  If an old
client performs a GET request on a task, it first retrieves that task using the
modern API, and then uses the
lens's \PutLeft{} function to convert this task into the legacy format.  Similarly,
if an old client performs a PUT request a task, it first uses the lens's
\PutRight{} function to convert that task into the
new format, and then inputs that task through the modern APIs.  The two
...
\CF{{\color{blue} ""} | {\color{darkbrown} text\_char}+}, and then we must
...
& \Regex{},\RegexAlt{} & \GEq{} & \hspace{1.06em}s \\
          & & & \GBar{} $\emptyset$ \\
          & & & \GBar{} \Regex{}* \\
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \Case {\SomeOf{\DNFLens}}
    \State $\ReturnVal{\IterateLensOf{\DNFLens}}$
    \EndCase
    \Case {\None}
    ...
    \Case {\SomeOf{\ListOf{\AtomLens_1;\ldots;\AtomLens_n}}}
    \State $\ReturnVal{\SomeOf{(\SequenceLensOf{(\String_0,\StringAlt_0);\AtomLens_1;\ldots;\AtomLens_n;(\String_n,\StringAlt_n)},\sigma)}}$
    \EndCase
    \Case {\None}
    \State $\ReturnVal{\None}$
    \EndCase
    ...
    \Case {\SomeOf{\ListOf{\SequenceLens_1;\ldots;\SequenceLens_n}}}
    \State $\ReturnVal{\SomeOf{(\DNFLensOf{\SequenceLens_1;\ldots;\SequenceLens_n},\sigma)}}$
    \EndCase
    \Case {\None}
    \State $\ReturnVal{\None}$
    \EndCase
...
benchmark programs derived from practice.  In this evaluation, we describe the
construction of our benchmark suite, investigate the impact of our optimizations,
discuss the importance of examples, and provide a
...
\begin{figure}
  \includegraphics{SizeVsTime}
  \caption{Relation between lens size and synthesis time}
  \label{fig:size-vs-time}
\end{figure}

\afm{This whole paragraph needs work.  Main point I want to say: We are doing
  hard problems, synthesizing very complex lenses, this is evidenced by the fact
  that the number of AST nodes in the synthesized lenses is large.}
Our examples were fairly complex.  Figure~\ref{fig:size-vs-time} shows the sizes
of our generated lenses, as well as the amount of time that it takes to generate
those lenses.  \afm{Ok, I don't really think the size vs time makes perfect
  sense here.  I want to include size somehow, but can't think of some y value
  that makes sense.  Maybe I could do spec size vs lens size?} We generate lenses of sizes between 5 AST nodes, for simple
problems like changing how dates are represented, and 211 AST nodes, for complex
tasks like transforming XML to a dictionary representation.  Furthermore, we
scale as expected, with more complex tasks taking longer \afm{Again, this
  doesn't really go in here.  I'm not even sure there is a place to say it,
  should it just be omitted?  It doesn't really add to any of the main points
...
\begin{figure}
  ...
  \caption{Number of benchmarks that can be solved by a given algorithm in a given
    ...
    synthesis on Boomerang combinators.}
...
\begin{figure}
  \includegraphics{InferencesDone}
  \caption{Number of expansions inferred for tasks requiring a given number of
    ...
    {\bf Forced Only} only counts forced inferences.}
  \label{fig:inferences-done}
\end{figure}
...
Inferring expansions gives the fanout less of an impact.
...
\emph{all} the expansions.  In \afm{15} of the \afm{26} examples, all expansions
were forced.  This allows most of the examples to be tractable, but rather slow.
This is able to be sped up by directing the search using more than just blind
breadth-first search, when more information can be used to direct it.  Adding in
...
existing lenses either written by the user, or previously synthesized.
...
\begin{figure}
  \includegraphics{AllPaths}
  \caption{Number of examples required to exercise all codepaths of benchmark
    programs.}
  \label{fig:all-paths}
\end{figure}
...
Figure~\ref{fig:all-paths} lists
...
input of pairs of regular expressions.  However, by requiring this additional
work by the user, \SynthLens{} is able to synthesize significantly more programs
