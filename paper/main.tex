...
\begin{abstract}
\bcp{We need to carefully check that we're using the right format.  My
  reading of the submission instructions is that we should use the
  ``acmsmall'' option to the ``acmart'' class, but we were using
  ``acmlarge''.}
  % % begin abstract 
...
can be found all over modern software systems, in serializers and
...
requires learning a new language and paradigm.  \bcp{This criticism isn't
  very convincing.  I think a better approach is ``Writing lenses by hand is
great.  But synthesizing them is even better...''}
...
Boomerang program that behaves as specified by the given examples.
%
The main technical challenges our algorithm addresses are (1) 
combinatorial explosion in the search space size caused by the language
equivalence relation on regular expressions\bcp{Needlessly puzzling (why should
  language equivalence cause a combinatorial explosion?)} and the sequential
composition operator on
lenses\bcp{ditto} and (2) the size and complexity of the regular expressions
describing real-world data formats.  We address these challenges by moving the
search to a different space where both regular expressions and lenses are
constrained to a ``disjunctive normal form'' and treating user-defined
names for regular expressions semi-opaquely\bcp{That word will be
  semi-opaque to readers...}.

We evaluate our algorithm on a benchmark suite of 26\bcp{more now?} examples,
including both
...
The growing importance of big data makes tools for reliably parsing,
printing, cleaning, and transforming data in {\em ad hoc} formats
increasingly important.\bcp{Why does ``big'' imply ``ad hoc''?}  In what is known as the classic
``view update problem''~\cite{Bancilhon81, Dayal82},
programmers often need to reliably transform source data into a target
format (or view) and transform the data from the view back into the source
format.\bcp{Experts may be confused by this: We are not handling the full
  view update problem, just a bijective version of it.}
\emph{Lenses}~\cite{Focal2005-long} solve this problem\bcp{which problem? in
what sense do they really/fully ``solve'' it?}
by providing strong guarantees about the round-trip behavior of the data
transformations.  These guarantees help guard against data corruption while
reading, editing, and writing data sources.  

A lens comprises two functions,
\emph{get} and \emph{put}.  The \emph{get} function translates
source data into the view.  If the view is updated, the
\emph{put} function can translate the edited data back into the
source format.  \bcp{Rest of paragraph is confusing.}In lens-based languages, the \emph{get} and
\emph{put} functions are both represented
by a single expression that can be interpreted in two different ways.  When
these expressions are 
well typed, the resulting functions are guaranteed to satisfy the desired
invertibility properties.

Such\bcp{?} lens-based languages have been applied in a variety of practical
settings.\bcp{The rest of the paragraph doesn't support that assertion.}
Boomerang~\cite{boomerang, Matching10} is a well-researched tool for providing
...
files, uses the \emph{get} part of a lens to transform them into a simple,
uniform tree representation that can be edited either manually or
programmatically, and then uses the lens's \emph{put} to merge the edited
...
%
and many others.  \bcp{There are probably some new ones from Tokyo, by
  now...  We should check.}
...
time---a tricky task for most programmers.\bcp{I don't really find either of
these asertions convincing.  The DSL for lenses is pretty small and simple.
And in the bijective case there is no need to think about both directions at
the same time.}  In addition, the details are
often fiddly, making lens programming a slow and tedious task.
For example, Boomerang
programmers must often rearrange the order of data items by recursively
using operators that swap the order of adjacent fields.\bcp{This is more
  convincing to me, but I doubt all readers will understand the explanation
  of what's fiddly.}  The
challenge of learning these languages and using them effectively may have
slowed\bcp{weak} 
their adoption in many settings where well-behaved bidirectional transformations
are needed.

To make programming with lenses languages faster and easier, we have
developed a
tool for synthesizing lenses from simpler, higher-level specifications
...
Specifically, our system supports the synthesis of \emph{bijective lenses}, a
useful subset of Boomerang.
As inputs, it takes specifications of the source and target formats, plus
(optionally\bcp{confusing to emphasize this here}) a collection of concrete
examples of the desired 
transformation.  Format specifications are supplied as ordinary regular expressions.
...
than the unfamiliar syntax of lenses.\bcp{Again, I think we're insisting too
much on this.}  Moreover, including these format
descriptions communicates a
great deal of information to the synthesis system.  Hence, this
design choice\bcp{which one? (also, I don't really understand ``hence''...)}
appears to make synthesis very robust, helps the system scale
to handle large and complex data sources, and constrains the
search space sufficiently that the user typically needs to give very few, if
any, examples.

Despite the benefits of such informative types,\bcp{what's before and after
  the comma don't fit together} certain features of
Boomerang make it ill
suited to synthesis in its raw form.  Boomerang's types are regular expression
pairs, and there are infinite equivalent regular expressions\bcp{???}.
To synthesize all Boomerang terms, we
must be able to traverse these equivalences.  Furthermore, composition is not
admissible in Boomerang, making it necessary to search through possible
central types.

To resolve these issues, we introduce new languages of ``Disjunctive Normal
Form Lenses'' and ``DNF
Regular Expressions''.  These
lenses and regular expressions loosely correspond to fully distributed lenses
and regular expressions\bcp{is the reader supposed to know what those are?}.
...
Furthermore, DNF lenses' typing derivations have two typing judgements to fix\bcp{?}
where and how equivalences on DNF regular expressions can be processed.
The first typing judgement processes equivalences on the types, and
the second judgement aligns the terms with the types\bcp{that sentence is mysterious}.
Despite DNF lenses' higly restrictive syntax and type system,
they are equivalent to our bijective fragment of Boomerang.

\begin{figure}
  \centering
  \begin{tikzpicture}[auto,node distance=1.5cm]
    \node[align=center] (spec) { Spec };
    ...
    \node (synth-name) [above=.5cm of middle] {\SynthLens{}};
    
    \node[fit=(sketch-gen) (sketch-compl) (start-alg) (synth-name) (failure) (success) ,draw] (surrounding) {};
  \end{tikzpicture}
  \caption{Schematic Diagram for Synthesis Algorithm}
  \label{fig:schematic-diagram}
\end{figure}

\bcp{Why is it important for the reader to know, here, that the synthesis
  algorithm has two phases?  (We're not explaining things in enough detail
  for people really understand what the two phases do---we just say that the
  first phase proposes something and the second phase does something with it
  and then we loop if needed.  Why is that useful to know at this
  point?)}The typing rules of DNF lenses lend themselves to a two-phase
synthesis 
algorithm.  A schematic diagram of this two-phase algorithm is given in
Figure~\ref{fig:schematic-diagram}.
\SynthLens{} takes a pair of regular expressions describing the data formats
as well as a list of input/output examples as input specifications.
By searching through the first phase of the typing derivation\bcp{huh? (what
  could it mean to search through a phase of a typing derivation??)},
\TypeProp{} 
proposes a pair of DNF regular expressions.
These DNF regular expressions are then given as input to \RigidSynth{}, which
efficiently searches through the second phase of the typing derivation.
If \RigidSynth{} successfully generates a DNF lens (\Success{}), this DNF lens is
then converted into Boomerang lens, and returned to the user.  If \RigidSynth{} fails to
generate a DNF lens (\Failure{}), then this failure is provided to \TypeProp{},
which then proposes another DNF regular expression pair.  This loop continues
until \RigidSynth{} successfully completes.
We use the two typing judgements to prove that our two-phase synthesis
algorithm is both sound and complete.\bcp{I think there's a category error
  in that sentence: it's like saying ``We use numbers to prove the prime
  factorization theorem.''}

\paragraph*{Contributions.}  Our synthesis algorithm is intended to make
bidirectional programming more accessible.  We begin by motivating our problem
with an extended, real-world example (\S\ref{motivation}).
After we briefly recapitulate some background on regular expressions and core
lens combinators (\S\ref{preliminaries}),
we offer the following technical contributions: \bcp{The way this bit is
  written makes it sound as though we are claiming ``pseudo-canonical DNF
  regular expressions and DNF lenses'' as our primary contribution, which
  comes off as rather weak---these things are not an especially exciting
  contribution in themselves: they're just a technical device to facilitate
  synthesis.  } 

\begin{itemize}
\item We introduce pseudo-canonical DNF regular expressions (\S
\ref{sec:dnfre}) and DNF lenses, a new variant of standard lenses designed
specifically for synthesis (\S\ref{sec:dnf}).  We show how to convert
between ordinary regular expressions and lenses and the DNF forms.  While it is
common to introduce alternative DSLs designed for synthesis, we have further
proven DNF lenses sound and complete with
respect to our high-level lens syntax (despite permitting an efficient
synthesis algorithm).

\item We present a sound and complete type-directed algorithm for synthesizing
  DNF lenses (\S\ref{algorithm}).  While applying type-directed synthesis
  to DNF lenses provides a substantial improvement over existing algorithms,
  it still proved insufficient for synthesizing many of the large format
  transformations in our benchmark suite.  We provide refinements to these naive
  algorithms to make these more difficult problems tractable.\bcp{This
    doesn't pack much of a punch either, after the first sentence (which is
    fine as far as it goes).  Basically the rest says, ``A naive algorithm
    isn't good enough so we propose a better one.''  Yawn.}

\item We evaluate our core algorithm, its optimizations, and synthesis tools in
  a similar domain on 26 benchmarks, including examples derived from
Flash Fill~\cite{gulwani-popl-2014} and the Augeas~\cite{augeas}
system (\S \ref{evaluation}).  Our full algorithm succeeds on
all benchmarks in under 5 seconds,\bcp{the parts of the sentence before and
  after the comma don't connect very clearly} despite competing tools finding fewer
than \afm{20\%} of the desired transformations.
\end{itemize}
...
\begin{figure}
  ...
  \begin{subfigure}[b]{.49\textwidth}
    ...
    \begin{tikzpicture}[auto,node distance=1.5cm]
      ...
      % server stack
      \node[text width=2cm,align=center,draw,rectangle,minimum height = .9cm, fill=lightgreen] (server-stack) [below = .3cm of new-server-api] {Server Stack};
...
\end{lstlisting}
  ...
  \end{subfigure}
  \caption{\bcp{The left-hand part  seems to be missing some stuff.}VSTS Architecture Using Lenses.
    ...
    which the lens then converts into the modern format for the new endpoints to
    process.
...
example inspired by the history of Microsoft's Visual Studio Team
...
released new web service endpoints~\cite{bharry}.  However,\bcp{careful of
  grammar here: Who is it (grammatically, not logically) that is doing the
  ``introducing''??} despite introducing 
new, modernized web APIs, the old, legacy web APIs must be maintained for continued
support of legacy
clients~\cite{team-explorer}.  Instead of maintaining server code for each endpoint,
our proposed architecture provides a lens that converts between resources of
the old form into resources of the new form, as shown in
Figure~\ref{subfig:vsts-arch}.  Because of the large number of existing service
endpoints, writing these converters will be a pain, so we would like to be able
to synthesize them.\bcp{Will our system really make it all that much better?
 We'll still have to collect types and examples by hand for the same large
 number of existing endpoints...}
...
old APIs are given in Figure~\ref{subfig:idealized-representations}.  If an old
client performs a GET request to receive a work item, it first
retrieves\bcp{this is what it does now, or what it will do once people are
  using our system?  (I know the answer logically; the issue is that the
  logical and grammatical answers are not the same.)} that work item
using the
modern API, and then uses the
lens's \PutLeft{} function to convert this task into the legacy format.  Similarly,
if an old client performs a PUT request\bcp{on?} a work item, it first uses the lens's
...
differently.  \bcp{Reader: ``Are these GET and PUT requests the same as the {\em get}
  and {\em put} parts of a lens?''}

For simplicity, let's consider only finding the mapping between the ``Title''
field of the task\bcp{``between'' takes two arguments}.
The legacy client accepts inputs of the form
%
\begin{lstlisting}
   !legacy_title! = @"<Field Id=2>"@ !text_char!* @"</Field>"@
\end{lstlisting}
\bcp{what is text-char?}
while the modern client accepts inputs of the form\bcp{Put the empty case
  first---I missed it on first reading and was confused.}
\begin{lstlisting}
   !modern_title! = (@"Title="@ !text_char! !text_char!* @","@) | @""@
\end{lstlisting}
%
We would like to be able to synthesize a lens $\Lens$, which\bcp{please look
up rules for commas, dependent and independent clauses, and which/that}  satisfies the
typing judgement $\Lens \OfType \LegacyTitle \Leftrightarrow \ModernTitle$
(i.e.\ the lens maps between the legacy representation \LegacyTitle{}, and the modern
representation \ModernTitle{}).
Because the modern API omits the title field if it is blank, the lens must
perform different 
actions depending on the number of text characters present.  An \OrLens{} lens
performs different actions depending on the structure of the input.  The typing
derivation for an \OrLens{} lens is:
\[
  \inferrule
  {
...
An \OrLens{} lens applies one of two lenses, depending on which of the
regular expressions matches the input string.
The hypothesis that $\UnambigOrOf{\Regex_1}{\Regex_2}$ asserts that $\Regex_1$
and $\Regex_2$ are \emph{disjoint}.  These constraints guarantee that the
\emph{get} and \emph{put} operations of $\OrLensOf{\Lens_1}{\Lens_2}$ form a
bijective pair if $\Lens_1$ and $\Lens_2$ are themselves bijective.

However, the rule above does not suffice to type check lenses that maps data
between \LegacyTitle{} and \ModernTitle{}.  While
\CF{\color{darkbrown} modern\_title} is a regular expression with an
outermost or operation, \CF{\color{darkbrown} legacy\_title}
is a regular expression with an outermost concatenation, so the rule above
cannot be applied directly.  To handle this problem, the Boomerang type checker
will replace one regular expression with another equivalent to it.\bcp{This
  is not accurate.  The boomerang typechecker always deals with regular {\em
    languages}.}  
This replacement is done by a type-conversion rule:
\[
  \inferrule
  {
...
This rule we can convert \CF{\color{darkbrown} legacy\_title} into
%
\begin{lstlisting}
   !legacy_title'! = @"<Field Id=2></Field>"@ | (@"<Field Id=2>"@ !text_char! !text_char!* @"</Field>"@)
\end{lstlisting}
%
...
$\Regex_1$ and $\Regex_3$, we must find an intermediate type $\Regex_2$
and lenses with types $\Regex_1 \Leftrightarrow \Regex_2$ and
...
given ones but with ``similar shapes,'' 
(2) a lens \emph{expression} that has the given type and is consistent
with the user's examples, and 
(3) an {\em intermediate} regular expression for each use of the composition
operator.  \bcp{2 and 3 are a bit confusingly tangled...}
...
$\DNFOf{\ldots}$ represents $n$-ary concatenation.\bcp{This example doesn't
  show distribution!}
...
synthesis algorithm no longer needs to search through possible intermediate
regular expressions for compositions.

\bcp{this paragraph might flow better if the part in parens came first.}
...
structural search---i.e., DNF regular expressions are only
\emph{pseudo-canonical}.  For instance, regular expressions such
as $B*$ and $\epsilon \Or BB*$ do not have the same normal form.  (Nor do we
want them to: If we normalized $\epsilon \Or BB*$ to $B*$, we would have
trouble with synthesizing lenses with types like $\EmptyString \Or BB*
...
Consequently\bcp{{\em Now} the two phases make sense!}, we structure our
synthesis algorithm in two phases. 
In the first phase, \SynthLens{} searches through equivalent DNF regular
expressions.  By using the axioms of a star semiring to unfold Kleene star
operators in one or both types, we obtain equivalent (but larger) DNF regular
expression types. In the second phase, \SynthLens{} perform a syntax-directed
search based on the 
structure of the provided DNF regular expression.
If the second phase fails, we return to the first phase, to find different
representations of the regular expressions which hopefully will succeed in the
second phase.\bcp{...except that the word ``phase'' is confusing: generally,
  when something is structured into phases, that means that the first phase
  is completely finished before the second begins.}

\bcp{Transition missing.}

\paragraph*{Challenge 2: Large Types}
In practical examples, the regular expressions describing complex {\em ad hoc} data
formats may be very large.  
The key to addressing this challenge\bcp{why is it a challenge?} is to observe that users naturally
construct large types incrementally, introducing named abbreviations for
major subcomponents.  For example, in the specification of \LegacyTitle{} and
\ModernTitle{}, we use the variable \TextChar{} in describing which values the
title can take.
The data type \TextChar{} stands for
one of many possible characters.  To include this large regular expression
everywhere it was used would be unmaintainable and difficult to
read.

We can exploit the structure inherent in these named
abbreviations to speed up the search dramatically.  For example, if
\TextChar{}
 appears just once in both the source and the target types, the system
...
\TextChar{} must be replaced with its
...
search by deducing when certain expansions must be taken, or when one of a class
of expansions must be taken.

\bcp{This would be a good place to review the structure of the next few
...
$\Sigma$ is the alphabet of individual characters $c$; strings $s$ and $t$ are
are elements of $\StarOf{\Sigma}$.  Regular expressions, abbreviated REs, are
used to express \emph{languages}, subsets of $\StarOf{\Sigma}$.
REs over $\Sigma$ are: \bcp{set on one line}
\begin{center}
\begin{tabular}{l@{\hspace*{5mm}}l@{\ }c@{\ }l}
& \Regex{},\RegexAlt{} & \GEq{} & \hspace{1.06em}c \\
          & & & \GBar{} $\emptyset$ \\
          & & & \GBar{} \Regex{}* \\
          & & & \GBar{} $\RegexConcat{\Regex_1}{\Regex_2}$ \\
          & & & \GBar{} $\RegexOr{\Regex_1}{\Regex_2}$ \\
\end{tabular}
\end{center}
...
rules for equivalences involving $\Star{}$:\bcp{Set the last one on a single
line}
\begin{center}
...
% Can't na\"{i}vely add both directions.
  ...
  \begin{tabular}{l@{\ }l@{\ }l@{\ }l}
    % REGEX
     \Lens{} & \GEq{} & & $\ConstLensOf{s_1 \in \StarOf{\Alphabet}}{s_2 \in \StarOf{\Alphabet}}$ \\
            ...
            & & \GBar{} & $\ComposeLensOf{\Lens_2}{\Lens_1}$ \\
...
\fi
  ...
  \caption{Lens Typing Rules \iffull\else (except $\mathit{or}$ and $;$) \fi}
...
as we next explain.  We have not found this restriction to limit
...
The inputs to the algorithm are the source and target
regular expression types and some examples. The search proceeds
bottom up.  
...
equivalences: any equivalences that don't satisfy the properties
outlined below could produce a language that is no longer closed under
composition, requiring a more complex search procedure. 

To be compatible with our lens synthesis strategy, we require that the
equivalence relation used in subsumption
is the equivalence closure of rewrite rules satisfying two properties.
These properties involve the ability to be well typed with no use of the the
retyping rule (which we will write as $\Lens \OfRewritelessType \Regex
\Leftrightarrow \RegexAlt$).
We call the first property \textit{confluence}.  It says:
Whenever $\Lens_1 \OfRewritelessType \Regex_1 \Leftrightarrow \RegexAlt_1$,
if $\Regex_1 \Rewrite
\Regex_2$ and $\RegexAlt_1 \Rewrite \RegexAlt_2$, there exist regular
expressions $\Regex_3$, and
$\RegexAlt_3$ and a lens $\Lens_3$, such that:
\begin{enumerate}
\item $\Regex_2 \Rewrite \Regex_3$
\item $\RegexAlt_2 \Rewrite \RegexAlt_3$
\item $\Lens_3 \OfRewritelessType \Regex_3 \Leftrightarrow \RegexAlt_3$
\item $\SemanticsOf{\Lens_3} = \SemanticsOf{\Lens_1}$
\end{enumerate}
We call the second property \textit{bisimilarity}.  Bisimilarity requires two
symmetric conditions.
Whenever $\Lens_1 \OfRewritelessType \Regex_1 \Leftrightarrow \RegexAlt_1$ and 
$\Regex_1 \to \Regex_2$, there exist a regular expression $\RegexAlt_2$ and a
lens $\Lens_2$ such that
\begin{enumerate}
\item $\RegexAlt_1 \Rewrite \RegexAlt_2$
\item $\Lens_2 \OfRewritelessType \Regex_2 \Leftrightarrow \RegexAlt_2$
\item $\SemanticsOf{\Lens_2} = \SemanticsOf{\Lens_1}$
\end{enumerate}
To be bisimilar, the symmetric property must also hold for $\RegexAlt_1 \Rewrite \RegexAlt_2$.
...
Our solution for handling subsumption is to use $\SSREquiv$, the equivalence relation
generated by the axioms of a \textit{star semiring}~\cite{Droste2009}.
These axioms are
the semiring axioms, plus two ``unrolling'' rules that
replace the Conway axioms of \SumstarRule{}, \ProductstarRule{}, \StarstarRule{}, and
\DicyclicityRule{}:
\begin{center}
...
orienting these unrolling rules from left to right presents us with a rewrite relation
that is both confluent and bisimilar, and whose equivalence closure is
$\SSREquiv$.
The star semiring axioms are the 
coarsest subset of regular expression equivalences we could
find that is generated by a rewrite relation that are still confluent and
bisimilar.  We have not been able to prove that it is the coarsest
such relation possible, but it is sufficient to cover all of the test
...
\begin{figure}
  ...
  [\String_0\SequenceSep\Atom_1\SequenceSep\ldots\SequenceSep\Atom_n\SequenceSep\String_n\Concat\StringAlt_0\SequenceSep\AtomAlt_1\SequenceSep\ldots\SequenceSep\AtomAlt_m\SequenceSep\StringAlt_m]$\\
...
\begin{center}
  \begin{tabular}{@{}r@{\ }c@{}l@{}}
    ...
    $\SemanticsOf{(\SequenceLensOf{(\String_0,\StringAlt_0)\SequenceLensSep
      ...
      \SequenceLensSep(\String_n,\StringAlt_n)},\Permutation)}$ & \GEq{} & $\SetOf{
    (\String_0\String_1'\ldots\String_n'\String_n,
...
An overview of the full synthesis algorithm, \SynthLens{}, is given in
Algorithm~\ref{alg:synthlens}.  \SynthLens{} takes the source and target regular
expressions $\Regex$ and $\RegexAlt$, and a list of examples $\Examples$, as
input. The algorithm first converts
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \While{$\True$}
    \State $(\QueueElement,\Queue) \gets \Call{\Pop}{\Queue}$ \Comment{\TypeProp{}}
    ...
    \EndFunction
  \end{algorithmic}
\bcp{Why is TypeProp over on the right?  Also, I don't like the naming
  much---DR0 is an ugly name}
\end{algorithm}
...
$\DNFRegexAlt_0$. The priority of each queue element is $\ExpCount$; DNF regular
expressions which have undergone fewer expansions will get popped earlier.
...
highest priority element from the queue (\TypeProp{} in
Figure~\ref{fig:schematic-diagram}), using the
...
\paragraph*{\Expand}
\begin{algorithm}
  \caption{\Expand{} \bcp{Two columns?}}
  \label{alg:synthlens}
  \begin{algorithmic}[1]
...
The inference of expansions is much of what allows our synthesis algorithm to be
efficient.  \Expand{} has three parts: \ExpandRequired{}, \FixProblemElts{}, and
...
if a user defined data type $\UserDef$, at star depth $\Int$ is impossible to be
reached through any number of expansions on the opposite side, then that user
defined data type \emph{must} be replaced by its definition at that depth.
This is apparent
in trying to find a lens between \VarCF{legacy\_title} and
\VarCF{modern\_title}.  No matter how many expansions are taken on
\VarCF{modern\_title}, \VarCF{legacy\_title} will not be exposed at depth 0 (or
...
$ $(\TextChar,0),(\TextChar,1)\}$.  Because no number of expansions can reach
$\VarCF{\LegacyTitle}$, it must be expanded for a lens to be found.
\ExpandRequired{} continues until no more expansions can be forced.
...
between them, for each $(\UserDef,\Int)$ pair, on one regular expression, that
same pair must be present on the other regular expression.  \FixProblemElts{}
...
\Reveal{} to find the expansions which will reveal this element.  For example,
after \LegacyTitle{} and \ModernTitle{} have been expanded into
\CF{\textcolor{blue}{"<Field Id=2>"}
  \textcolor{darkbrown}{text\_char}* \textcolor{blue}{"</Field>"}} and
\CF{(\textcolor{blue}{"Title="}
  \textcolor{darkbrown}{text\_char} \textcolor{darkbrown}{text\_char}*
  \textcolor{blue}{","}) | \textcolor{blue}{""}}, we can see that the modern expansion
has an instance of \TextChar{} at depth 0, where the legacy one does not.  This
means that \TextChar{} must be revealed at star depth 0 in the legacy expansion.
Revealing this will give back 2 DNF regular expressions,
one from an application of
\AtomUnrollstarLeftRule{}, and one from an application of
\AtomUnrollstarRightRule{}.

While \FixProblemElts{} and \ExpandRequired{} are able to infer a huge number
of expansions, there are still situations where they can't infer all necessary
expansions. This typically appears in data formats with large amounts
of similar information.  For example, in trying to synthesize the identity
...
While another expansion must be applied, but the set of pairs
($\SetOf{(\VarCF{U},0),(\VarCF{U},1)}$) is the same.  When this situation
arises, \ExpandOnce{} performs a random search by performing all single-step
expansions.

\paragraph*{\RigidSynth} 
This function implements the portion of our synthesis algorithm that generates
the lens from the types and examples.
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \Function{\RigidSynthAtom}{$\StarOf{\DNFRegex},\StarOf{\DNFRegexAlt},\Examples$}
    ...
    \State $\SequenceOf{\String_0;\Atom_1;\ldots;\Atom_n;\String_n} \gets
    \Sequence$
    \State $\SequenceOf{\StringAlt_0;\AtomAlt_1;\ldots;\AtomAlt_m;\StringAlt_m} \gets
    \Sequence$
    \If {$n \neq m$}
    \State $\ReturnVal{\None}$
    \EndIf
    \State $\sigma_1 \gets \SortingOf{\AtomLeq^\Examples}{\ListOf{\Atom_1;\ldots;\Atom_n}}$
    \State $\sigma_2 \gets \SortingOf{\AtomLeq^\Examples}{\ListOf{\AtomAlt_1;\ldots;\AtomAlt_n}}$
    \State $\sigma \gets \InverseOf{\sigma_2} \circ \sigma_1$
    \State $\mathit{ABs} \gets
    \Call{Zip}{\ListOf{\Atom_1;\ldots;\Atom_n},\ListOf{\AtomAlt_{\sigma(1)};\ldots;\AtomAlt_{\sigma(n)}}}$
    \State $\mathit{alos} \gets
    \Call{\Map}{\RigidSynthAtom(\Examples),\mathit{ABs}}$
    \Switch{$\Call{AllSome}{\mathit{alos}}$}
    \CaseTwo {\SomeOf{\ListOf{\AtomLens_1;\ldots;\AtomLens_n}}}{$\ReturnVal{\SomeOf{(\SequenceLensOf{(\String_0,\StringAlt_0);\AtomLens_1;\ldots;\AtomLens_n;(\String_n,\StringAlt_n)},\sigma)}}$}
    \EndCaseTwo
    \CaseTwo {\None}{$\ReturnVal{\None}$}
    \EndCaseTwo
    \EndSwitch
    ...
    \State $\DNFOf{\Sequence_1;\ldots;\Sequence_n} \gets
    \DNFRegex$
    \State $\DNFOf{\SequenceAlt_1;\ldots;\SequenceAlt_m} \gets
    \DNFRegexAlt$
    \If {$n \neq m$}
    \State $\ReturnVal{\None}$
    \EndIf
    \State $\sigma_1 \gets \SortingOf{\SequenceLeq^\Examples}{\ListOf{\Sequence_1;\ldots;\Sequence_n}}$
    \State $\sigma_2 \gets \SortingOf{\SequenceLeq^\Examples}{\ListOf{\SequenceAlt_1;\ldots;\SequenceAlt_n}}$
    \State $\sigma \gets \InverseOf{\sigma_2} \circ \sigma_1$
    \State $\mathit{STQs} \gets
    \Call{Zip}{\ListOf{\Sequence_1;\ldots;\Sequence_n},\ListOf{\SequenceAlt_{\sigma(1)};\ldots;\SequenceAlt_{\sigma(n)}}}$
    \State $\mathit{sqlos} \gets
    \Call{\Map}{\RigidSynthSequence(\Examples),\mathit{STQs}}$
    \Switch{$\Call{AllSome}{\mathit{sqlos}}$}
    \CaseTwo {\SomeOf{\ListOf{\SequenceLens_1;\ldots;\SequenceLens_n}}}{$\ReturnVal{\SomeOf{(\DNFLensOf{\SequenceLens_1;\ldots;\SequenceLens_n},\sigma)}}$}
    \EndCaseTwo
    \CaseTwo {\None}{$\ReturnVal{\None}$}
    \EndCaseTwo
    \EndSwitch
...
permutations required to sort the sequences, we can discover the permutation
used in the lens.

Through incorporating information about how examples are parsed in the
orderings,
we can guarantee not only that there will be a lens between the regular
...
As an example, consider trying to find a lens between
\begin{lstlisting}
 @"<Field Id=2></Field>"@ | @"<Field Id=2>"@ !text_char! !text_char!* @"</Field>"@
\end{lstlisting}
and
\begin{lstlisting}
 @"Title="@ !text_char! !text_char!* @","@ | @""@
\end{lstlisting}
As it passes to \RigidSynth{}, the two sequences in the legacy format get ordered
by maintaining the current order:  first \CF{\StringCF{"<Field Id=2></Field>"}}, then
\CF{\StringCF{"<Field Id=2>"} \VarCF{text\_char} \VarCF{text\_char}*
  \StringCF{"</Field>"}}.  However, the two sequences of the modern version get
reordered, making \CF{\StringCF{""}} first, and \CF{\StringCF{"Title="}
  \VarCF{text\_char} \VarCF{text\_char}* \StringCF{","}} second.
the overall permutation of a swap.  This causes the two sequences which consist
only of a string to get aligned, and the complex sequences to also get aligned.

Then, \RigidSynthSequence{} gets called on the two aligned sequence pairs.
There are no atoms in both
\StringCF{"<Field Id=2></Field>"} and \StringCF{""} which trivially creates the
sequence lens:\\
\\(\SequenceLensOf{(\StringCF{"<Field
    Id=2></Field>"},\StringCF{""})},\Identity)\\
\\
\afm{should I do a match for the rigidsynthatom for variables vs iterations}
Next
\CF{\StringCF{"<Field Id=2>"} \VarCF{text\_char} \VarCF{text\_char}*
  \StringCF{"</Field>"}} and \CF{\StringCF{"Title="}
  \VarCF{text\_char}} $ $ \CF{\VarCF{text\_char}* \StringCF{","}} would be sent
to \RigidSynthSequence{}.
In this, the atoms would not be reordered, aligning \VarCF{text\_char} with
\VarCF{text\_char}, and $\StarOf{\VarCF{text\_char}}$ with
$\StarOf{\VarCF{text\_char}}$.  Immediately, \RigidSynthAtom{} finds the
identity transformation on \VarCF{text\_char}, and will recurse to find
\IterateLensOf{\IdentityLensOf{\TextChar}} for $\StarOf{\VarCF{text\_char}}$.
...
\begin{restatable}[Algorithm Soundness]{theorem}{algs}
...
\begin{restatable}[Algorithm Completeness]{theorem}{algc}
  \label{thm:alg-completeness}
  Given a lens $\Lens$, regular expressions $\Regex$ and $\RegexAlt$, and a set
  of examples $\Examples$, if
  $\Lens \OfType \Regex \Leftrightarrow \RegexAlt$ and for all
  $(\String,\StringAlt)$ in $\Examples$, $(\String,\StringAlt) \in
...
\section{Evaluation}
\label{evaluation}
We evaluate our synthesis algorithm by applying it to 26
benchmark programs.
...
from Augeas with the letters `B' and `C' were tested after our synthesis
algorithm was finalized, demonstrating that our optimizations were not
...
\afm{211} AST nodes, for complex tasks like transforming XML to a dictionary
representation. 
...
\begin{figure}
  ...
  \caption{
    ...
    amount of time. {\bf Full} is the full synthesis algorithm.  {\bf DNFFI} is
    the synthesis algorithm using all optimizations but the use of the lens
    context.  {\bf DNFPI} is the core DNF synthesis algorithm with forced
    expansions performed.  {\bf DNF} is the core synthesis algorithm.  {\bf
      Flash Fill} is the existing Flash Fill system.  {\bf FlashExtract} is the
    existing FlashExtract system.  {\bf Na\"{i}ve} is na\"{i}ve type-directed
...
synthesis algorithm dramatically.  Indeed, a number of benchmark lenses cannot
be synthesized in any reasonable amount of time without these optimizations.
...
be synthesized within the 2 minute timeframe.  Our DNF synthesis procedure
(with \emph{no} intelligent search through expansions) succeeds on
\afm{12} of the benchmarks.

\begin{figure}
  \includegraphics{UninferredExpansions}
  \caption{Number of expansions inferred for tasks requiring a given number of
    ...
    requiring any random search.}
  \label{fig:uninferred-exps}
\end{figure}

Conversion to DNF form helped significantly, but still left many problems
unsolved.  In particular, with no inference of expansions, only the lenses
that required few expansions were solvable.  Because of the large fan-out
in the search space for expansions, of the \afm{12} could be solved simply
with DNF conversion and \RigidSynth{}, none required more than \afm{5}
expansions to be performed.  As up to \afm{30} expansions are needed for
synthesizing some of the lenses, the large fanout an undirected traversal of
expansions insufficient for many examples.
However, we can identify that many of these expansions must be performed, so
the random search is often unnecessary.
Figure~\ref{fig:uninferred-exps} shows that in a majority of examples, all the
expansions can be identified as required, making the large fan-out have less of
an impact.  While it is unable to infer all the examples, it still is able to
infer quite a large amount: a maximum of 26 expansions, and a median of 11
expansions.
...
\emph{all} the expansions.  In \afm{15} of the \afm{25} examples solvable with
expansion inference, all expansions
were forced.  However, the remaining \afm{11} still require random search.
...
Our full synthesis algorithm makes use of lenses in the context.  Through using
these lenses, the synthesis algorithm is not solely
...
The benchmark which our algorithm requires contextual information for is also
the slowest benchmark in the full synthesis algorithm, \CF{aug/xml}.  This
...
repetitive nature of the specification for \CF{aug/xml}.  XML tags occur at many
...
of required expansions.  The large fanout of expansions present in a large data
format, combined with the number of expansions which must be performed, creates
...
any examples: only 3 benchmarks need a nonzero number of examples tosynthesize
...
which were mapped, in the generated lens, either through an identity
transformation, or through a lens in the context, whereas strings which were
...
regular expressions,
\SynthLens{} is able to synthesize significantly more programs
than either tool.  Unlike these two tools, which must spend a great deal of
effort figuring out the appearance of the data, SynthLens{} avoids
...
synthesizing unidirectional string transformations~\cite{singh2012learning,le-pldi-2014,gulwani-popl-2014,perelman2014test,Singh:blinkfill}.  Much of this
...
(usually to zero), makes the system scale well, and
allows it to handle large, complex formats, as shown in \S \ref{evaluation}.
Furthermore, because our language is one of \emph{bidirectional}
transformations, we are not merely transforming or extracting data, but writing
two-way convertersions between the formats.  Of course, because we are writing
bidirectional languages, we are less expressive in some of the functions
we can express, as we require the synthesized functions to be bijective.

There are many other recent results showing how to synthesize functions from
type-based specifications~\cite{augustsson-2004,gvero-pldi-2013,tds-pldi,feser-pldi-2015,scherer-icfp-2015,frankle+:popl16,armando+:pldi16}.  While we were inspired by this work,
...
similarity of approach, there is not actually much technical similarity
with past type-directed synthesis algorithms.

\bcp{Just going by length, the related work section seems a bit skimpy...} 
