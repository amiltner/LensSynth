...
In particular, it is sufficient for the equivalences used in type equivalence
...
of DNF lenses, when those typing derivations do not use type equivalence
(if $\DNFLens$ is a DNF lens that
goes between DNF regular expressions $\DNFRegex$ and $\DNFRegexAlt$, and the
typing derivation contains no instances of the type equivalence rule, then
...
currying''~\cite{gvero-pldi-2013} identified.
...
permit us to identify all equivalent terms. Instead \TypeProp{} must traverse
...
Our systems differ in that an outline for \Morpheus{} is a sketch (an expression
containing holes) where
an the outline for \Optician{} is a pair of DNF regular expressions, a type in
our language.
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \Function{\RigidSynthAtom}{$\StarOf{\ExampledDNFRegex},\StarOf{\ExampledDNFRegexAlt{}}$}
    \Switch{$\RigidSynth(\ExampledDNFRegex,\ExampledDNFRegexAlt)$}
    \CaseTwo{\SomeOf{\DNFLens}}{\ReturnVal{\IterateLensOf{\DNFLens}}}
    \EndCaseTwo
    \CaseTwo {\None}{$\ReturnVal{\None}$}
    \EndCaseTwo
    ...
    \State $\SequenceOf{\StringAlt_0 \SeqSep \ExampledAtomAlt_1 \SeqSep \ldots \SeqSep
      \ExampledAtomAlt_m \SeqSep \StringAlt_m} \gets
    \ExampledSequenceAlt$
    \If {$n \neq m$}
    \State $\ReturnVal{\None}$
    \EndIf
    ...
    \State $\DNFOf{\ExampledSequence_1 \DNFSep \ldots \DNFSep \ExampledSequence_n} \gets
    \ExampledDNFRegex$
    \State $\DNFOf{\ExampledSequenceAlt_1 \DNFSep \ldots \DNFSep \ExampledSequenceAlt_m} \gets
    \ExampledDNFRegexAlt$
    \If {$n \neq m$}
    \State $\ReturnVal{\None}$
    \EndIf
...
\begin{lemma}\leavevmode
  \begin{itemize}
  \item
    Let $\Atom$ and $\AtomAlt$ be DNF regular expressions.
    ...
    Let $\StringIntListSet_1 \in \Atom \Generates \ExampledDNFRegex$.
    Let $\StringIntListSet_2 \in \AtomAlt \Generates \ExampledDNFRegexAlt$.
  ...
  \item
    Let $\Sequence$ and $\SequenceAlt$ be DNF regular expressions.
    Let $\StringIntListSet_1$ be a string int list set.
    Let $\StringIntListSet_2$ be a string int list set.
    Let $\ProjectILSOf{\StringIntListSet_1} =
    \ProjectILSOf{\StringIntListSet_2}$.
    Let $\ExampledSequence$ and $\ExampledSequenceAlt$ be exampled sequences.
    Let $\StringIntListSet_1 \in \Sequence \Generates \ExampledDNFRegex$.
    Let $\StringIntListSet_2 \in \SequenceAlt \Generates \ExampledDNFRegexAlt$.
  ...
  \item
    Let $\DNFRegex$ and $\DNFRegexAlt$ be DNF regular expressions.
    Let $\StringIntListSet_1$ be a string int list set.
    Let $\StringIntListSet_2$ be a string int list set.
    Let $\ProjectILSOf{\StringIntListSet_1} =
    \ProjectILSOf{\StringIntListSet_2}$.
    ...
    expressions.
    Let $\StringIntListSet_1 \in \DNFRegex \Generates \ExampledDNFRegex$.
    Let $\StringIntListSet_2 \in \DNFRegexAlt \Generates \ExampledDNFRegexAlt$.
...
\begin{lemma}\leavevmode
  ...
  \item
    ...
    $\RigidSynthSequence$ returns a lens.
  \item
    ...
    $\RigidSynthInternal$ returns a lens.
  \end{itemize}
\end{lemma}
