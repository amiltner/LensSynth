...
requires learning a new language and paradigm.
...
We evaluate our algorithm on a benchmark suite of 26 examples including
...
increasingly important.  In what is known as the classic
``view update problem''~\cite{Bancilhon81, Dayal82},
programmers want to be able to transform source data into a target
format (or view) and transform the data from the view back into the source
format.
Solutions to this problem that provide strong guarantees about the
round-trip behavior of the two components are formalized as
\emph{lenses}~\cite{Focal2005-long}.
Lenses help guard against data corruption while reading,
editing, and writing data sources.  A lens comprises two functions, called 
\emph{get} and \emph{put}.  The \emph{get} function translates between source data
into the view.  If the view is updated, the
\emph{put} function can translate the edited data back into the
source format.  In lens-based languages, the \emph{get} and
...
Such lens-based languages have been applied in a variety of practical settings.
...
Augeas~\cite{augeas} is a popular tool that reads Linux system configuration
...
using operators that swap the order of adjacent fields.  The
challenge of learning these languages and using them effectively has slowed
their adoption in many settings where well-behaved bidirectional transformations
are needed.

To make programming in lens-based languages faster and easier, we have
developed a
...
%
This work continues a recent trend toward making programming easier
through synthesizing programs in a variety of domain-specific
...
than the unfamiliar syntax of lenses.  Including these format descriptions communicates a
...
Despite the benefits of such informative types, certain features of
Boomerang make it ill
suited to synthesis in their raw form.  Boomerang's types are regular expression
pairs, and there are infinite equivalent regular expressions.
...
To resolve these issues, we introduce new languages of ``DNF (Disjunctive Normal
Form) Lenses'' and ``DNF
...
Furthermore, DNF lenses have a \emph{two-phased} typing derivation which fixes
where and how equivalences on DNF regular expressions can be processed.
The first phase of the typing derivation handles equivalences on the types, and
the second phase of the typing derivation aligns the terms with the types.
...
The typing rules of DNF lenses lend themselves to a two-phased synthesis
algorithm.  A schematic diagram of this two phased algorithm is given in
...
By searching through the first phase of the typing derivation, \TypeProp{}
...
Two-phased synthesis algorithms are present in many other
systems~\cite{?,?,?}, but \SynthLens{} is the first that ties this two-phased synthesis
algorithm to a two-phased typing derivation.
...
all benchmarks in under 5 seconds.
\end{itemize}
...
example inspired by Visual Studio Team Services (VSTS).  VSTS is
...
In 2014, to increase third party developer interaction, Visual Studio Online
released new web service endpoints~\cite{bharry}.  However, despite introducing
...
client performs a GET request to receive a work item, it first retrieves that work item
using the
modern API, and then uses the
lens's \PutLeft{} function to convert this task into the legacy format.  Similarly,
if an old client performs a PUT request a work item, it first uses the lens's
...
representations contain the same information, but they are presented differently.

For simplicity, let's consider only finding the mapping between the ``Title''
field of the task.
The legacy client allows for inputs of the form
\begin{lstlisting}
   !legacy_title! = @"<Field Id=2>"@ !text_char!* @"</Field>"@
\end{lstlisting}
while the modern client allows for inputs of the form
\begin{lstlisting}
   !modern_title! = (@"Title="@ !text_char! !text_char!* @","@) | @""@
\end{lstlisting}
We would like to be able to synthesize a lens that maps between the legacy
representation and the modern representation.
Because the modern api omits the title field if it's blank, we must do different
actions depending on the number of text characters present.  We can perform
different actions depending on the structure of the input through the use of an
\OrLens{} lens.  The typing derivation for an \OrLens{} lens is:
\[
  \inferrule
  {
...
However, we cannot immediately make an \OrLens{} lens using our types.  While
\CF{\color{darkbrown} modern\_title} is a regular expression with an
outermost or operation, \CF{\color{darkbrown} legacy\_title}
is a regular expression with an outermost concatenation, so an
\OrLens{} lens would not be well typed.  To handle this problem, Boomerang
allows for replacing regular expressions with regular expressions which are
equivalent to them.  This is done by a type-conversion rule:
\[
  \inferrule
  {
...
With this rule, we can convert \CF{\color{darkbrown} legacy\_title} into
%
\begin{lstlisting}
   !legacy_title'! = @"<Field Id=2></Field>"@ | (@"<Field Id=2>"@ !text_char! !text_char!* @"</Field>"@)
\end{lstlisting}
%
This allows us to now make a well typed \OrLens{} lens, and the two cases of an empty and a
nonempty number of text characters can be handled separately.  However, the need
to find this equivalent type highlights a signficant challenge in synthesizing
boomerang lenses.
...
consider all of them?  In what order?  To convert from
\CF{\color{darkbrown} legacy\_title} to \CF{\color{darkbrown} legacy\_title'},
we must first unroll \CF{{\color{darkbrown} text\_char}*} into
\CF{{\color{blue} ""} | {\color{darkbrown} text\_char} {\color{darkbrown}
    text\_char}*}, and then we must
distribute this \OrRegexType{} on the left and the right.
...
Consequently, we structure our synthesis algorithm in two phases.
In the first phase, \SynthLens{} searches through equivalent DNF regular
expressions.  By using the axioms of a star semiring to unfold * operators in
one or both types, we obtain equivalent (but larger) DNF regular expression types.
In the second phase, \SynthLens{} perform a syntax-directed search based on the
...
major subcomponents.  This is highlighted by our use of \TextChar{} in our
description of the title field.  The data type \TextChar{} represents stands for
one of many possible characters.  To include this large regular expression
everywhere it was used would be very unmanageable.
...
definition.  In this way, where names occur
...
way that allows for unambiguous parsing. $\Regex$ and $\RegexAlt$ are
...
any other depth).  This is because the set of possible reachable pairs of
...
Through incorporating how examples are parsed in the orderings, we
can guarantee not only that there will be a lens between the two, but also that
the lens will satisfy the examples!  This is done by refining our orderings with
the elements, such that $\Sequence \SequenceLeq^{\Examples} \SequenceAlt$ and
$\SequenceAlt \SequenceLeq^{\Examples} \Sequence$ not only means that there is a
sequence lens between $\Sequence$ and $\SequenceAlt$, but also that such a lens
would satisfy the examples!
...
benchmark programs derived from practice.  We describe the benchmark suite
construction to demonstrate that our benchmarks are realistic and interesting.
We investigate the imapct of our optimizations to highlight how we were able to
obtain our final performance.
We discuss the importance of examples to show that our approach increases
productivity, instead of merely changing time spent programming into time spent
generating appropriate examples.
Lastly, to demonstrate the novelty and strength of our approach, we provide a
comparison against other, related tools.
...
algorithm. Furthermore, the synthesis problems derived
...
We generate lenses of sizes between 5 AST
nodes, for simple problems like changing how dates are represented, and 211 AST
nodes, for complex tasks like transforming XML to a dictionary representation.
...
%revise the regular expressions to have a bijective lens between them.
%
As we iterated on our design, we added in a variety of optimizations to make all
the benchmarks tractable.  To demonstrate how critical these optimizations
...
demonstrating that as we add in additional optimizations, more benchmarks become
tractable, and previously tractable benchmarks are processed faster.

Conversion to DNF form, and the use of \RigidSynth{} was critical to the
tractability of our algorithm; using
...
be synthesized within the 2 minute timeframe.  Converting into the DNF
language and performing our synthesis procedure (with \emph{no} intelligent
intelligent search through expansions)
immediately allows for 12 benchmarks to be synthesized.
...
intractible.  In particular, with no inference of expansions, only the lenses
which required few expansions were tractible.  Because of the large fan-out
...
tractable.  This is because inferring forced expansions, in many cases, infers
...
This allows most of the examples to be tractable, but rather slow.
This is able to be sped up by directing the search using more than just blind
breadth-first search, when more information can be used to direct it.  When
...
Our final optimization is the use of lenses in the context.  Through using
lenses present in the context, the synthesis algorithm is not solely
...
\afm{should this potentially go elsewhere?}
In our full synthesis algorithm, two examples take longer than the others to
infer:  \CF{aug/xml} and \CF{cust/2-cap-prob}. The example \CF{aug/xml} was
slowest because it was both a large data format, a large number of expansions
were required, and not all expansions were inferred.  While a fairly large
number of expansions were forced (15), there were 3 expansions that were
inferred, but not forced, and 1 expansion that had to be found through random
search.  This is not always a problem, however, as there were other formats that
required even more search, both random search and directed search.  What
differentiated \CF{aug/xml} was that it also was a very large data format: it
had the \afm{second} largest lens and the \afm{seventh} largest specification.
The amount of random search it required had a larger impact because, with an
increased specification size, there is also an increase in the fanout, with
additional iterations that can be unfolded, and additional user defined data
types that can be replaced by their definitions. This large fanout, alongside
the depth that needed to be searched through, made the search very difficult,
requiring \afm{164} types to be proposed before the correct one was
found.\footnote{Since xml syntax is regular, the source and target regular
...
any examples: only 3 benchmarks need a nonzero number synthesize the correct
lens. A clever user may be able to reduce the
...
specifications through generating random elements of the source language, and
...
on the user.  These tools only require the use of IO examples (for Flash
Fill), or marked text regions (for FlashExtract), as opposed to requiring the
input of pairs of regular expressions.  However, by requiring the user to
...
have right distributivity.  This also allows for an analogue to the
commutativity of regular expression $\OrRegexType$ to be expressed.
