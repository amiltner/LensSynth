...
\saz{The flow and transition of this paragraph is not great.  Also the
  sentence structure uses the phrase ``back and forth'' too often.
  The point is to get quickly to a list of uses of bidirectional programming.}
...
Bidirectional programs perform both the back and the forth
...
\saz{Is the subject ``Boomerang's lens language'' languages like
  Boomerang but without the composition operator?  The point is that
  leaving out composition reduces expressiveness.}
...
\saz{Throughout: the word ``expressibility'' is the \textit{property}
  of being expressible, so it should be used as: The expressibility of
  the string ``foo'' by the language L,...  The words
  ``expressiveness'' or ``expressivity'' refer to how many things can
  be expressed, as in ``lense languages with composition are strictly
  more expressive (i.e. have more expressivity) than languages without.''}

\saz{This paragraph is roundabout: the structures of the DNF lenses
  permit the synthesis of more lenses than would a language without composition.}
...
\saz{This paragraph feels redundant, and this exposition is getting long.}
...
\saz{Introduce a new paragraph header about equivalence.}
...
search procedure for generating equivalent expressions.\saz{This is
  passive.  Why not just say that an equational theory provides a straightforward
search procedure for generating equivalent expressions?}

\saz{What is the flow here? The next few paragraphs seem to talk about
  picking out a particular set of equations, the ones in Figure 3,
  that we actually use for synthesis.  This is straying from
  ``background'' material into ``design.'' The reader doesn't
  understand how the equational theory fits into the synthesis process
yet, so this discussion is too soon.}
It is possible to look into finer \saz{coarser?} equivalence rules than this.
...
\saz{This is, again, design, not background.  What are the ``required properties''?}
...
of its type.  \saz{You should ``pronounce'' the semantics and rules so
  the reader can understand them.  What does it mean for the pair
  $(s,t)$ to be in the meaning of $l$?}
...
\saz{This needs a transition to remind the reader of what the running
  example is.}
...
\saz{This paragraph seems to be out of sync with the Figures.}
...
\saz{Somehow the priority queue comes out of nowhere.}
...
Because the regular expressions of  \CF{DNFBib'} and \CF{DNFEndUnroll} \saz{``are so
similar, type-directed synthesis derived from the typings in
Figure~\ref{fig:lens-typing} can now happen between them.'' $\to$
``type-directed synthesis applies.''}
...
\saz{This transition is old.  The need for DNF Regular expressions has
...
into a normal form called DNF regular expressions. \saz{What does
  ``DNF'' stand for?}
A DNF regular expression is, intuitively, a regular expression with
distributivity applied wherever possible, \saz{``causing'' $\to$
  ``yielding an''} outer layer of \Or{} operations,
with inner sequences of \Concat{} operations.
\saz{This paragraph needs serious word smithing.}
...
\saz{Can we explicitly write down the pseudo-code for the synthesis
  algorithm? Having the concrete steps of synthesis to refer to in the
  description in this section would help a lot.}
...
\saz{``To create a DNF regular expresion from a regular expression,
some functions need to be defined on DNF regular expressions,'' $\to$
...
\saz{This definition and the upcoming theorems need some explanatory
  setup. This discussion perhaps merits its own subsection.}
...
\saz{DNF Rewrites --- this needs a seque and more context, also
...
regular expressions.  \saz{``can be'' or ``are''?}
...
\begin{figure}
  \begin{mathpar}
    ...
    \inferrule[\AtomPowerstarRuleLeft{}]
    {
      n\in\Nats_{\geq1}
    }
    {
    ...
    \inferrule[\AtomPowerstarRuleRight{}]
    {
      n\in\Nats_{\geq1}
    }
    {
...
\begin{restatable}[Partial Completeness of DNF Lenses]{theorem}{dnflc}
  ...
  \RegexAlt$,
  where the proof of equivalence of regular expressions is done using the
...
\begin{definition}[Dictionary Ordering]
...
\afm{There's some overloading of syntax here, where [] is used for Sequences,
  but also it's used for lists.  \#1: should I explicitly describe that [] is
  used for the syntax of lists.  \#2: should one of them be changed for clarity?
  Maybe I can use $\{\}$ for DNFs, and $\langle \rangle$ for Sequences, and []
  for lists?}

\begin{definition}[Sorting of a List]
...
\begin{definition}[Orderings on DNF Regular Expressions]\leavevmode
  \begin{enumerate}
  \item
...
\begin{definition}[Normalization of DNF Regular Expressions]
  \leavevmode
  \begin{enumerate}
...
\afm{So this shows a nice theorem, but it doesn't actually show the construction
of the DNF lens from the normalized forms.  Should I actually show the
construction, or is that not worth going into / clear enough from the
description?}
...
\begin{proof}By mutual induction on the derivation of $\ParallelRewriteSwap$ and
  ...
  \item[\LeftQuotientRewriteRuleOf{n}]
    Let $\ToDNFRegexOf{\Regex} = \DNFOf{\SequenceOf{\Atom}}$, and
    $\Atom \ParallelRewriteSwapAtom \ToDNFRegexOf{\RegexAlt}$ from
    \LeftQuotientRewriteRuleOf{n}.
    This means that $\Atom = \StarOf{\DNFRegex}$ and
  ...
  \item[\RightQuotientRewriteRuleOf{n}]
    Let $\ToDNFRegexOf{\Regex} = \DNFOf{\SequenceOf{\Atom}}$, and
    $\Atom \ParallelRewriteSwapAtom \ToDNFRegexOf{\RegexAlt}$ from
    \RightQuotientRewriteRuleOf{n}.
    This means that $\Atom = \StarOf{\DNFRegex}$ and
    $\ToDNFRegexOf{\RegexAlt} =
    \ConcatDNFOf{\DNFOf{\SequenceOf{\StarOf{(\DNFRegex^n)}})}}
    {\DNFRegex^{<n}}$.
    ...
    Lemma~\ref{lem:preimage-equiv}, $\StarOf{\Regex'}
    \DefinitionalEquiv \Regex$.
    Similarly, as $\ToDNFRegexOf{(\StarOf{(\Regex'^n)}) \Concat
      \Regex'^{<n}} =
    \ConcatDNFOf{\DNFOf{\SequenceOf{\StarOf{(\DNFRegex^n)}})}}
    {\DNFRegex^{<n}}$,
    then from
    Lemma~\ref{lem:preimage-equiv}, $\StarOf{(\Regex'^n)} \Concat
      \Regex'^{<n} \DefinitionalEquiv \RegexAlt$.
    
    $\Regex \DefinitionalEquiv \StarOf{\Regex'} \DefinitionalEquiv
     \StarOf{(\Regex'^n)} \Concat \Regex'^{<n} \DefinitionalEquiv \RegexAlt$,
    as desired.
...
\begin{proof}
  ...
  \item[\EmptyProjectionRuleRightRule{}]
    Let $\Regex \DefinitionalEquiv \RegexAlt$, and the last step of the
  ...
  \item[\EmptyProjectionRuleLeftRule{}]
    Let $\Regex \DefinitionalEquiv \RegexAlt$, and the last step of the
    deduction is an application of \EmptyProjectionRuleRightRule{}.  Without
  ...
  \item[\DistributivityLeftRule{}]

  \item[\DistributivityRightRule{}]

  \item[\ConcatIdentityRule{}]

  \item[\UnrollstarLeftRule{}]

  \item[\UnrollstarRightRule{}]

  \item[\QuotientstarRule{}]
  \end{description}
\end{proof}
...
\begin{proof}
  ...
  \item[\LeftQuotientRewriteRuleOf{n}]
    Let $\AtomLens \OfType \Atom \Leftrightarrow \AtomAlt$, and
    $\Atom\ParallelRewriteAtom\DNFRegex$, and $\AtomAlt\ParallelRewriteAtom\DNFRegexAlt$
    through an application of \LeftQuotientRewriteRuleOf{n}.
  \item[\RightQuotientRewriteRuleOf{n}]
    Let $\AtomLens \OfType \Atom \Leftrightarrow \AtomAlt$, and
    $\Atom\ParallelRewriteAtom\DNFRegex$, and $\AtomAlt\ParallelRewriteAtom\DNFRegexAlt$
    through an application of \RightQuotientRewriteRuleOf{n}.
  \item[\StarIntroRewriteRule{}]
    Let $\AtomLens \OfType \Atom \Leftrightarrow \AtomAlt$, and
    $\Atom\ParallelRewriteAtom\DNFRegex$, and $\AtomAlt\ParallelRewriteAtom\DNFRegexAlt$
