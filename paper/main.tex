...
which require programmers to think in two directions at the same time---a tricky
task that is foreign to most programmers.  In addition, the details are often fiddly.
For example, in the case of Boomerang, programmers must often rearrange the order
of data items by recursively using operators that swap the order of adjacent
fields.  The challenge of learning these languages and using them effectively
has impeded their adoption in many settings where
well-behaved bidirectional transformations are needed.
...
%
More specifically, our system supports the synthesis of \emph{bijective lenses}, a useful 
subset of the Boomerang language.
%%  that convert
...
As inputs, our synthesis system takes specifications of
the format of both source and target files, as well as zero or more concrete examples 
of the desired transformation.  Our format specifications are supplied in the
form of regular expressions.  Because regular expressions are so widely understood,
we anticipate such inputs will be substantially easier for everyday programmers
to work with.  Importantly, these format descriptions make it possible to communicate a great
deal of information to the synthesis system.  This design choice appears to make synthesis
very robust, to help the system scale to handle
large and complex data sources, including those with nested iteration that other
synthesis algorithms preclude, and to constrain the search space so
a programmer typically need very few, if any, examples.
...
\begin{proof}
  ...
  \begin{case}[\IdentityLensType{}]
    ...
    The relation on $\LanguageOf{\Regex}$ is a bijection.
  \end{case}
  ...
  \begin{case}[\IterateLensType{}]
    ...
    Let $\String_1,\String_2$ in $\StarOf{\Regex}$, and
  \end{case}
\end{proof}
