...
updaters, and a multitude of ad hoc data transformers. 
%
Building such transformers manually---by writing two separate functions that
are intended to be inverses---is both tedious and error-prone.
%
Domain-specific languages in which both directions can be written as a
single expression are better, but can be somewhat fiddly.  For example
Boomerang, an influential bidirectional language based on the theory of {\em
  lenses}, imposes {\em unambiguity} typing constraints on most of its
primitives that programmers sometimes find confusing.

Fortunately, this is a perfect domain for type-directed program synthesis.
Boomerang's type system is based on regular expressions 

Fortunately, we can do better: We show how this programming task can
be automated using techniques of type-based program synthesis.  
%
 Specifically, we present an algorithm for synthesizing bijective
lenses between 
...
%
 (Briefly!) The main challenge is... We address it by...
% 
We evaluate our algorithm on a benchmark suite
  containing 23 examples comprising both microbenchmarks and realistic
  examples adapted from FlashFill, a tool for synthesizing string
  transformations in spreadsheets, and Augeas, a tool for bidirectional
  processing of Linux system configuration files.
  Our implementation infers lenses for all benchmark programs
  in an average of 0.134\bcp{digits?} seconds.
\end{abstract}
