...
type into $\DNFOf{A;B;C} \Leftrightarrow \DNFOf{A',B',C'}$.
...
unrolling of Kleene Star.  Attempting to normalize by putting
...
\afm{neither, we didn't prove the implementation to be complete or correct}
\item We evaluate this algorithm on 23 benchmarks, including examples from
  FlashFill, an another automatic string transformation system,
  and from the Augeas system for
...
seconds\bcp{how many significant digits?}\afm{what does this mean?}.
\end{itemize}
% end introduction
...
\bcp{Could we find a more pronounceable name for it?}
...
lens between the \CF{BibStar} and \CF{EndStar} formats.  The source
and target both contain two instances of \CF{Name}.  The annotations
help determine which \CF{Name} in the source corresponds to
which \CF{Name} in the target by tracking how the examples are mapped.
For example, the algorithm would align source and target \CF{Name}s
that share the annotation ``\CF{Kleene}''.
...
determine which expansions to perform.  Second, they reduce the
number of sequences we must manage. \ksf{how?}  Third, they reduce the sizes of
the regular expression types we must consider because we do not expand
user-defined data types during normalization.  Removing the ability to
use previously defined lenses makes certain problems in our benchmark
suite intractable, like \texttt{xml}.
...
Finally, we added examples to highlight weaknesses of
our algorithm (\texttt{cust/cap-prob} and \texttt{cust/2-cap-prob}) 
and to test situations for which we thought the tool would be
particularly useful (\texttt{cust/date-probs}, \texttt{cust/bib-prob},
and \texttt{cust/addr-probs}).   These examples convert between date
...
seconds}; the slowest example (\texttt{aug/xml}) completes in just under four 
seconds. 
...
The example \texttt{aug/xml} takes as long as it does primarily because of the
...
We constructed the pathological example \texttt{cust/2-cap-prob} to
...
In boomerang, a \CF{to\_lower} function is already provided, so the usage of
user defined lenses would solve this.  In more generality, whenever there is a
bijection between so many complex pieces of data, we expect reasonable
programmers to create auxiliary lenses, and not handle the complex task of
determining these mappings only within even more complex lenses.
