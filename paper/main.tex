...
Figure~\ref{fig:schematic-diagram-synthesis} shows provides a high level
...
converted back into a regular lens with \ToLens{}, and returned to the user.
...
functionality proivded by \OrLens{} lenses.
...
directly applied.  Boomerang's type checker addresses this problem
...
regular expression types with the subsumption rule.
...
ones.  Furthermore, DNF lenses do not composition operator, eliminating
an entire dimension of search.  From this dual stratification and lack of
composition, a very tight relationship between the structure of a well typed
DNF lens and its DNF regular expression types is created.
...
To handle this, we structure $\SynthDNFLens{}$ as two
...
composition (we describe sufficient properties for this shortly).

In particular, it is sufficient for the equivalences used in subsumption
...
of DNF lenses, when those typing derivations do not use subsumption
(if $\DNFLens$ is a DNF lens which
goes between DNF regular expressions $\DNFRegex$ and $\DNFRegexAlt$, and the
typing derivation contains no instances of the subsumption rule, then
...
algorithm.  By using these named abbreviations, \TypeProp{} guides the
transformation of regular expression types
during search by deducing when certain expansions must be taken, or when one of
a class of expansions must be taken.  However, this approach does require
\TypeProp{} to now do more than merely unroll stars, it must also replace types
...
\begin{restatable}[Soundness of DNF Lenses]{theorem}{dnfls}
  ...
  then there exist a lens, $\Lens$, and regular expressions, $\Regex$ and 
