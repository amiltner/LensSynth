...
than the syntax of lenses.  Including these format descriptions communicates a
...
support of legacy
clients~\cite{team-explorer}.  Instead of maintaining server code for each client,
...
Now, we can make a well typed \OrLens{} lens, and the two cases of an empty and a
...
To be compatible with our lens synthesis strategy, we require that our retyping
equivalence relation is the equivalence closure of a set of rewrite rules that
...
if $\Regex_1 \Rewrite
\Regex_2$ and $\RegexAlt_1 \Rewrite \RegexAlt_2$, there should exist $\Regex_3$,
$\RegexAlt_3$, and $\Lens_3$ such that $\Regex_2 \Rewrite \Regex_3$, $\RegexAlt_2
\Rewrite \RegexAlt_3$, and $\Lens_3 \OfRewritelessType \Regex_3 \Leftrightarrow
\RegexAlt_3$, where $\SemanticsOf{\Lens_3} = \SemanticsOf{\Lens_1}$.  
We call the second property \textit{bisimilarity}.
It says: Whenever $\Lens_1 \OfRewritelessType \Regex_1 \Leftrightarrow \RegexAlt_1$ and 
$\Regex_1 \to \Regex_2$, there should exist $\RegexAlt_2$ and  $\DNFLens_2$ such
that $\RegexAlt_1 \to \RegexAlt_2$,
$\Lens_2 \OfType \Regex_2 \Leftrightarrow \RegexAlt_2$, and
$\SemanticsOf{\Lens_2} = \SemanticsOf{\Lens_1}$,
and similarly for $\RegexAlt_1 \to \RegexAlt_2$.
...
Our solution for handling subsumption is to use
the \textit{star semiring axioms},
$\SSREquiv$.  These axioms are
...
orienting these rules from left to right presents us with a rewrite relation
