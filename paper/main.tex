...
%
The main technical challenges addressed by our algorithm are (1) a
...
lenses and and (2) the size and complexity of the regular expressions
...
names for regular expressions semi-opaquely during synthesis.
...
an average of 0.134\bcp{digits?} seconds.
\end{abstract}
...
\emph{lenses}~\cite{Focal2005-long, boomerang, symmetric-lenses,
  ManyMoreLensCitations}, bidirectional programs that help users solve the
classic ``view update problem'' \cite{chen2010comparative,
  FirstViewUpdateProblemCitation}.  A lens comprises two functions, called
...
original string format. \bcp{Mention some other lens-based languages,
  including several from Tokyo.  We want to be as complete as possible here,
  since these people are likely to be our reviewers.}  However, these
languages are often based around an unfamiliar collection of primitives,
which require programmers to think in two directions at the same time---a
tricky task that is foreign to most programmers.  In addition, the details
are often fiddly.  For example, in Boomerang~\cite{boomerang,
  MoreBoomerangCitesIfThereAreAny}, programmers must often rearrange the
order of data items by recursively using operators that swap the order of
adjacent fields.  The challenge of learning these languages and using them
effectively has impeded their adoption in many settings where well-behaved
...
\end{lstlisting}
  \caption{Synthesis Example: Idealized Bibtex to EndNote\bcp{single quotes?}}
...
Just as DNF regular expressions are comprised of DNF sequences and
DNF atoms, DNF lenses (\DNFLens{}) are comprised of 
sequence lenses (\SequenceLens{})
and atom lenses (\AtomLens{}).
The grammar for these terms is defined below.
...
\begin{center}
  \begin{tabular}{@{}l@{\ }c@{}l@{}}
    ...
    \SequenceLens{} & \GEq{} & $(\SequenceLensOf{(\String_0,\StringAlt_0)\SequenceLensSep\AtomLens_1\SequenceLensSep\ldots\SequenceLensSep\AtomLens_n\SequenceLensSep(\String_n,\StringAlt_n)},\sigma \in S_n)$ \\
    \DNFLens{} & \GEq{} & $(\DNFLensOf{\SequenceLens_1\DNFLensSep\ldots\DNFLensSep\SequenceLens_n}, \sigma \in S_n)$ \\
  \end{tabular}
\end{center}
...
%   statement?}
% BCP: Much better!
\bcp{Can we take away ``end case'' too?}
\end{algorithm}
...
\paragraph*{\SynthRWLDNFLens(\DNFRegex,\DNFRegexAlt,\Examples)} 
\ksf{This section is under revision; I'll work more on it Tuesday morning.}
...
and nested atoms while taking into account the information contained
in the examples.  Once it finds an alignment, it generates the
corresponding lens. 

To capture the information in the examples, the function starts by
annotating each node in the data structure representing the source DNF
regular expression with the corresponding portion of each example, and
similarly for the target. 

Next, it orders the annotated DNF regular expressions, remembering
the permutations used 
to perform those orderings.  Next, it does a tree zip of the two
ordered, exampled dnf regular expressions, to pair up subcomponents of
the two dnf regular expressions.  

Every subcomponent of one of the DNF regular expression is paired up
with a single subcomponent of the other regular expression.  These
dictate which subcomponent has a sublens between them.  However, we
have only built a lens between the normalized regular expressions, not
the original ones.  To make a lens between the original regular
expressions, it uses the permutations generated for normalization to
determine which permutations in the generated dnf lenses permit the
matched subcomponents to still map to each other.
...
recognize can be mapped to each other, and
...
Ordering the sequence components and atom subcomponents of a dnf
regular expression further normalizes it
...
\bcp{More: Other bidirectional languages include
biXid~\cite{bixid} language, which expresses 
