...
original string format. \bcp{Mention some other lens-based languages,
  ...
  since these people are likely to be our reviewers.}  However, the
unfamilarity of these languages has impeded adoption in many settings where
well-behaved bidirectional transformations are needed.

Our long-term goal is to make lens programming easier by {\em synthesizing}
lenses from examples instead of asking programmers to write them explicitly.
%
...
can be written entirely by example.  Doing this involves two significant
...
step, leaving the second to future work.  Moreover, we work with a
...
lens syntax based on a bijective subset of Boomerang.  
