...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \Function{\RigidSynthAtom}{($\StarOf{\ExampledDNFRegex},\_),(\StarOf{\ExampledDNFRegexAlt{}},\_)$}
    ...
    \State $(\DNFOf{\ExampledSequence_1 \DNFSep \ldots \DNFSep \ExampledSequence_n},\_) \gets
    \ExampledDNFRegex$
    \State $(\DNFOf{\ExampledSequenceAlt_1 \DNFSep \ldots \DNFSep \ExampledSequenceAlt_m},\_) \gets
    \ExampledDNFRegexAlt$
    \If {$n \neq m$}
    \State $\ReturnVal{\None}$
    \EndIf
...
\begin{lemma}\leavevmode
  ...
  \item
    ...
    a sequence lens, $\SomeOf{\SequenceLens}$, then $\SequenceLens
    \OfRewritelessType \Sequence 
  ...
  \item
    ...
    Let $\StringIntListSet_2 \in \ExampledDNFRegexAlt \Generates
    \ExampledDNFRegexAlt$.
    If $\RigidSynthDNFRegex(\ExampledDNFRegex,\ExampledDNFRegexAlt)$ returns
    a sequence lens, $\SomeOf{\DNFLens}$, then $\DNFLens
    \OfRewritelessType \DNFRegex 
...
\begin{proof}
  \begin{case}[atom]
    ...
    $\String_{1,1}'\Concat\ldots\Concat\String_{1,n_1'}' = \String_1$,\\
    $\StringAlt_{1,1}'\Concat\ldots\Concat\StringAlt_{1,n_1''}' = \String_1$,\\
    $\ExampledAtom =
...
\begin{lemma}
  ...
  $(\String,\StringAlt)$ in $\Examples$, $(\String,\StringAlt) \in
  \SemanticsOf{\DNFLens}$, then $\SynthDNFLens(\DNFRegex,\DNFRegexAlt,\Examples)$ will
  return a DNF lens.
\end{lemma}
\begin{proof}
