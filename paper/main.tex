...
\documentclass[sigplan,acmsmall,anonymous]{acmart}
...
theorems.  We have omitted these theorems for space, and have included these
...
\cite{Krob}.\footnote{There are other complete axiomitizations for
  regular expression equivalence, such as Kozen's\cite{kozen-complete} and Solomaa's\cite{salomaa-complete}.
  We focus on Conways for the sake of specificity, but discuss alternative
...
pretty printing the generated lenses make the synthesized lenses easy to
understand.
...
are provided submitted to POPL 2018 as a part of the anonymous gi \afm{fill}.
...
We generate lenses of sizes between \MinLensSize AST
nodes, for simple problems like changing how dates are represented, and
\MaxLensSize AST nodes, for complex tasks like transforming arbitrary XML of depth
...
\begin{itemize}
\item[\FullMode{}:] All optimizations are enabled.  We run the core synthesis
  algorithm presented in \SynthLens{} and apply previously
  defined lenses whenever possible.
\item[\NoPDMode{}:] Like \FullMode{}, but previously
  defined lenses are not applied.
\item[\NoFPEMode{}:] Like \NoPDMode{}, but \FixProblemElts{} is never called,
...
Attempting
a na\"{i}ve type-directed synthesis on the bijective lens combinators doesn't
work at all.  With a 
...
procedure, labeled \NoUDMode{} in the graph, that performs 
\emph{no} intelligent expansion search and makes no use
of user-defined data types succeeds on \NoUDSuccessCount{} of the benchmarks.
...
abstractions provide.  In particular, because of the large fanout of possible 
...
unreasonably slow.  To leverage the benefit of keeping data types abstract, we
need a more intelligent way to traverse expansions.
...
solvable by \NoPDMode{}, all expansions
were forced.  However, the remaining \ExpansionsNotAllForcedNoLC{} still require enumerative search.
The \NoFPEMode{} version of the algorithm will find lenses for
...
Our full synthesis algorithm (\FullMode{}) makes use of
previously defined lenses to 
...
The benchmark that requires using previously define lenses is also
...
omitted, a number of examples are needed to correctly identify which field maps
to which.
...
In searching for equivalent regular expressions, we focused Conway's equational
theory. 
However, there are a number of alternative axiomatizations for regular
expression equivalence.  However, these theories are not equational theories;
to apply certain inference rules side conditions must be satisfied.
Using these axiomatizations does not permit a simple search strategy --
we can no longer merely apply rewrite rules, for some we must
first confirm that the side conditions are satisfied. To avoid these
complications, we focus on the equational theory presented by Conway. 
