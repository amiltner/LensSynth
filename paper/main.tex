...
of domain-specific languages~\cite[etc.]{yag+:pldi16, osera+:pldi15,
  ...
  to be sure that we're doing a good job of it.}\saz{Yes, this is
  definitely incomplete--these are only type-directed, functional
  programming synthesis systems.  We haven't mentioned any of
  Gulwani's work, Kuncak, etc.  We either need to add a lot of
  citations or focus only on type-directed systems. }, many guided by
...
\afm{we don't actually prove anything about our
  implementation}\BCP{Really?  Yikes.  So then what does our algorithm have to do
with synthesizing DNF lenses?}
...
system (\S \ref{evaluation}).  Our tool succeeds on
all of our benchmarks, with an average synthesis time of 0.134
seconds\bcp{how many significant digits?}\afm{what does this mean?}\bcp{How
  many of the digits ``1'', ``3'', and ``4'' are significant?}.
\end{itemize}
...
difficult.\footnote{Note that xml is not a regular language, so 
...
labelling the appropriate labels for other occurrences of those substrings.
