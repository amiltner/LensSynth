...
composition in DNF lenses, we do not want the equivalences to make us no longer
...
on their associated DNF forms is compatible with our lens synthesis strategy.
...
\DNFRegexAlt''$, where $\DNFLens'$ has the same semantics as $\DNFLens$.  We call this
property \textit{confluence}.
Second, whenever $\DNFLens \OfRewritelessType{} \DNFRegex \Leftrightarrow \DNFRegexAlt$ and 
...
\DNFRegexAlt'$.
We call this property \textit{bisimilarity}.
...
The corresponding rewriting relation on DNF regular expression, which orients these rules from
left to right is both confluent and bisimilar.  
This choice of definitional equivalence rules is the coarsest subset of the
regular expression equivalence rules we could find has analogous rewrite rules that still are confluent
