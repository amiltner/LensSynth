...
highly efficient solutions, able to synthesize lenses
...
expressions, $\Regex$ and $\RegexAlt$, and confirm that they parse the
...
are provided submitted to POPL 2018 as a part of the anonymous supplementary
material.
...
target to make the transformations bijective and thus suitable
for our study.
...
Many of the examples from Flash Fill are non-bijective because
...
Flash Fill paper~\cite{gulwani-popl-2014} that were close to bijections.
...
\FullMode{} synthesized all 49 benchmarks, \NoCSMode{} synthesized 48
...
theory, despite there being a number of alternative axiomatizations for regular
expression equivalence, like Kozen's~\cite{kozen-complete} and
Salomaa's~\cite{salomaa-complete}.  However, Kozen and Salomaa's axiomatizations
are not equational theories; 
...
out algorithm could no longer merely apply rewrite rules, to apply some rules,
it would first have to confirm that the preliminary conditions are satisfied. To
avoid 
these complications, we focused on the equational theory presented by Conway. 
