...
an average of 283 milliseconds.
\end{abstract}
...
existing bidirection languages make them ill
suited to synthesis in their raw form.  These languages have many equivalences
on their types -- Boomerang's types are regular expression pairs, and there are
infinite equivalent regular expressions.  To synthesize terms from these, we
...
Similarly to how lenses have pairs of regular expressions as types, DNF lenses
have pairs of DNF regular expressions as types.
Furthermore, the typing derivation of a DNF lens fixes where and how
equivalences on DNF regular expressions can be processed with
a two-phase typing derivation.  In the first
phase of the typing derivation, equivalences on the languages are handled, and
in the second phase of the typing derivation, the terms are aligned with
the types.
...
Our algorithm takes as input specifications given as a pair of regular
expressions describing the data formats, as well as a list of input/output
examples.
...
until \RigidSynth{} successf completes.
Two-phased synthesis algorithms are present in many other
systems~\cite{?,?,?}, but we are the first that ties this two-phased synthesis
...
lens combinators (\S\ref{background}) and illustrate
the essential features of our algorithm with our extended example
(\S\ref{example}), we offer the following technical contributions:
...
support of legacy
clients~\cite{team-explorer}.  Instead of keeping server code for each client,
...
old apis are given in Figure~\ref{fig:idealized-representations}.  If an old client GETs a work
item, it first retrieves that task using the modern API, and then uses the
lens to convert this task into the legacy format.  Similarly, if an old
client PUTs a task, it converts that task into the new format, and
then inputs that task through the modern APIs.  The two representations
contain the same information, but they are presented differently.
...
actions depending on the number of text characters present.  This performing of
different actions is achieved through the use of an \OrLensType{} lens.  The typing
derivation for an \OrLensType{} lens is:
\[
  \inferrule
  {
...
An \OrLensType{} lens applies one of two lenses, depending on which of the
regular expressions the input string is in.
...
However, our types do not fit within this framework.  While
\CF{\color{darkbrown} modern\_title} is a regular expression with an
\OrRegexType{} as its outermost operation, \CF{\color{darkbrown} legacy\_title}
is a regular expression with an outermost \ConcatRegexType{}, so an
\OrLensType{} lens would not be well typed.  To handle this problem, Boomerang
...
Now, the \OrLensType{} rule applies, and the two cases of an empty and a
...
expressions.  By unfolding * operators in one or both types, using the axioms of
a star semiring, we obtain equivalent (but larger) DNF regular expression types.
...
definition.  In this way, differing  of occurrences of the same name
...
Recall the previous example of synthesizing a lens between the two regular
expressions:
\begin{align*}
  \Regex_0 &= \VarCF{legacy\_title}\\
  \RegexAlt_0 &= \VarCF{modern\_title}
\end{align*}
This synthesis is done in two major phases.
\textit{\textbf{Phase 1: Traversing Equivalences}} uses built in deductive
techniques, and breadth first search when those techniques fail, to infer which
equivalence rules to apply.
For the regular expression pairs generated through the equivalence traversal,
\textit{\textbf{Phase 2: Generating DNF Lenses}} applies type directed synthesis to
the regular expression types to create the DNF lenses.

\paragraph*{Phase 1: Traversing Equivalences.}

At the start of phase one, the input regular expressions are put into DNF form
by being fully distributed, and having their associativity information removed.
Immediately, the synthesis algorithm knows that no lens can be generated between
$\Regex_0$ and $\RegexAlt_0$, as only the identity lens can
be applied to variables.  Through looking at the variables reachable from
\VarCF{legacy\_title} and \VarCF{modern\_title}, our synthesis algorithm
recognizes that both \VarCF{legacy\_title} in $\Regex_0$ and
\VarCF{modern\_title} in $\RegexAlt_0$ must be replaced by their definitions.

Upon doing this, the synthesis task now becomes finding a lens between:
\begin{align*}
\Regex_1 &= \CF{\textcolor{blue}{"<Field Id=2>"}
  \textcolor{darkbrown}{text\_char}* \textcolor{blue}{"</Field>"}}\\
\RegexAlt_1 &= \CF{\textcolor{blue}{""} | (\textcolor{blue}{"Title="}
  \textcolor{darkbrown}{text\_char} \textcolor{darkbrown}{text\_char}*
  \textcolor{blue}{","})}
\end{align*}
While these may look similar, we can deduce another
expansion that must be taken.  There is an instance of \VarCF{text\_char} under
no stars in the expansion of \VarCF{modern\_title} but there is no such
\VarCF{text\_char} for the expansion of \VarCF{legacy\_title}.  Our deduction
engine recognizes one of the following 3 expansions must occur
\begin{enumerate}
\item \CF{\VarCF{text\_char}*} must be unrolled on the right in the expansion
  of \VarCF{legacy\_title}
\item \CF{\VarCF{text\_char}*} must be unrolled on the left in the expansion
  of \VarCF{legacy\_title}
\item \VarCF{text\_char} must be replaced by its definition in the expansion
  of \VarCF{modern\_title}
\end{enumerate}

At this point, the deduction system no longer knows that an expansion must
occur, and the generated pairs of regular expressions are sent to \RigidSynth{}
to synthesize a lens.  If \RigidSynth{} cannot find any lenses between these
regular expressions, then single expansions on these regular expressions are
performed, and given back to the synthesis engine.

\paragraph*{Phase 2: Synthesizing Lenses.}
...
$\DNFRegexAlt_0$. This value is used for the priority of the queue element.
...
Specifics of the orderings likely only in full version.
...
\emph{star depth} is critical to how \Expand{} works.
...
\VarCF{modern\_title}, it is impossible to expose \VarCF{legacy\_title} in
\VarCF{modern\_title}.  This is because the set of possible reachable pairs of
data types and star depths in \VarCF{modern\_title} is $\SetOf{(\ModernTitle,0),
(\TextChar,0),(\TextChar,1)}$.  Because no number of expansions can reach
$\VarCF{\LegacyTitle}$, it must be expanded for a lens to be found.  This is
procedure continues calling itself until no more expansions can be forced.

\ExpandRequired{} find a lot of necessary expansions, but it only does a bit of
the work.  There are still times when it is easy to identify that an expansion of
some form must be performed.  For any pair of types that have a rewriteless lens
...
\CF{(\textcolor{blue}{"Title="}
  \textcolor{darkbrown}{text\_char} \textcolor{darkbrown}{text\_char}*
  \textcolor{blue}{","}) | \textcolor{blue}{""}}, respectively, we can see that the modern expansion
has an instance of \TextChar{} at depth 0, where the legacy one does not.  This
means that we will try to reveal $(\TextChar,0)$ in the legacy expansion.
Revealing this will give back 2 DNF regular expressions, one from an application of
\AtomUnrollstarLeftRule{}, and one from an application of
\AtomUnrollstarRightRule{}.

While \FixProblemElts{} is able to get a huge number of expansions, there are
certain edge cases which it doesn't handle.  This typically appears in very
large data formats when irregular transformations are applied.  A minimal
example can be seen in trying to synthesize the identity transformation between
\CF{\textcolor{blue}{""} | \textcolor{darkbrown}{U} |
  \textcolor{darkbrown}{UU}(\textcolor{darkbrown}{U}*)} and
\CF{\textcolor{blue}{""} | \textcolor{darkbrown}{U}(\textcolor{darkbrown}{U}*)}.
Another expansion must be applied, but the pairs of user defined data types at
certain depth is the same.  When this situation arises, we perform all possible
single expansions for a random search.  Typically, this situation doesn't arise
and we have sufficient information to use our more informed search strategies.
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \Function{\RigidSynthAtom}{$\StarOf{\DNFRegex},\StarOf{\DNFRegexAlt}$}
    \Switch{$\RigidSynth(\DNFRegex,\DNFRegexAlt)$}
    \Case {\SomeOf{\DNFLens}}
    ...
    \Function{\RigidSynthSequence}{$\Sequence,\SequenceAlt$}
    ...
    \State $\ReturnVal{\None}$
    \EndIf
    \State $\sigma_1 \gets \SortingOf{\AtomLeq}{\ListOf{\Atom_1;\ldots;\Atom_n}}$
    \State $\sigma_2 \gets \SortingOf{\AtomLeq}{\ListOf{\AtomAlt_1;\ldots;\AtomAlt_n}}$
    ...
    \Call{\Map}{\RigidSynthAtom,\mathit{ABs}}$
    ...
    \Function{\RigidSynth}{$\DNFRegex,\DNFRegexAlt$}
    ...
    \State $\ReturnVal{\None}$
    \EndIf
    \State $\sigma_1 \gets \SortingOf{\SequenceLeq}{\ListOf{\Sequence_1;\ldots;\Sequence_n}}$
    \State $\sigma_2 \gets \SortingOf{\SequenceLeq}{\ListOf{\SequenceAlt_1;\ldots;\SequenceAlt_n}}$
    \State $\sigma \gets \InverseOf{\sigma_2} \circ \sigma_1$
    ...
    \Call{\Map}{\RigidSynthSequence,\mathit{STQs}}$
...
This identification is done through finding intelligent orderings.  For
...
\SequenceLeq \Sequence$.  Utilizing these orderings, aligning the components
becomes merely sorting and zipping them.  Furthermore, through composing the
permutations required to sort the sequences, we can discover the permutation
used in the lens.

Furthermore, through incorporating how examples are parsed in the orderings, we
can guarantee not only that there will be a lens between the two, but also that
the lens will satisfy the examples!
...
\begin{lstlisting}
 ...
 | @""@
\end{lstlisting}
As it passes to \RigidSynth{}, the two clauses of legacy get ordered in the same
order they are right now, first \CF{\StringCF{"<Field Id=2></Field>"}}, then
\CF{\StringCF{"<Field Id=2>"} \VarCF{text\_char} \VarCF{text\_char}*
  \StringCF{"</Field>"}}.  However, the two clauses of the modern version get
reordered, into \CF{\StringCF{""}} first, followed by \CF{\StringCF{"Title="}
  \VarCF{text\_char} \VarCF{text\_char}* \StringCF{","}}.  This generates the
overall permutation of swapping the clause ordering.  Then, the clause lens
between \StringCF{"<Field Id=2></Field>"} and \StringCF{""} is attempted to be
found (and would quickly be found as \CF{const(\StringCF{"<Field
    Id=2></Field>"},\StringCF{""})}).  Next the clause lens between
\CF{\StringCF{"<Field Id=2>"} \VarCF{text\_char} \VarCF{text\_char}*
  \StringCF{"</Field>"}} and \CF{\StringCF{"Title="}
  \VarCF{text\_char}} $ $ \CF{\VarCF{text\_char}* \StringCF{","}} would be attempted to
be found.  In this, the atoms would not be reordered, and their lenses would
quickly be found.  Then, the generated atom lenses could be combined into a
clause lens.  Finally these two generated clause lenses would be combined into a
final DNF lens.
...
\begin{figure}
  \begin{tikzpicture}
    \begin{axis}[
      ybar,
      symbolic x coords={Naive,FF,FE,DNF,DNFEI,Full},
      ylabel=Tests Completed in 2 Minutes,
      xlabel=Algorithm,
      grid=major,
      xmajorgrids=false,
      ymin=0,
      ...
      \addplot 
      coordinates {(Naive,0) (FF,3) (FE,4) (DNF,12) (DNFEI,25) (Full, 26)};
    \end{axis}
  \end{tikzpicture}
...
benchmark programs derived from practice.  We investigated (TODO: Fill in).
The tests were run on a 2.5 GHz Intel Core i7 processor with 16 GB of 1600 MHz DDR3
running Mac OS X Yosemite.
...
Across all examples, the average synthesis time is 283 milliseconds;
...
example of this phenomenon in the example \CF{ff/extract-fname}, where the
