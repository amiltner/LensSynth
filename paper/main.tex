...
\item
  We present \Optician{}, an efficient, type-directed synthesis algorithm for
  synthesizing lenses (\S\ref{algorithm}).  We prove that if there is a lens
  that satisfies the input specification, \Optician{} will return such a lens.

\item
  We evaluate our core algorithm, its optimizations, and
...
\SynthDNFLens{} is quite fast, with \RigidSynth{} running in polynomial time and
\TypeProp{} using a number of deductive techniques to typically propose a small
number ot types before finding the correct one,
...
$\Sigma$ is the alphabet of individual characters $\Character$;
...
$\String_1\Concat\StringAlt_1=\String_2\Concat\StringAlt_2$ we have
...
subsumption rule (which we will write as $\Lens \OfRewritelessType \Regex
...
An overview of the full synthesis algorithm, \Optician{}, is given in
Algorithm~\ref{alg:synthlens}.  \Optician{} takes the source and target regular
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \Statex
    \Function{Optician}{$\Regex,\RegexAlt,\Examples$}
...
\begin{restatable}[Algorithm Soundness]{theorem}{algs}
  ...
  examples $\Examples$, 
  if $\Lens = \Optician(\Regex,\RegexAlt,\Examples)$, then
...
\begin{restatable}[Algorithm Completeness]{theorem}{algc}
  ...
  $\Lens \OfType \Regex \Leftrightarrow \RegexAlt$ and for all
  $(\String,\StringAlt)$ in $\Examples$, $(\String,\StringAlt) \in
  \SemanticsOf{\Lens}$, then $\Optician(\Regex,\RegexAlt,\Examples)$ will
  return a lens.
\end{restatable}
...
On one benchmark \ForceExpand{} infers \afm{26} expansions, and \ForceExpand{}
infers a median of \afm{11} expansions.
...
\begin{definition}
  ...
  with rewriteless lenses.  In particular, $\Regex \DNFLensHasSemanticsOf{\Lens}
  \RegexAlt$ if there exists $\Lens'$ such that $\SemanticsOf{\Lens} =
  \SemanticsOf{\Lens'}$ and $\Lens' \OfRewritelessType \Regex \Leftrightarrow
  \RegexAlt$.
\end{definition}

\begin{lemma}
  If $\IsConfluentWithPropertyOf{\Rewrite}{\DNFLensHasSemanticsOf}$, then
  $\IsConfluentWithPropertyOf{\Rewrite_{\ToDNFRegex}}{\DNFLensHasSemanticsOf}$.
\end{lemma}
\begin{proof}
  Let $\DNFLens \OfRewritelessType \DNFRegex \Leftrightarrow \DNFRegexAlt$.
  From the soundness proofs, $\DNFLens$ can be split into multiple compositions
  of rewriteless lenses, which all have typings of $\ToDNFRegexOf{\Regex} =
  \DNFRegex$ and $\ToDNFRegexOf{\RegexAlt} = \DNFRegexAlt$.
  Consider $\DNFRegex \Rewrite_{\ToDNFRegex} \DNFRegex_1$ and $\DNFRegexAlt
  \Rewrite_{\ToDNFRegex} \DNFRegexAlt_1$.  This means that $\Regex \Rewrite
  \Regex_1$ and $\RegexAlt \Rewrite \RegexAlt_1$.  By assumption, that means that
  there exists $\Regex_2$ and $\RegexAlt_2$ such that, for each lens $\Lens'$ in
  the compositions, there exists $\Lens''$ such that $\Lens'' \OfRewritelessType
  \Regex_2 \Leftrightarrow \RegexAlt_2$, with the same semantics.  Then, using
  rewriteless completeness, there are DNF lenses $\DNFLens'' \OfRewritelessType
  \ToDNFRegexOf{Regex_2} \Leftrightarrow \ToDNFRegexOf{\RegexAlt_2}$.  Then,
  using composition of rewriteless DNF lenses, we are done.
\end{proof}

\begin{lemma}
  If $\IsBisimilarWithPropertyOf{\Rewrite}{\DNFLensHasSemanticsOf}$, then
  $\IsBisimilarWithPropertyOf{\Rewrite_{\ToDNFRegex}}{\DNFLensHasSemanticsOf}$.
\end{lemma}
\begin{proof}
  The same proof as the previous lemma can be applied with very minor
  alterations.
\end{proof}

Thus our claim about having confluence and bisimilarity of lenses as a
