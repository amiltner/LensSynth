...
unrolling of Kleene Star.\BCP{Why not?}  \afm{Attempting to normalize by putting
...
rule can be applied an arbitrarily large number of times.}
% \bcp{Why
...
\emph{without adding a composition operator}.  Obviating the need for a
...
other hand, the algorithm must be able to guarantee that the generated lens
satisfies the many constraints introduced by these large and complex types. 
...
Conway's axioms consist of the semiring axioms (associativity of
\OrRegexType{} and \ConcatRegexType{}, commutativity of \OrRegexType{},
distributivity of \OrRegexType{} over \ConcatRegexType{}, and identities for
\OrRegexType{} and \ConcatRegexType{}) plus the following 
...
While this equational theory is complete, using the rewrite rules associated
with
each equivalence does not help us much for
...
\begin{figure}
  ...
  \caption{Lens Typing\bcp{We can save quite a few lines by moving the rule
      names}\afm{What do you mean by this?}\saz{This}}
...
BibTex':
  $\EmptyString$  | ($\mathit{author=}$ { Name $\mathit{,}$ Name Names
...
\bcp{All the double quotes make the examples quite hard to read.  Could we
  ...
  keep string constants in tt font?}
  }
\end{figure}
...
\begin{figure}
  ...
  \begin{tabular}{|l|c|c|c|c|}
    ...
    & \bfseries Avg\# Exs & \bfseries Max\# Exs
  ...
  \caption{Impirical Results.  
  \ksf{The benchmarks are sorted in increasing order by the sizes of
  the regular expression types.}   The \textbf{Time} column reports
  the average running time of 10 synthesis runs.  The \textbf{Avg \#
  Exs} reports the average required number of randomly generated
  examples. The \textbf{Max\# Exs} represents the maximum
  number of examples that the user would have to supply to force the
  synthesizer to explore all choices.
}
\end{figure}

We evaluated our synthesis algorithm by applying it to \ksf{XX} benchmark
programs derived from practice.   We investigated how long it took to
synthesize each lens, how many user-supplied examples synthesis required,
and how important user-defined types were to the process.
Figure~\ref{fig:evaluation-data} shows the results for each of our
benchmark programs.  \ksf{The rows are sorted in increasing order by
the sizes of the regular expression types.}
...
On average, synthesis required \ksf{0.134 seconds} and \ksf{UU
user-supplied examples.}

\paragraph*{Benchmark Suite Construction}
We constructed our benchmarks by adapting examples from Augeas and
FlashFill and by handcrafting specific examples to test various
features of the algorithm.
Specifically, we derived \ksf{17} of the benchmark programs by
...
FlashFill paper~\cite{gulwani-popl-2014} .

Specifically, \ksf{YY} of the
programs came from Augeas data transformation problems and \ksf{ZZ}
from FlashFill examples.  We wrote \ksf{WWW} custom examples to test
various features of the algorithm.
...
program.  FlashFill takes input and 
...
\begin{proof}
  ...
  \begin{case}[\ParallelDNFStructuralRewriteRule{},\ParallelDNFStructuralRewriteRule{}]
    ...
    \item
      ...
      \DNFOf{\SequenceOf{\String_{i,0}}} \ConcatDNF \DNFRegex_{i,1}'
      \ConcatDNF \ldots \ConcatDNF \DNFRegex_{i,n_i}' \ConcatDNF
      \DNFOf{\SequenceOf{\String_{i,n_i}}}$, and $\DNFLens_i$ has the identity
      ...
      \DNFRegex_1' \OrDNF \ldots \OrDNF \DNFRegex_n'$, so
    ...
    \item
      ...
      $\DNFRegex_{\sigma(i)}'$ such that $\DNFRegex_{\sigma(i)} =
      \DNFOf{\SequenceOf{\String_{i,0}}} \ConcatDNF \DNFRegex_{\sigma(i),\sigma_i(1)}
      \ConcatDNF \ldots \ConcatDNF \DNFRegex_{\sigma(i),\sigma_i(n_i)} \ConcatDNF
      \DNFOf{\SequenceOf{\String_{\sigma(i),n_i}}} \ParallelRewrite \DNFRegex_i'$,
      and there exists $\DNFLens_i'$ such that
      $\DNFLens_i' \OfRewritelessType
      \DNFOf{\SequenceOf{\String_{i,0}}} \ConcatDNF \DNFRegex_{i,1}'
      \ConcatDNF \ldots \ConcatDNF \DNFRegex_{i,n_i}' \ConcatDNF
      \DNFOf{\SequenceOf{\String_{i,n_i}}} \Leftrightarrow
