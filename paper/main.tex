...
all benchmarks, with an average synthesis time of 283
milliseconds.
\end{itemize}
...
We have shown that two properties are sufficient to ensure that any
equational theory of regular expressions implemented via rewrite rules $\to$
on their associated DNF forms is compatible with our lens synthesis
strategy.
These properties involve the DNF Lens typing judgement 
$\DNFLens \OfRewritelessType \DNFRegex \Leftrightarrow \DNFRegexAlt$,
defined later in Figure~\ref{fig:dnf-lens-typing}, that can be read
``DNF lens $\DNFLens$ maps between data typed with source DNF regular 
expression type $\DNFRegex$ and target type $\DNFRegexAlt$.
We call the first property \textit{confluence}.  It says:
Whenever $\DNFLens \OfRewritelessType \DNFRegex \Leftrightarrow \DNFRegexAlt$,
if $\DNFRegex \Rewrite
\DNFRegex'$ and $\DNFRegexAlt \Rewrite \DNFRegexAlt'$, there should exist $\DNFRegex''$,
$\DNFRegexAlt''$, and $\DNFLens'$ such that $\DNFRegex' \Rewrite \DNFRegex''$, $\DNFRegexAlt'
\Rewrite \DNFRegexAlt''$, and $\DNFLens' \OfRewritelessType \DNFRegex'' \Leftrightarrow
\DNFRegexAlt''$, where $\DNFLens'$ has the same semantics as
$\DNFLens$.  
We call the second property \textit{bisimilarity}.
It says: Whenever $\DNFLens \OfRewritelessType{} \DNFRegex \Leftrightarrow \DNFRegexAlt$ and 
$\DNFRegex \to \DNFRegex'$, there should exist $\DNFRegexAlt'$ and  $\DNFLens'$ such
that $\DNFRegexAlt \to \DNFRegexAlt'$ using the \textit{same} rewrite rule, 
$\DNFLens' \OfType{} \DNFRegex' \Leftrightarrow \DNFRegexAlt'$, and
$\SemanticsOf{\DNFLens} = \SemanticsOf{\DNFLens'}$, and similarly for $\DNFRegexAlt \to
\DNFRegexAlt'$.
...
Our solution for handling subsumption is to define
 the \textit{definitional equivalence rules} 
 $\DefinitionalEquiv$ to 
...
\end{center}
\noindent
The corresponding rewriting relation on DNF regular expressions, which
orients these rules from left to right, is both confluent and
bisimilar and so is compatible with our lens synthesis strategy.  
Definitional equivalence is the 
coarsest subset of regular expression equivalence we could
find that has analogous rewrite rules that are still confluent and
...
\textit{\textbf{Step 1:
  Inlining Definitions}} uses built in deductive techniques to infer when
user defined regular expressions must be replaced by their definitions.
\textit{\textbf{Step 2: Traversing Equivalences}} searches through various equivalences
of the regular expression types to find different representations of the types.
Finally, \textit{\textbf{Step 3: Creating Lenses}} applies type directed synthesis to the
...
\end{lstlisting}
  ...
  \caption{
    \CF{BibTeX} and \CF{EndNote} with various transformations performed.
...
However, now a single problem of finding a lens between regular expressions has
become two problems!  Furthermore, trying to find lenses satisfying either of these
two specifications could create even more problems.  Because of this
problem explosion, it is
important to find a way to determine which subproblem should be approached next.
...
running Mac OS X Yosemite.  On average, synthesis required 283 milliseconds
