...
\bcp{Larger comment: Is this seriously using {\em null} to
  ...
  laugh at us.}\afm{is $\bot$ better?}\bcp{Still a bit horrible.}
\end{algorithm}
...
programs derived from practice.   We investigated the correctness of
the inferred lenses, how long it took to
...
\afm{We also created custom example to highlight some weaknesses of our
  algorithm, and to test our system on some situations we thought the tool would
  be useful for.  The benchmarks which highlight the algorithm's weaknesses are
  the capitalization problem, and the double capitalization problem.  The other
  custom examples are scenarios where the algorithm would be very useful -
  converting between date formats, converting between bibliography formats, and
  converting address information from CSV form into letter form.}
...
\afm{The size is given as the size of the total number of tokens in the
  regular expressions.  For example, \BaseRegexType{} string regular expression has only
  one token, where a \ConcatRegexType{} has one for the concatenation operator,
  as well as all those in the regular expressions it is built out of.  This size
  gives an approximate metric for how complex the data formats are.}
...
\afm{There is a slight correlation with complexity of the program specification
  with the amount of time synthesis takes.  However, this correlation is only
  rough because other factors, like degree of difference between the
  specifications, has a large impact.}
...
Figure~\ref{fig:evaluation-data}, the column \textbf{Avg\# Exs} shows
...
number of required examples is \ksf{XXX}. The mode is zero, and the
maximum is 19.  We note that a clever user may be able to reduce the
...
For example, the benchmark aug/addr-probs requires examples because it
must differentiate between and first and last names in both the
source and target and map between them appropriately.

As the bib\_prob example illustrates, the average number of examples to
...
Column \textbf{Max\# Exs} in Figure~\ref{fig:evaluation-data} lists
...
In synthesizing the lenses for our benchmark suite, we found that not
being able to synthesize lenses for which retyping used full regular
expression equivalence was not a detriment.  The only regular
expression equivalences that occured in the benchmarks were ones that
we had already incorporated in our equivalences.
...
representations.  The only places where \StarRegexType{} equivalences
were used was in differentiating number of iterations, and in placing
separators, the equivalences of which are already accounted for in
$\DefinitionalEquiv$.
% end evaluation
