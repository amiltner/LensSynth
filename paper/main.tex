...
combinatorial explosion in the search space size caused by the unique lens
typing rules
(2) the size and complexity of the regular expressions
describing real-world data formats.  We address (1) by moving the
search to a different space where both regular expressions and lenses are
constrained to a ``disjunctive normal form'' and address (2) by only replacing
  user-defined regular expressions by their definitions when non-identity
  transformations must be applied to them.

We evaluate our algorithm on a benchmark suite of \afm{26} examples,
including both
...
\afm{Is big data the best start?  I know it's hip, but I feel like there's a
  disconnect as we don't talk about big-data types of inputs for the rest of the
  paper}
Companies are increasingly performing big data anlytics for consumer studies,
process evaluation, performance optimization, and more.
...
source format.  Lens-based languages use a single expression to express both
\emph{get} and \emph{put}.
Furthermore, these resulting functions are guaranteed to satisfy the
desired invertibility properties, when the expression they are derived from is
well typed.
...
%
However, the details of these languages are
often fiddly, making lens programming a slow and tedious process.
For example, Boomerang
programmers must often rearrange the order of data items by recursively
using operators that swap the order of adjacent fields.
The Boomerang type checker is also very strict, disallowing many programs
because they contain ambiguity about how certain data is transformed.
While these languages provide strong bidirectional guarantees, worrying about
minute details while satisfying the finicky type system slows down development.
...
other features of the language and its type system make it ill
...
However, the typing judgements for DNF lenses limit how equivalences can be
used, and require no composition operator.
...
\begin{figure}
  ...
  \begin{tikzpicture}[auto,node distance=1.5cm]
    \node[text width=2.3cm,align=center,draw,rectangle] (todnfregex) {\ToDNFRegexText{}};
    
    \node[align=center] (regex1) [above left = .1cm and .2cm of todnfregex]{ RX1 };
    \node[align=center] (regex2) [below left = .1cm and .2cm of todnfregex]{ RX2 };
    
    \node[align=center] (dnfregex1) [above right = .1cm and .2cm of todnfregex]{ DNFRX1 };
    \node[align=center] (dnfregex2) [below right = .1cm and .2cm of todnfregex]{ DNFRX2 };

    \node[text width=2.3cm,align=center,draw,rectangle] [below right = .1cm and .2cm of dnfregex1] (dnflenssynth) {\SynthDNFLens{}};
    
    \node[align=center] [right = .2cm of dnflenssynth] (dnflens) {DNFLens};
    
    \node[text width=2.3cm,align=center,draw,rectangle] [right = .2cm of dnflens] (tolens) {\ToLens{}};
    
    \node[align=center] [right = .2cm of tolens] (lens) {Lens};
    
    \path[->] (regex1.south east) edge (todnfregex.north west);
    \path[->] (regex2.north east) edge (todnfregex.south west);
    
    \path[->] (todnfregex.north east) edge (dnfregex1.south west);
    \path[->] (todnfregex.south east) edge (dnfregex2.north west);
    
    \path[->] (dnfregex1.south east) edge (dnflenssynth.north west);
    \path[->] (dnfregex2.north east) edge (dnflenssynth.south west);
    
    \path[->] (dnflenssynth) edge (dnflens);
  ...
  \end{tikzpicture}
  \caption{Schematic Diagram for Synthesis Algorithm.  Regular expressions are
    given as input, and converted into DNF form.  Then type-directed synthesis
    techniques synthesize a DNF lens from these DNF regular expressions.
    Finally, the synthesized DNF lens is converted into a lens in the original
    language.}
  \label{fig:schematic-diagram-synthesis}
\end{figure}

DNF regular expressions and DNF lenses are critical to the success to our
synthesis algorithm,
shown in Figure~\ref{fig:schematic-diagram-synthesis}.  First, we convert the
regular expressions into DNF regular expressions using \ToDNFRegexText{}.  Next,
\SynthDNFLens{} performs type-directed synthesis on the DNF regular
expressions to synthesize a DNF lens.  Finally, this DNF lens is converted
back into a regular lens with \ToLens{}, and returned to the user.

\paragraph*{Contributions.}  Our synthesis algorithm is intended to make
bidirectional programming more accessible by removing many of
the pain points inhibiting broader adoption.
We begin by motivating our problem
...
we offer the following technical contributions: \bcp{The way this bit is
  ...
  synthesis.  } \afm{I tried changing the contributions up a bit (in particular,
  the second bullet), but I'm not
  sure if it achieves the desired effect.}
...
\item We present an optimized, type-directed synthesis algorithm for
    synthesizing DNF lenses (\S\ref{algorithm}).  We prove that this algorithm,
    when used in the schematic diagram above, provides a sound and complete
    synthesis algorithm for lenses.

  \item We evaluate our core algorithm, its optimizations, and synthesis tools in
    a similar domain on \afm{26} benchmarks, including examples derived from
    Flash Fill~\cite{gulwani-popl-2014} and the Augeas~\cite{augeas}
    system (\S \ref{evaluation}).
    We show our algorithm significantly outperforms systems in similar domains.
    We show our optimizations are critical for synthesizing many of the higly
    complex lenses in our benchmark suite.
    Our full algorithm succeeds on all benchmarks in under 5 seconds.
  \end{itemize}
...
to synthesize them.\bcp{Will our system really make it all that much better?
 ...
 number of existing endpoints...} \afm{I think so, we're reducing it by a
 constant factor.}
...
cannot be applied directly.  Boomerang's type-checker addresses this problem
through representing types as regular languages, not regular expressions.
We formalize it here by allowing conversions between equivalent
...
With this rule, the synthesis algorithm can convert \CF{\color{darkbrown}
  legacy\_title} into
%
\begin{lstlisting}
...
$(A | B) C \Leftrightarrow A' C' | B' C'$, our algorithm will first convert this
type into $\DNFOf{\SequenceOf{A;C};\SequenceOf{B;C}} \Leftrightarrow
\DNFOf{\SequenceOf{A';C'};\SequenceOf{B';C'}}$, where
...
However, translation to DNF form does not fully normalize the regular expressions,
...
\begin{figure}
  ...
  \begin{tikzpicture}[auto,node distance=1.5cm]
    ...
    \node (synth-name) [above=.5cm of middle] {\SynthDNFLens{}};
...
Consequently, a type-directed synthesis algorithm must still search through some
regular expression equivalences.
To handle this, we structure our synthesis algorithm as two
...
equivalent to the input regular expressions.
...
expression types. The second synthesizer, \RigidSynth{}, perform a syntax-directed
search based on the structure of the provided DNF regular expressions.
If the second phase fails, \TypeProp{} learns of that failure,
and proposes new candiate DNF regular expression pairs.

\paragraph*{Challenge 2: Large Types}
This algorithm is complete, but the conversion to DNF encurs an exponential blowup.
In practical examples, the regular expressions describing complex {\em ad hoc} data
formats may be very large, making the exponential blowup have a large impact.
The key to addressing this issue is to observe that users naturally
...
Our synthesis algorithm can exploit the structure inherent in these named
...
algorithm.  \TypeProp{} guides the transformation of regular expression types
during search by deducing when certain expansions must be taken, or when one of
a class of expansions must be taken.

We will now review the formal definitions of regular
expressions and lenses before introducing DNF regular expressions and DNF
lenses.
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \EndFunction
  \end{algorithmic}
\bcp{I don't like the naming much---DR0 is an ugly name}
\end{algorithm}
...
Figure~\ref{fig:2-phased-schematic-diagram}), using the
