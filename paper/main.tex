...
%
A better approach is to use a domain-specific language (there are many) in
which both directions can be written as a single expression, but this
requires learning a new notation.

We present an algorithm for {\em type-directed synthesis} of bidirectional
transformations in a bijective subset of Boomerang, a bidirectional string
transformation language based on the theory of {\em lenses}.  The inputs to
our algorithm are a pair of ordinary regular expressions representing the
schemas we want to convert between, plus a few concrete examples to guide
synthesis.  The output is a well-typed Boomerang program that behaves
as specified.
%
\bcp{(Briefly!) The main challenge is... We address it by...}
% 
We evaluate our algorithm on a benchmark suite of 23 examples comprising
