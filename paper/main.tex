...
\begin{abstract}
  ...
  by evaluating it on a benchmark suite containing 27 examples
...
\begin{figure}
  ...
  \end{tabular}
  \caption{Regular Expression Equivalences\bcp{Suggest dropping this figure
...
Augeas uses combinators similar to Boomerang, but enables transformations of
the data into a structured format, instead of merely a string format.  We
mimicked these in our lens synthesis problems by making the output formats
look like the a serialized version of the structure.  Furthermore, Augeas
supports a more general class of {\em asymmetric
  lenses}~\cite{Focal2005-long}, whereas our synthesis algorithm works only
with bijective lenses.  By including extra fields in the tree format on the
right, we were able to turn non-bijective lenses into bijective ones for
...
\bcp{This table is both pretty big and pretty impenetrable.  Can we find a
  more perpicuous way to display this information?}%
...
\begin{lemma}[$\ParallelRewrite$ Maintained Under $\ConcatDNF$ up to
  ...
  unambiguous.
  If $\DNFRegex \ParallelRewrite \DNFRegex'$ and $\DNFRegexAlt
...
\begin{lemma}[$\StarOf{\ParallelRewrite}$ Maintained Under $\ConcatDNF$]
  \label{lem:star-parallel-rewrite-concatenation}
...
\begin{lemma}[$\StarOf{\ParallelRewrite}$ Maintained Under $\OrDNF$]
  \label{lem:star-parallel-rewrite-or}
  Let $\DNFRegex_1 \StarOf{\ParallelRewrite} \DNFRegexAlt_1$ and $\DNFRegex_2
  \StarOf{\ParallelRewrite} \DNFRegexAlt_2$ then
...
\begin{proof}
  From Theorem~\ref{thm:parallel-star-equivalence} applied to
  Lemma~\ref{lem:star-parallel-rewrite-concatenation}.
\end{proof}
...
\begin{proof}
  From Theorem~\ref{thm:parallel-star-equivalence} applied to
  Lemma~\ref{lem:star-parallel-rewrite-or}.
\end{proof}
