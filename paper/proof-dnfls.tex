\bcp{Can we please put everything into one file?  It took me a long time to
  find this just now.}
...
\ToRegex(\AtomAlt_i)$ such that \bcp{can't parse this}
...
\bcp{What happened to $(s_0,t_0)$ and $(s_n,t_n)$?}
\begin{proof}
...
$\Language_n\in\StarOf{\Alphabet}$,\bcp{Always separate adjacent math
  expressions by more than just punctuation.  Also, why is the $\Sep$ needed
at the beginning and end?  Also, the next expression is hard to understand:
are there concatenations between each pair of elements, or just between $i$
and $i+1$ with ordinary concatenation elsewhere?}  $\Sep\Language_1\Sep\ldots\Sep\Language_i
\UnambigConcat
\Sep\Language_{i+1}\ldots\Sep\Language_n\Sep$.
\end{theorem}
\begin{proof}
We know\bcp{why?} that all of the string before the $i+1$st instance of $\Sep$ is
...
Atom.\bcp{Huh?  Should be by (mutual) induction on DNF lens typing
  derivations, no?  Also, the explanation (here, and everywhere) needs a lot
  of polishing.}\\ 
\\
...
\begin{proof}\bcp{Same rules of grammar apply
  to mathematical writing as ordinary english: subject/verb agreement (there
exist X and Y vs. there exists X...), capitalization, punctuation, etc.}
