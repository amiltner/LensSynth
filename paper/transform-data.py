...
def generate_examples_required_data(input_csv):
    zero_count = "0"
    one_to_five_count = "1-5"
    six_to_ten_count = "6-10"
    eleven_to_fifteen_count = "11-15"
    sixteen_to_twenty_count = "16-20"
    over_twenty_count = "$>20$"
    experimental_values = {zero_count:0,
                           one_to_five_count:0,
                           six_to_ten_count:0,
                           eleven_to_fifteen_count:0,
                           sixteen_to_twenty_count:0,
                           over_twenty_count:0}
    determinizing_values = {zero_count:0,
                            one_to_five_count:0,
                            six_to_ten_count:0,
                            eleven_to_fifteen_count:0,
                            sixteen_to_twenty_count:0,
    ...
    def add_to_correct_group(count_values, n):
        ...
        if n == 0.0:
            count_values[zero_count] = count_values[zero_count]+1
        elif n <= 5.0:
            count_values[one_to_five_count] = count_values[one_to_five_count]+1
        elif n <= 10.0:
            count_values[six_to_ten_count] = count_values[six_to_ten_count]+1
        elif n <= 15.0:
            count_values[eleven_to_fifteen_count] = count_values[eleven_to_fifteen_count]+1
        elif n <= 20.0:
            count_values[sixteen_to_twenty_count] = count_values[sixteen_to_twenty_count]+1
        else:
            count_values[over_twenty_count] = count_values[over_twenty_count]+1
    ...
    for example_num in determinizing_vals:
        add_to_correct_group(determinizing_values, int(example_num))
    experimental_string = "\n".join([str(key) + "," + str(val) for (key,val) in experimental_values.items()])
    determinizing_string = "\n".join([str(key) + "," + str(val) for (key,val) in determinizing_values.items()])
    write_to_filename("transformed-data/experimental-example-counts.txt", experimental_string)
    write_to_filename("transformed-data/determinizing-example-counts.txt", determinizing_string)

def main(args):
    if len(args) == 2:
        input_filepath = args[1]
        input_csv = retrieve_csv(input_filepath)
        ensure_dir("transformed-data/")
