...
theories, like assymmetric lenses~\cite{Focal2005-long} and quotient
lenses~\cite{QuotientLenses08}, we focus on synthesizing lenses encoding bijections.
We theorize many of these more complex lenses have a bijective core,
surrounded by
projections (for asymmetric lenses) and canonizers (for quotient lenses).
If the appropriate projections and canonizers are provided
(either from the user or from another synthesis algorithm),
\Optician{} can be used as a drop-in component for synthesizing full terms in these
...
presented first (like \StringCF{"Kleene, Stephen Cole"}) and names expressed with
the first name presented first (like \StringCF{"Stephen Cole Kleene"})
is:
...
required reasoning about which components must be swapped to make the last name
appear at the end, and which components must be swapped to properly place
whitespace between names.  These difficulties become even more apparent when
writing the complex transformations that occur between large formats, and
when ensuring the lenses are well-typed. 
...
\else
available in the extended version of this paper~\cite{extended-version}.
\fi
...
would be (will make sure this appears on one page in final):
%
\begin{lstlisting}
...
symbolic finite transducers. However, this tool
cannot work on many of the programs we are interested in: \SwapLens{} cannot
be expressed in full generality using these transducers.  Furthermore, this tool
still requires user to input a program, instead of merely specifications.
