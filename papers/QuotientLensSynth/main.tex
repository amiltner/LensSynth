...
\newcommand{\project}[2]{\ensuremath{\mathit{project} \; #1 \mapsto #2}}
\newcommand{\squash}[3]{\ensuremath{\mathit{squash} \; #1 \rightarrow #2
\mathit{using} \; #3}}
...
equivalence classes can be challenging due to the rather fiddly, mainly due to
``non-ambiguity'' conditions in the lens well-formedness rules.

This paper addresses both of these issues.  First, in Section~\ref{example} we
...
considered the same by the lens,
\begin{verbatim}
    @Book {conway,
      Author = "J. H. Conway",
      Title = {Regular Algebra and Finite Machines},
    ...
    @Book{     conway,
        Title = {Regular Algebra 
           ...
           Author = "Conway, J. H.",     }
\end{verbatim}
with the EndNote records having similar rules defined on them.
...
equivalence is composed of three separate equivalences which nest and interact
...
To address this issue, we propose {\em Quotient Regular Expressions} or QREs,
...
The expression \cd{project [ \\n\\t\\r]}$^+$ $\to$ \cd{" "} is a QRE that
matches any number of whitespace characters, and then considers all strings of
whitespace characters as equivalent with a single space as the canonical
representative. Another QRE in the example above is the expression

\begin{center}
\begin{lstlisting}
squash comma_name -> space_name using (get space_to_comma)
\end{lstlisting}
\end{center}

\noindent which considers the \cd{comma_name} representation of the author's
name to be equivalent to the \cd{space_name} representation, with \cd{get
space_to_comma} giving the function that transforms names in the \cd{comma_name}
representation to names in the \cd{space_name} representation.

The third example of a QRE in the example is the expression
\begin{center}
\begin{lstlisting}
...
Now QREs make it easier to write quotient lenses by specifying the data and
the equivalence relation on the data in one expression, but the programmer still
...
itself a tedious and error-prone task, as the code snippet below illustrates:
...
For example, suppose \cd{q_1 : bibtex <=> endnote} is a QRE lens that maps
between \bibtex{} and EndNote formats, and that \cd{q_2 : endnote <=> xml}
is a QRE lens that maps between EndNote and XML formats. The composition 
\cd{(q_1 ; q2) : bibtex <=> xml} of \cd{q_1} and \cd{q_2} is not in normal form
due to the presence of canonizers on each side of the intermediary EndNote
format:

\sam{TODO: Put diagram of composition here}

\noindent However, while \cd{q_1 ; q_2} is not in normal form, there is another
QRE lens \cd{q : bibtex <=> xml} of the same type as \cd{q_1 ; q_2} such that
\cd{q_1 ; q_2} and \cd{q} define the same transformation. This proof will allow
justify our approach for synthesizing quotient lenses so that the programmer
can simply type
...
\end{lstlisting}
\noindent to synthesize a quotient lens that maps between the \cd{bibtex} and
...
&q \semicolon q' \sep q \cdot q' \sep (q \sep q') \sep q^*,
\end{align*}
...
The $q \semicolon q'$ QRE is a compostion combinator used to successively apply
...
The final combinator is the  $\normalize{R}{R'}{f}$ combinator, which expresses
the equivalence relation which sends each element in $\mathcal{L}(R)$ to its
...
ambiguity} which comes up when defining QREs. If $q, q'$ are QREs, then
...
\BinaryInfC{$squash(R, R', \ell)$ is well formed}
\end{prooftree}
...
\end{theorem}
 ...
 \begin{figure}[t]
  ...
  \begin{prooftree}
\AxiomC{$\ell : R \Leftrightarrow S$}
\UnaryInfC{$\mathit{lift}(\ell): id(R) \Leftrightarrow id(S)$}
\end{prooftree}
  \begin{align*}
...
\AxiomC{$\substack{\mathcal{L}(W(c'))= \mathcal{L}(W(c'')) \\ K(c') \equiv^d
K(c'')}$}
\AxiomC{$\mathit{Canonizer}(c') = \mathit{Canonizer}(c'')$}
\QuaternaryInfC{$q_2 \circ q_1: c \Leftrightarrow c'''$}
\end{prooftree}
  \begin{align*}
