...
However, quotient lenses are sometimes difficult to program because the
equivalence relations defined on the data, as well as the underlying lens which
operates on the equivalence classes are complicated. These two challenges make
quotient languages impractical to use in many cases spite of their advantages.
...
programmer to explicitly define a broad class of equivalence relations on
regular languages using a compact and convenient notation. We then define a
class of {\em QRE lenses}, a class of quotient lenses which are typed using
QREs, and show how to {\em synthesize} QRE lenses from their QRE types and a
set of input-output examples. Finally, we demonstrate the applicability of this
...
in the EndNote format, and vice versa.
...
that only change the order of fields, or add whitespace characters that in reality are not
part of the data. Foster et al therefore proposed \textit{quotient lenses}
~\cite{quotient} as a solution to this problem. Quotient lenses enable a
programmer to write lenses which transform data that is quotiented out by an
equivalence relation, so that records which are equivalent to each other modulo
this relation are considered the same by the underlying lens.

Unfortunately, quotient lenses are sometimes difficult to program because the
equivalence relations defined on the data, as well as the underlying lens which
operates on the equivalence classes are complicated. For example, suppose that
the programmer wants to program the \bibtex to EndNote transformation in such a
way that the program allows for any of two representations of the authors
name (e.g. ``Conway, J. H." and ``Conway J. H"), ignores the order in which the
author and title fields occur in each record, and allows for arbitrary amounts
of whitespace in between the author and title fields. These rules allow for
these two \bibtex citation records to be considered the same:
...
The Boomerang quotient lens which does the \bibtex to EndNote conversion is

\sam{TODO: PUT BOOMERANG EXPRESSION HERE!},

an expression which is difficult to write and understand.

We address the challenge of programming quotient lenses in two ways. First, we
propose the language of {\em Quotient Regular Expressions} or QREs, which
enables the programmer to explicitly define a broad class of equivalence
relations on regular languages using a compact and convenient notation. For
example, the QRE which describes the equivalence relation on the \bibtex
citation records is as follows:

\sam{TODO: PUT QRE FOR BIBTEX HERE!}

This expression better describes the equivalence relation since it is explicit
and more precise.

However, even though QREs enable the programmer to describe complicated
equivalence relations on data more simply, the underlying lens which maps the
equivalence classes of the \bibtex records to the equivalence classes of the
EndNote records is also challenging to program, mostly because of the verbosity
of the expression:

\sam{TODO: PUT QRE LENS FOR BIBTEX HERE!}
...
source QRE to compute a unique representative for a datum modulo the
...
$$\mathit{synth} \; \mathtt{BIBTEX} \; \mathtt{ENDNOTE}$$

to derive a quotient lens that does the \bibtex to EndNote.
...
squash(R, R', l) or maybe even just have only squash-synth(R, R')? Perhaps
having to explicitly write the lens weakens our case that QREs are compact and
convenient}
