...
\begin{abstract} 
  ...
  However, quotient lenses are often difficult to program.  The programmer
  often has to give a specification for the data on each side of the lens and a
  separate specification for the equivalence relations defined on the data. The
...
\bcp{I thought we were going to allow newlines within titles and authors...?}

\sam{I wanted to illustrate how to use the squash QRE which is why I changed my
mind on that, but on second thought since the problem of quotienting away
whitespace at will is going to be more familiar to the reader I changed it back
to the original plan. I will explain squash in the QRE section.}
\begin{verbatim}
@Book {conway,
  Author = "J. H. Conway",
  Title = {Regular Algebra and Finite Machines},
...
relations on the \bibtex{} records in Boomerang using canonizers:
...
With QREs we can specify \bibtex{} records as follows:
...
\begin{figure}[t]
  ...
  screenshot from Nate's thesis. Are we allowed to use it?}}
