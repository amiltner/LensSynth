...
addresses those difficulties.  Quotient regular expressions allow programmers to
specify a broad class of equivalence relations over regular languages using a
compact and convenient notation in which the regular language and its
equivalence classes are captured by a single expression.  We then define {\em
  QRE lenses}, a class of bijective quotient lenses that are typed using QREs,
and we show how to {\em synthesize} QRE lenses from their QRE types and a set of
input-output examples.  We demonstrate the practicality of this approach by
synthesizing QRE lenses between several real-world data formats from the {\tt
  data.gov} database.
...
\section{Lenses and Quotient Regular Expressions by Example}
...
\begin{figure}[t]
  ...
  \caption{Quotient Lens with ``Canonizers at the Edges''\sam{This is a
      ...
      language so we just use the inclusion as the choose function. Also,
      perhaps it would be clearer if we said source structures and target
      structres instead of all structures?}}
