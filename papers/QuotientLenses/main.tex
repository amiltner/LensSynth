...
\kw{get} functions must fit together. \bcp{See my email about deemphasizing
  ``storing'' multiple formats.}
...
language {\em Boomerang}~\cite{boomerang}\bcp{All of them?  I'm not certain
  that's accurate.}. Each lens $\ell : S \Leftrightarrow T$
...
names:\bcp{Solomon: Someone noted on their copy of the intro from
  ...
  marked-up copies that I gave you?}
%
\begin{lstlisting}
...
\end{verbatim}
\noindent
and equivalent EndNote records like the following\bcp{bad punctuation: needs
a colon}
\begin{verbatim}
   %0 Book
   %F Lovelace
...
following grammar:\bcp{bad punctuation}
\newcommand{\bsep}{\ \ \sep{} \ \ }
\begin{align*}
...
\end{cases}\\[3ex]
\canonize(Q^*)(\epsilon) &=& \epsilon \\
\canonize(Q^*)(w_1 \cdot w_2) &=& \canonize(Q)(w_1)\cdot\canonize(Q^*)(w_2) \\[2ex]
...
\multicolumn{3}{l}{\ =\ \ \canonize(Q_1)(r_1)(w_1) \cdot \canonize(Q)(s_1) \cdot \ldots \cdot
...
combinators.\bcp{bad punctuation}
...
goes wrong if this premise is dropped.\bcp{Didn't we used to have a discussion
  about how implementing this rule is hard because it involves checking language
  equivalence so we follow Boomerang's rule? I think this is important to
  mention because this was a significant restriction on the power of composition
  in Boomerang. It doesn't apply here (right?) because we intend to synthesize
  these lenses, not write them by hand, so we will never actually need to
  typecheck a functional composition expression.}\afm{Boomerang does check
  language equivalence, we restrict the check to star-semiring equivalence
  because we aren't complete when we allow arbitrary equivalence.}) The
condition $\mathcal{L}(W(Q_2)) = \mathcal{L}(W(Q_3))$ says that the intermediary
language is the same on both sides, while the condition $K(Q_2) \equiv^s K(Q_3)$
says that the kernel regular expressions are equivalent modulo the star-semiring
axioms. Finally, the condition $\canonize(Q_2) = \canonize(Q_3)$ says that $Q_2$
and $Q_3$ define the same equivalence relation on $\mathcal{L}(W(Q_2))=
\mathcal{L}(W(Q_3))$.
...
By the induction hypothesis, there exist bijective lenses\bcp{every display
  for the rest of the section needs punctuation help}
...
the example pair \cd{(bib_example , end_example)} where\bcp{punctuation}
...
\longrightarrow C$ such that for every $b \in B$,\bcp{punctuation}
...
result in intermediate types that are ambiguous\BCP{???}\sam{Before I had this
as unambiguous. Was this the issue? Or does this need more explaining even if
the word unambiguous was the issue}, and this ambiguity can cause unintended
updates to be made the source, especially when information is discarded in this
process.
