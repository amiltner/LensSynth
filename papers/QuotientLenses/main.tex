...
Amongst the most restrictive lenses are the \emph{bijective lenses}.
A bijective lens $\ell : S \Leftrightarrow T$ is simply a bijection from
a source set $S$ to a target set $T$:
\begin{equation}\label{bijectivelenslaws}
\ell.\get \; (\ell.\lput \; t) = t \text{, and } \ell.\lput \; (\ell.\get \; s)
= s
\end{equation}
Since these lenses demand that get and put functions form a bijection, then
no information is lost when one translates from one data format to another. 
While this strong guarantee can be useful in some contexts, it is more often
the case that some of the information in one or both of the data formats is
superfluous. For instance, one typically does not care to preserve the amount of
...
{\em Quotient lenses}~\cite{quotientlenses} loosen many of the restrictions of
bijective string lenses by allowing a programmer to define an equivalence
relation on the source and target formats so that the round-tripping lens laws
hold modulo these equivalence rather than modulo equality. Quotient lenses thus
make it possible to align many more data sources than would otherwise be
possible, by ignoring details that a programmer views as inessential. The only
language we are aware of that implements a general class of quotient lenses is
{\em Boomerang}~\cite{boomerang}. While Boomerang enables a programmer to
define a very general class of quotient lenses, these lenses to have two main
drawbacks.
...
\begin{enumerate}
  ...
  \item We introduce {\em QRE lenses} which are lenses that map data specified
  using QREs.
  In addition, we will descibe a particular set of QRE lens combinators and
  prove that QRE lenses defined using these combinators can be transformed to
  a QRE lens that us defined using just two of these combinators; this is the
  ...
  lenses to the problem of synthesizing bijective string lenses which we studied
  in~\cite{optician}(Section~\ref{synth}).
  \item We extend the Boomerang {\em implementation} with QREs
  and QRE lens synthesis.  We demonstrate the utility of our implementation by
  using it to synthesize QRE lenses between data formats drawn from the
...
Boomerang's bijective string lenses are designed to define a bidirectional
transformation such as this, where the data formats can be matched in a
one-to-one manner, in this case by matching the label, author and title fields.
Boomerang allows programmers to define such bijecive transformations using
combinators which favour a compositional approach in which the programmer
defines small lenses which are then put together using the combinators to
define larger and larger lenses until the final lens is defined.

Each bijective string lens is constructed from simple primitive combinators including:
\begin{itemize}
...
Unfortunately, programming bijective string lenses in Boomerang is often slow and
...
shortcomings of bijective string lenses by developing a tool called {\em Optician}
which enables a programmer to synthesize a bijective string lens from a pair of regular
...
formats and the behavior of the lens on a few inputs in order to define a
bijective string lens. As a result defining bijective string lenses becomes faster and less
error prone, since the programmer need only invoke the synthesis procedure
\lstinline{synth S <=> T using exs} in order to define a bijective string lens rather
...
Now let $Q_1$ and $Q_2$ be QREs. Given a bijection $\ell : K(Q_1)
\Leftrightarrow K(Q_2)$, we may define a quotient lens $q : W(Q_1)/\eqrel{Q_1}
\Leftrightarrow W(Q_2)/\eqrel{Q_2}$ by $q.\get = \ell \circ \canonizer(Q_1)
$ and $q.\lput = \ell_1^{-1} \circ \canonizer(Q_2)$. In other words, the get
function of $q$ first canonizes the source data using $\canonizer(Q_1)$
and then applies $f$ to the result, while the put function of $q$ first
canonizes the target data using $\canonizer(Q_2)$ and the applies $f^{-1}$ to
the result. This is our approach in defining QRE lenses.

One potential pitfall with QRE lenses is that a lens derived using the QRE lens
combinators may not have the form of a bijection with ``canonizers at the
edges''. Such a situation would be undesirable since our strategy for
synthesizing QRE lenses is to synthesize a bijective string lens which plugs
into the two QREs provided as input to the synthesis procedure; if there were
some QREs which could not be transformed to the desired form, then our synthesis
algorithm would necessarily be incomplete.

Thankfully, this turns out not to be an issue if we use the bijections that
can be expressed in Boomerang; following ~\cite{optician} we call these
lenses {\em bijective string lenses}. Once we restrict the bijections that we
use to derive QRE lenses to include just bijective string lenses, we will be
able show that every QRE lens can be transformed to a QRE lens which has the
form of a bijective string lens with canonizers at the edges.

\subsection{Bijective String Lenses}
The syntax for the language of bijective string lenses is given by $$\ell_1 :=
\mathit{id} \; (R) \sep \const(s_1, s_2) \sep  \swap(\ell_1, \ell_2) \sep
\ell_1 \cdot \ell_2 \; |  \; (\ell_1 \sep \ell_2) \sep \ell^* \; | \; \ell_1 \;
; \;  \ell_2,$$ where $R$ ranges over regular expressions and $s$ ranges over
character strings.
...
direction, while $\mathit{id}(R)$ applies the identity function to the
...
is $\mathcal{L}(S)$. Interestingly, with the bijective string lens type system,
a lens $\ell : R \Leftrightarrow S$ can also be considered to be of type $\ell
: R' \Leftrightarrow S'$ provided that $R$ (resp. $S$) can be proven to be
equivalent to $R'$ (resp. $S$) from the star-semiring axioms (associativity and
commutativity of | and $\cdot$, identity of the empty string $\epsilon$,
distibutivity of $\cdot$ over |, annihilative property of $\varnothing$ with
respect to $\cdot$, and that $R^* = \epsilon \; | \; R \cdot R* = \epsilon \; |
\; R^* \cdot R$ for all $R$):
...
The other typing rules for bijective string lenses are given in
...
\Leftrightarrow Q_1$ where $c, Q_1$ are QREs. In contrast, the approach used to
...
$q_1 : Q_1 \Leftrightarrow Q_2$. Since bijective lenses are closed under strongly unambiguous
