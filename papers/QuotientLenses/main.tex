...
QRE synthesis.  She must write QRE specifications of the source and
...
of Optician.  She must write canonizers by hand, along with the
regular expressions describing the external
representations of the source and target formats. (The internal
regular expressions can be inferred from the canonizers.) She may also
...
programmer writes the lens between the source and target lenses
entirely by hand.

To measure the programmer effort using each of these approaches, we
calculate three different quantities for each problem in the benchmark suite:
%
\begin{itemize}
  \item[\QRESize{}:] 
  ...
  formats, (2) the number of AST nodes $\canonize(q)$ for each such QRE $q$ with a
  non-trivial canonizer, and (3) the number of AST nodes in the
  examples.  We use (1) and (2) to estimate the burden of describing
  the external source and target formats and of writing the requisite canonizers
  ...
  estimate the burdern for describing the source and target formats
  and (2) to estimate the burdern for writing the appropriate lens by
