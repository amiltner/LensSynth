...
hold modulo these equivalences rather than modulo equality. Quotient lenses thus
...
\afm{Right now comma\_name is just a regular expression.  Further details in Fig
2}
...
\bcp{And a readability suggestion:
  define a named constant for a quote character.}\fi}
...
to do something to make the structure clearer.}
...
Another QRE primitive is the sequential composition combinator,
written ``;''.
For an example of its use, suppose we have already defined a QRE
\codefont{comma_to_space} that accepts names in either space-separated
or name-separated format and chooses the space-separated format as canonical.
Suppose we have also already defined a
QRE \codefont{project_in_space}, which projects away extra white space
between the first and last name in the space-separated name format. 
We can use the sequential composition combinator to combine these two QREs:
\begin{lstlisting}
let name = comma_to_space ; space_name_p
\end{lstlisting}
...
project QRE \kw{p\_wsp} defined above can
...
let comma_to_space = squash comma_name $\to$ space_name using c_to_s
...
let space_name_p = first_name . p_wsp . last_name
...
let bib_names = name . (name_sep . name)$^*$)
let bib_author = "author = \"" . bib_names . "\""
let title = word . (p_wsp . word)$^*$
...
spec vs a QRE lens spec?}\afm{I don't really like the way this is set up.  The
use of the sequential composition operator makes the QREs appear really
operational, and in my opinion makes them seem less cool, and distinct from
canonizers, than they actually are.  I include a proposed alternative in the
next figure.  However, this would mean that the previous section which describes
the sequential composition operator would require it be talked about without the
concrete example.}}
...
\begin{lstlisting}
let wsp = [ \n\t\r]+
let wsp_s = project wsp $\to$ " "

(* define name representations with a space and with a comma *)
let space_name = first_name . wsp_s . last_name
let comma_name = last_name . "," . wsp_s . first_name
...
(* define rest of bibtex fields *)
let bib_names = name . (wsp_s . "and" . wsp_s . name)$^*$
let bib_author = "author = \"" . bib_names . "\""
let title = word . (wsp_s . word)$^*$
let bib_title = "title = {" . title . "}"
...
\label{fig:example-qre-anders-proposal}
\end{figure}
...
\ksf{What is the notation ``lens in'' below?  We should explain it
intuitively here.}
\begin{lstlisting}
...
\BCP{What became of my suggestion that we start with just ``normalize'' and
  then, later, introduce the other, more specific (and pragmatic, and
  incomplete) ones?}
...
\begin{enumerate}
  ...
  than ``canonize''??}\sam{because \canonizer(Q) is a function}\bcp{My ear
  consistently finds it quite awkward.  I'd be happier with
  ``canonize'' (which describes an action).}
...
\item The remaining eight benchmarks were derived from datasets found at 
  ...
  plain text or PDF documents.  \BCP{Besides differing formats, why did we
    ...
    flashfill and augues examples.}\afm{So would it be appropriate to say it was
    these were merely the first examples in the data.gov website whose languages
    are regular?}\BCP{Or would anything be lost by just deleting them?  It
    seems like we're tying ourselves in knots at bit trying to explain how
    they relate to the others and what is interesting about them, when the
    answer seems to be they don't and nothing.}
\end{enumerate}
...
Rajkumar et. al.~\cite{rajkumar2014lenses} observe that formlets are a useful
...
assumes that the synthesizer is provided with a collection of examples. Optician
and Optometrist\bcp{Sole use?!} differ in that they require that the
programmer supplies both 
examples {\em and} format descriptions in the form of regular expressions or
QREs, though we are far from the only ones to consider type-based synthesis.
