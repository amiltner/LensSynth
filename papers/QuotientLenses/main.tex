...
\newcommand{\wf}[1]{\vdash \ensuremath{#1\;\mathsf{wf}}}
...
equivalence classes can be challenging due to the rather fiddly typechecking,
mainly due to ``non-ambiguity'' conditions in the lens well-formedness rules.
...
enable a programmer to simultaneously specify a regular expression as well as an
equivalence relation on that regular expression. QREs make programming quotient
lenses easire by saving the programmer the effort of defining the data and the
equivalence relation on the data separately.

Secondly, we use QREs to define a class of quotient lenses called {\em QRE
lenses} which map betweeen QREs, and show how to synthesize QRE lenses given
an specification described by pair of QREs and a set of input-output examples.
This way, the programmer need not manually derive the lens herself. The net
effect of these two contributions, that is QREs and QRE lens synthesis, is that
in order to derive a quotient lens, the programmer will only need to specify the
data formats on each side of the desired quotient lens and possibly a set of
example input-output pairs that guide synthesis of the lens, a marked
improvement over the current situation.

In summary, our main contributions are:
\begin{enumerate}
\item We introduce a novel language of {\em Quotient Regular Expressions}
(QREs) that a provide compact, convenient notation for a natural and useful
...
and show, by way of example, how our novel language of {\em Quotient Regular
Expressions}, or QREs and QRE lenses address these difficulties.
...
equivalence is composed of three separate equivalences---whitespace quotienting
inside fields, whitespace quotenting in between fields, and permutation
quotenting of fields---nest with each other in a way that makes them difficult
to express with the standard Boomerang combinators. 

In general, there is also another issue in describing equivalences in
Boomerang, which is that that the programmer often has to first specify the data
itself in the form of a regular expression before she can define the
equivalence relation on that data. This leads to verbose code.

To address these issues, we propose {\em Quotient Regular Expressions} or QREs
which enable the programmer to define a regular expression $R$ as well as an
equivalence relation on $R$ in the same expression. For example, with QREs we
...
(* map arbitrary occurrences of whitespace to a single space *)
...
In Figure~\ref{fig:example-qre}, the QRE \cd{project wsp}+ $\to$ \cd{" "}
matches any non-zero number of whitespace characters, but considers the matched
string of whitespace characters as equivalent with a single space as the
canonical representative. We call this QRE a \textit{projection} because it
projects an entire regular language down to a single representative string in
the language.
...
specify the squash transformation. (We'll say more about synthesis below.)

The third example of a QRE in the Figure is \cd{bib_perm}, defined by:
\begin{center}
\begin{lstlisting}
...
separated by a comma followed a newline. The \textit{permutation} QREs is
...
synthesizing {\em bijective lenses}, which are the bijections created from the
Boomerang lens combinators. This is a problem which we solved in our prior work
~\cite{popl18}.
...
\saz{Fix the notation in the following code.}
...
representatives for this equivalence relation. \saz{rewrite the next sentence to
  avoid so many uses of ``express''?} The $\normalize{R}{R'}{f}$
...
\saz{Add something about being ``complete'' here?  Say more about how normalize
  wouldn't be used in ``ordinary'' QRE programming?}
...
express many equivalences which\saz{``which'' $\to$ ``that''} occur in real world data as we shall demonstrate
...
ambiguity}, which comes up\saz{``comes up'' is a little colloquial; perhaps ``arises''?} when defining QREs. If $c$ and  $c'$ are QREs, then
...
\saz{Add citations to prior work that uses these definitions?}
...
\begin{figure}[p!]
\saz{This well-formedness judgment is better, but the \textbf{WF} is a bit
  jarring -- it detracts from the content of the judgment.  Maybe something
  lighter, like just $\mathit{wf}$?}
\begin{center}
\AxiomC{$R$ is strongly unambiguous}
\UnaryInfC{$\mathit{id}(R)$ \textbf{WF}}
\DisplayProof
\hskip 1.5em
\AxiomC{$s \in \mathcal{L}(R)$}
\UnaryInfC{$\project{R}{s}$ \textbf{WF}}
\DisplayProof
\hskip 1.5em 
\end{center}

\begin{prooftree}
\AxiomC{$c_i, c$ are well formed}
...
\QuaternaryInfC{$\squash{c}{c'}{f}$ \textbf{WF}}
\DisplayProof
\end{center}
\begin{center}
...
\AxiomC{$c'$ \textbf{WF}}
\AxiomC{$K(c) = W(c')$}
\TrinaryInfC{$c' \; ; \; c$ \textbf{WF}}
\DisplayProof
\hskip 1.5em
\AxiomC{$c$ \textbf{WF}}
\AxiomC{$W(c)^{*!}$}
\AxiomC{$K(c)^{*!}$}
\TrinaryInfC{$c^*$ \textbf{WF}}
\DisplayProof
\end{center}
\begin{center}
...
\AxiomC{$W(c) \cdot^! W(c')$}
\AxiomC{$K(c) \cdot^! K(c')$}
\QuaternaryInfC{$c \cdot c'$ \textbf{WF}}
\DisplayProof
\end{center}
...
combinator: \saz{fix the overflow by stacking ?  Also, it should use the WF
  predicate in the premise, right?}
...
As we mentioned before, each QRE $c$ enables us to express: \saz{I'm not sure we
  need to repeat the whole list here.}
\begin{enumerate}
  \item a regular expression $W(c)$ (the ``whole'' of $c$),
  \item an equivalence relation $\eqrel{c}$ on $\mathcal{L}(W(c))$,
  \item a regular expression $K(c)$ (the ``kernel'' of $c$)
  ...
  $k$ is equivalent to $w$ mod $\eqrel{c}$
  \end{enumerate}
Figures~\ref{fig:wk} gives the inductive definitions for the whole language
$W(c)$ of a QRE. The \textit{squash} and \textit{permutation} combinators
give the two interesting definitions. If $c = \squash{c}{c'}{f}$, then
the whole language of $c$ is $W(c) \sep W(c')$. This is because the
\textit{squash} combinator takes the whole language $W(c)$ of $c$, the whole
language $W(c')$ of $c'$ and a function $f : \mathcal{L}(W(c)) \longrightarrow
\mathcal{L}(W(c'))$, maps $\mathcal{L}(W(c))$ into $\mathcal{L}(W(c'))$ using
$f$ and then canonizes $W(c')$ into $K(c')$ using $\canonizer(c')$.
...
&
  \begin{array}[b]{rcl}
  \llbracket q \rrbracket.get &=&  \llbracket \ell \rrbracket\\
...
%   \begin{prooftree}
% \AxiomC{$\ell : R \Leftrightarrow S$}
% \UnaryInfC{$\kw{lift}(\ell): id(R) \Leftrightarrow id(S)$}
% \end{prooftree}
% \end{minipage}
% &
% \(
...
\AxiomC{$c$ \textbf{WF}}
\AxiomC{$K(c) = W(c')$}
\TrinaryInfC{$\kw{lquot}(c, q'): c' \circ c \Leftrightarrow c''$}
\end{prooftree}
  \begin{align*}
...
\AxiomC{$K(c'') = W(c')$}
\TrinaryInfC{$\kw{rquot}(q', c):c \Leftrightarrow c'' \circ c'$}
\end{prooftree}
  \begin{align*}
  \llbracket q \rrbracket.get &= \llbracket q'
  \rrbracket.get\\
...
\end{prooftree}
  ...
  \end{align*}
    ...
    \begin{prooftree}
\AxiomC{$q : c \Leftrightarrow c$}
\AxiomC{$\substack{W(c)^{*!} \text{ and } W(c')^{*!} \\ K(c)^{*!} \text{ and }
K(c')^{*!}}$}
\BinaryInfC{${q}^* : c^* \Leftrightarrow {c'}^*$}
\end{prooftree}
  \begin{align*}
  ...
  \end{align*}
    ...
    \begin{prooftree}
\AxiomC{$\substack{q_1 : c_1 \Leftrightarrow d_1 \\ q_2 : c_2 \Leftrightarrow
d_2}$}
\AxiomC{${\substack{W(c_1) \cdot^! W(c_2)\\ K(c_1) \cdot^! K(c_2)}}$}
\AxiomC{${\substack{W(d_1) \cdot^! W(d_2)\\ K(d_1) \cdot^! K(d_2)}}$}
\TrinaryInfC{$q_1 \cdot q_2: c_1 \cdot c_2
...
\AxiomC{$\substack{q_1 : c_1 \Leftrightarrow d_1 \\ q_2 : c_2 \Leftrightarrow
d_2}$}
\AxiomC{$\substack{\mathcal{L}(W(c_1)) \cap \mathcal{L}(W(c_2)) = \varnothing
...
In our previous work \saz{citation}, we showed that given regular expressions $R, S$ and a set
...
transformations~\cite{singh2012learning,le-pldi-2014,gulwani-popl-2014,perelman2014test,Singh:blinkfill}, our system Optician, which we introduced in
...
\saz{Maybe say again here how \Name{} improves on Optician?   Is it the case
  that if you use the ``regular language'' subset of QRE's that our new algorithm
  degenerates to the Optician one?  Is it worth trying that out experimentally?}
...
regular expressions.  \saz{Maybe elaborate more on the tradeoffs here?}


There are many other recent results showing how to
...
equivalences on types.  \saz{This kind of comparison is OK, but a bit
  superficial.  Is there a crisper, technical description of the differences?}
