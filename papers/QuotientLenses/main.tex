...
\usepackage{algorithm, amsmath, amssymb, verbatim, enumerate, graphicx, centernot, tikz, array, mathtools, bussproofs, stmaryrd, enumitem, stackengine, subcaption,flushend,relsize, listings, proof, xspace, booktabs}
...
% idea is simple and natural, but provokes the following question: do we give up
...
\end{cases}
\\[2ex]
\multicolumn{3}{l}{\canonize(\perm{Q_1, \ldots, Q_n}{Q})(w_{\sigma(1)} \cdot
s_1 \cdot \ldots \cdot s_{n-1} \cdot w_{\sigma(n)})}\\[.5ex]
\multicolumn{3}{l}{\ =\ \ \canonize(Q_1)(w_1) \cdot \canonize(Q)(s_1)
\cdot \ldots \cdot \canonize(Q)(s_{n-1})\cdot\canonize(Q_n)(w_n)}
\end{array}
\]
\end{center}
...
\[
\begin{array}{l}
\canonize(\perm{Q_1, \ldots, Q_n}{Q})(w_{\sigma(1)} \cdot s_1
  \cdot \ldots \cdot s_{n-1} \cdot w_{\sigma(n)})\\[.5ex]
\ =\ \ \canonize(Q_1)(w_1) \cdot \canonize(Q)(s_1) \cdot \ldots \cdot
\canonize(Q)(s_{n-1})\cdot\canonize(Q_n)(w_n)
\end{array}
\]
\noindent 
...
Instead, in this case, we need to write
\begin{lstlisting}
...
\end{lstlisting}
\end{center}
where \lstinline|nil| and \lstinline|space| pretty-print \lstinline|""| and
\lstinline|" "| respectively, but represent zero-or-more and one-or-more
whitespaces in parsing. FliPpr also allows annotations for optional
parentheses in programs. These features enable a user to easily define a
pretty-printer and a parser that also recognizes ``ugly'' strings; after a
round trip through the parser and printer, such 
strings will be mapped to a canonical representative modulo whitespace and parentheses quotienting.
...
\begin{acks}
  We thank the anonymous reviewers for their useful feedback and discussions and Nate Foster for extensive assistance in integrating
...
\[\begin{array}{l l}
\llbracket q_1 \rrbracket.\get = \llbracket \ell_1 \rrbracket \circ
\canonize(c) & \llbracket q_2 \rrbracket.\get = \llbracket \ell_2 \rrbracket \circ
\canonize(Q_1)\\
\llbracket q_1 \rrbracket.\lput = {\llbracket \ell_1 \rrbracket}^{-1} \circ
\canonize(Q_1) & \llbracket q_2 \rrbracket.\lput = {\llbracket \ell_2 \rrbracket}^{-1} \circ
\canonize(Q_2)
\end{array}
\]
Consequently:
...
\canonize(c)
\end{align*}
A similar argument shows that $\llbracket q_1 \; ; \; q_2 \rrbracket.\lput =
\llbracket \ell_1 \; ; \; \ell_2 \rrbracket^{-1} \circ
\canonize(c)$
\item
...
$$
\llbracket q^* \rrbracket.\get = (\llbracket q \rrbracket.\get)^* \text{ and }
