...
In recent work~\cite{optician}, we developed a tool called {\em Optician} to synthesize the
...
function that converts string in the language of regular expression
to a canonical form.
...
Recent work described the {\em Optician}\bcp{Why is this sometimes
  italicized and sometimes not?  (My preference would be not---we have too
  much italics.)}
...
\caption{QRE lens from source $P$ to target $Q$.  $P$ and $Q$ are QREs, each
  ...
  The {\emph get} function of the whole lens takes an argument from $W(P)$,
  applies the canonizer for $P$ to obtain a canonical representative in
  $K(P)$, then applies the {\em get} of the plain lens, yielding an element
  of $K(Q)$, which is a subset of $W(Q)$.  The {\em put} function does the
  ...
  ordinary bijective lens in the center. \BCP{Both the figure and the
    caption are new---what do you think??}}
...
definition:\bcp{Why is this right-justified?}
...
\end{array}
\]
\caption{QRE Equivalence Relations\bcp{The inter-letter spacing of ``True''
    is wrong, and anybody who has worked with tex for a while will be
    bothered by this rookie mistake.  A general rule in tex is that english
    words in math mode {\em must always} be enclosed in {\tt
      $\backslash$mathit}.}}
\label{fig:relations}
\end{figure}
...
function, and a modified namesource:\bcp{typesetting of arrows is funny}
\begin{center}
\begin{lstlisting}
...
be well behaved by defining a version of the classical \bcp{typeset in
  small caps}GETPUT and PUTGET laws
