...
\bcp{A question about variable names: If we use $R$ and $S$ for plain
  ...
  and q for QRE lenses, but if you disagree I will make the change.}\bcp{I
  agree that this is a problem.  But maybe the right fix is to choose
  different metavariables for QRE lenses.  Or to use capital letters other
  than $P$ and $Q$ for QREs.  I don't like $c_1$ and $c_2$, both for the
  lower-case letter and for the subscripts---it's not at all consistent with
...
straightforward induction on the derivation $q_1 : c_1 \Leftrightarrow c_2$ (the
full details can be found in the appendix). Since bijective lenses are closed
under strongly unambiguous regular operators, the tricky case is composition.
The composition case is more involved since we need to show that we can get rid
of the canonizers in the middle:
