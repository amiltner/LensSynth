...
\newcommand{\QREOptician}{\textbf{OQ}}
...
lenses to have two main drawbacks.  \bcp{This wording may raise a question
  in the reader's mind about whether this paper is really about quotient
  lenses in general or whether it is specifically a proposal to tweak
  Boomerang.  I.e., ``if I am not a Boomerang user, do I want to bother
  reading this paper?''  My answer would be yes, because quotient
  lenses and QREs are interesting in themselves.  But I don't think we've
  made this point very convincingly in the way we've framed the paper.}

The first shortcoming is that defining an equivalence
relation in Boomerang requires explicitly providing a {\em canonizer}, 
a pair of functions $(f,g)$ where $f$
...
a quotient lens in Boomerang requires separately defining four
...
canonical representations for the source and target data\bcp{Plus writing
  two canonizers!}.
...
The second drawback with writing quotient lenses in Boomerang is that even
after specifying the four required data formats, the programmer must still
define a lens for converting between the canonical representations.
Experience shows that writing such lenses can be tedious (e.g., consider
having to rearrange the order of data items by recursively using operators
to \kw{swap} adjacent fields) and that satisfying the type checker can be
difficult because the most obvious way to express a lens runs afoul of one
or more ``ambiguity checks'' that the type checker performs in order to make
sure that the lens laws hold~\cite{optician}.
...
space-separated form described by the QRE \codefont{space\_name}.  The
lenses generated by our synthesis algorithm are guaranteed to translate data
back and forth between source and target QREs and to satisfy the provided
examples.  Our synthesized quotient lenses have a relatively restricted
form, where the sequential composition operator is not used and all
canonization occurs at the very edges (which greatly speeds up synthesis),
but the normalization theorem means this restriction does not compromise
expressiveness.
...
interpreter~\cite{boomerang,quotientlenses}. 
The synthesis algorithm produces Boomerang lens values, so Boomerang
treats synthesized lenses the same as hand-written ones.
To evaluate the effectiveness of QREs, QRE lenses, and QRE lens
synthesis, we conducted experiments to answer the following questions:
%
\begin{itemize}
...
Augeas~\cite{augeas2}.  In the pre-QRE version of Optician, 10 of
these data formats had to be modified to work with the bijectivity
...
two approaches. \ksf{Anders, is there an order to the benchmarks in
this list?  It might be useful to list them in order of increasing
complexity of the format, perhaps measured by the sum of the number of
AST nodes in the source and target format descriptions.  It seems
better to have some order than just listing them randomly, even if it
doesn't show much interesting.}}
...
programmer uses bijective lens synthesis \`a la the original version
of Optician.  She must write canonizers by hand, along with 
...
\ksf{Anders: are these figures up to date?}
...
\end{subfigure}
\caption{Runtimes measurements. In (a), we run the
  original version of Optician (\OpticianRuntime{}) and \QOpt{}
  (\QREOptician{}) on the Optician benchmarks.  We find that there is only a negligible
  ...
  (In other words, we restored them to their original state and then
  ran \QOpt{}). 
  We find that \QOpt{} is able to
  synthesize all quotient lenses in under 20 seconds, and typically finishes in
  under 5 seconds.}
\label{fig:times}
\end{figure}

To assess the performance of QRE synthesis, we are interested in two
different questions.  First, how does the performance of \QOpt{} 
compare to the performance of the original
version of Optician on benchmarks that do not require QREs?  The
answer to this question tells us how much overhead we have introduced
by adopting the more general mechanism.  
Figure~\ref{fig:times}(a) shows that \QOpt{} was able to synthesize
all of the Optician benchmarks at a speed competative with the old
version. 
There is a small amount
of additional overhead introduced by QREs in calculating the $W$ and
$K$ functions, resulting in a slight decrease in performance.
...
lens for each of the 10 benchmark programs.  
We find that \QOpt{} is able to synthesize all quotient lenses in
under 20~seconds, and typically finishes in under 5~seconds.
