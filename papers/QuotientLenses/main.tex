...
This paper simplifies the task of programming quotient lenses with
three connected innovations.
...
Next, we introduce {\em QRE lenses}---lenses mapping
between QREs.
...
the {\tt data.gov} database.\bcp{The very last bit is inaccurate now.}
\end{abstract}
...
One way to synchronize such data is to use \emph{lenses}~\cite{Lenses}, which
...
in \cref{sec:example}, but first we discuss the \kw{normalize} combinator which
enjoys some special properties.
...
Having discussed the special properties of the \kw{normalize} combinator, we now
give the syntax and semantics of the other QRE combinators.
\subsection{Syntax and Semantics of QREs}
Formally, the language of Quotient Regular Expressions (QREs) is given by the
following grammar:
\begin{align*}
Q := \; &\id(R) \sep \project{R}{s} \sep \squash{Q_1}{Q_2}{f} \sep
...
\begin{enumerate}
  ...
  $\eqrel{Q}$, and
  \item a ``canonizing'' function $\canonize(Q):\mathcal{L}(W(Q))
  \longrightarrow \mathcal{L}(K(Q))$ which given any $w \in \mathcal{L}(W(Q))$,
  computes $\canonize(Q)(w)$ as the unique $k$ in $\mathcal{L}(K(Q))$ such
...
\begin{enumerate}
  ...
  \canonize(Q_1)(x_i)$ and ${y'_i} = \canonize(Q_2)(y_i)$,
\item synthesize, using Optician's algorithm, a bijective lens
  ...
  \item
  return $q = \rquot(\lquot(Q_1, \ell), Q_2) : Q_1 \Leftrightarrow
  Q_2$.
\end{enumerate}
This algorithm is complete in the following sense: given any QREs $Q_1$ and
$Q_2$ and set of examples $\{(x_1, y_2), \ldots, (x_n, y_n)\}$ with $x_i \in
W(Q_1)$ and $y_i \in W(Q_2)$, then if there exists a derivation of a QRE lens
$q : Q_1 \Leftrightarrow Q_2$ such that 
\begin{equation}\label{qrelensequation}
q.\get(x_i) \equiv_{Q_2} y_i \text{ and } q.\lput(y_i) \equiv_{Q_1} x_i \text{
for all } i,
\end{equation}
then the algorithm will return a QRE lens $q': Q_1 \Leftrightarrow Q_2$ that
also satisfies \cref{qrelensequation}. The justification for this fact follows
from the completeness of the Optician algorithm and \cref{normal form}.
...
benchmarks. (This experience alone suggests that QREs make the lens development
process easier.)
...
to be about half as large as \canonizeAndSpecSize, and about a third as large
as \LensAndSpecSize{}.\BCP{These are really the critical conclusions, and they
  are hard to see, the way the graph is drawn. I wonder whether we could instead
  present with the individual examples on the X axis (sorted by number of AST
  nodes), with a bar above each one containing three regions, colored
  differently, for LS, CS-LS, and QC-CS...}\afm{will do} This suggests that
introducing QREs saves programmers significant effort compared to both Optician
and basic Boomerang.
