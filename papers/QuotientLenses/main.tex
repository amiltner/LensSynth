...
and the \codefont{space\_name} format describes ``John Doe''
...
Another QRE primitive is the composition combinator ``;''. This combinator
applies two equivalence relations to a regular language in sequence. For
...
relation by a composition of a \textit{squash} QRE and a QRE that projects away
the extra whitespace in between \kw{first\_name} and \kw{last\_name}
...
project QRE \kw{p_wsp} which matches any non-zero number of whitespace
...
\end{lstlisting}
The QRE for the bibtex format is presented in its entirety in
...
interesting definitions. If $c = \squash{c}{c'}{f}$, then the whole language of
$c$ is $W(c) \sep W(c')$. This is because the \textit{squash} combinator takes
the whole language $W(c)$ of $c$, the whole language $W(c')$ of $c'$ and a
function $f : \mathcal{L}(W(c)) \longrightarrow \mathcal{L}(W(c'))$, maps
$\mathcal{L}(W(c))$ into $\mathcal{L}(W(c'))$ using $f$ and then canonizes
$W(c')$ into $K(c')$ using $\canonizer(c')$.
