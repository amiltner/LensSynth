...
{\em quotient} lenses in three steps.  First, we introduce {\em quotient regular
  expressions} (QREs), annotated regular expressions that conveniently mark
inessential aspects of string data formatsl; each QRE specifies,
...
one of the form $c ; \ell ; d^{-1}$, where $c$ and $d$ are {\em
canonizers}, $\ell$ is a bijective transformation, and $;$ is
composition\bcp{This seems not quite accurate.  It makes it look like
  canonizers are bidirectional things, but they aren't---at least, we
don't talk about them in that way (I suppose we could define the ``other
direction'' to be the identity, but we do not).  What we really mean are
left quotient and right quotient (and then we don't need the $^{-1}$).
Similarly, the meaning of 
``;'' here is not sequential composition of either lenses or QREs, which are
the only two meanings we give to this symbol in the paper.}.  
% Intuitively,
% these canonizers normalize data by 
% picking a canonical representative from the relevant equivalence
% relations. 
% BCP: Don't think this adds much, and it gets between the theorem and its use.
This theorem means we can push all normalization to the outer edges of a
composite quotient lens, leaving in the middle a pure bijective
transformation. Third, we leverage this normalization theorem to {\em
...
implemented QREs and QRE lens synthesis as an extension to the bidirectional programming
language Boomerang. We evaluate the effectiveness of our approach by 
synthesizing QRE lenses between various real-world data formats in the
...
are QREs. In the {\em get} direction, a QRE lens $q: P \Leftrightarrow Q$ uses the
source QRE $P$ to compute a canonical representative for the data modulo the
equivalence relation defined by $P$ and then applies the \kw{get} function of a
bijective string lens $\ell$ to this representative. In the {\em put} direction, $q$
operates similarly, but using the QRE $Q$ and the $\lput$ function of
$\ell$.
...
directed by these formats, $P$ and $Q$ are all that is required to
...
The \kw{Formless}\saz{Is this ``Formless'' ``Formlens'' or ``Formlet''? The code
  below says ``Formlens'' but the earlier parts of the paragraph say ``Formlet''}type is given by
\begin{center}
\begin{lstlisting}
...
\BCP{Wrote the rest---comments?}\saz{I swapped the last two paragraphs to end on
  a more exciting note.}
...
classes of lenses, such as ``classic'' asymmetric lenses~\cite{FosterLenses} or
