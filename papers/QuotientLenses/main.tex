...
\bcp{General typesetting comment: I frequently find the mixture of lens
  expressions and ordinary text hard to read because the fonts used for them
  are rather similar -- I have to look closely to notice that some
  characters have serifs and others don't.  We should not change the regular
  font -- that's mandated by the SIGPLAN style -- but could we make the lens
  font be something a bit more different?}
...
function $f : \mathcal{L}(W(c_1)) \longrightarrow \mathcal{L}(W(c_2))$ to map
...
\dpw{I believe the purpose of the preamble was to explain the structure of the subsections and why the
material presented there appears.  I tried to rewrite this to communicate that a bit more clearly.
...
One natural way to provide a language for defining bijective quotient lenses is to
(1) define a language of bijections and then (2) add quotients by allowing canonizers
to be prepended or postpended to bijections and (3) allowing composition of such quotient
lenses via the regular operators and functional composition.  Indeed, this is the program
that we follow, with Section \ref{subsec:bijective-lenses}
defining bijections and Sections\ref{subsec:qre-lenses-syntax} and
\ref{subsec:qre-lenses-semantics}
defining the quotients and their composition operators.

However, we also have a secondary objective, which is to support synthesis of lenses: When
provided with QREs describing the source and target languages, we would like to be able to
generate quotient lenses automatically.  One way to do that is to use the QREs to generate
canonizers $c$ and $c'$ and then to synthesize bijective lenses between the kernel languages
for $c$ and $c'$.  Doing so provides us with quotient lenses defined by a particular normal
form, one in which lenses are defined by a canonizer to transform source (and target) into a normal
form and a bijection to transform between canonical forms for the source and target.
However, this normal form does not allow for sequential composition (or composition via
the regular operators) of quotients. Hence, an important technical question is whether we give
up expressiveness if we restrict ourselves to this normal form.  Fortunately, as shown in
section~\ref{subsec:qre-lens-normal-forms}, we do not.
...
%% We define QRE lenses with canonizers on the edge and bijective lenses
%% in the middle because QREs define the canonizers ($c$ and $c'$) and the kernel
%% languages $K(c)$ and $K(c')$ directly.  The kernel languages can then
%% serve as inputs to a synthesis procedure for bijections~\cite{optician}.
%% Hence, from QREs alone, we are able to generate full bijective quotient
%% lenses.

%% \dpw{Check that I'm getting this right -- I replaced the previous paragraph.}
%% One potential drawback of our approach is that it does not admit
%% composition of quotient lenses ($q' \; ; \; q$) that might interleave canonizers and bijections.
%% Our key technical result is that the lack of
%% composition does not reduce the expressiveness of the language:  Any such composition can be
%% rewritten as a single quotient lens with canonizers on the edges.

%% One potential pitfall with QRE lenses is that the combinators that we define
%% on them may not preserve this normal form.
%% For example, the composition of
%% two quotient lenses $q' \; ; \; q$ of QRE lenses $q$ and $q'$ may
...
between regular languages constructed using the following combinators. \dpw{I said ``lens combinators''
instead of ``Boomerang lens
combinators'' because Boomerang has many other combinators... wasn't sure what the perfect thing to say was.}
...
direction. The $\mathit{id}(R)$ lens applies the identity function to the source and target
...
$\mathcal{L}(S)$.  The typing rules for bijective lenses are given in
Figure~\ref{fig:lensrules}.  The final rule in the figure deserves special attention.
It states that a lens
...
\label{subsec:qre-lens-normal-forms}
