...
the \emph{Quotient Regular Expression}, which simultaneously describes
external format, equivalence relation and canonical forms, and then to
synthesize lenses that map between these objects automatically.

\subsection{Quotient Regular Expressions}

A {\em Quotient Regular Expression} (QRE) defines an external regular
expression $R$,
an equivalence relation $\sim$ on the language described by $R$, and a canonical set
of representatives of $\sim$, all in one syntactic term.
Figure~\ref{fig:example-qre} illustrates how to 
specify \bibtex{} records and their canonical elements using QREs.
