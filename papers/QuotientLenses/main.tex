...
\caption{QRE lens from source $P$ to target $Q$.  $P$
  and $Q$ are QREs, each consisting of a ``whole'' set ($W(P)$ and $W(Q)$)
  and a ``kernel'' part ($K(P)$ and $K(Q)$)
  Between the two kernels is a plain bijective lens.  The {\emph get}
  function of the whole lens takes an argument from $W(P)$, applies the
  canonizer for $P$ to obtain a canonical representative in $K(P)$, then
  applies the {\em get} of the plain lens, yielding an element of $K(Q)$,
  which is a subset of $W(Q)$.  The {\em put} function does the reverse,
  mapping from $W(Q)$ to $K(P)$ (hence $W(P)$.\BCP{Both the figure and the
    caption are new---what do you think??  Too detailed?  Unreadable?}}
