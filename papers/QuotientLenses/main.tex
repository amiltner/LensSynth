...
Oftentimes, different applications store the same data in custom formats. For
example, \bibtex{} and EndNote both store bibliography data; spreadsheet data
can be represented as a CSV and as a TSV; web apis can return data as a JSON or
XML object. Synchronizing these applications amounts to synchronizing the data
formats. \emph{Lenses} comprise such synchronization functions, and the ``lens
...
in canonical forms. In \bibtex{}, an author can be specified as \cd{lastname,
  firstname} and \cd{firstname lastname}. While it is important to allow authors
...
{\em Quotient lenses}~\cite{quotientlenses} are a generalization of lenses where
each of the data formats is quotiented by some equivalence relation and the lens
laws are taken modulo equivalence. This both permits canonization functions and
allows lenses to be simpler and more flexible. Inessential details like
whitespace and orderings of fields can be ignored when writing the
transformations between the formats.
...
write a quotient lens, programmers must write with both standard lens
...
enable a programmer to simultaneously specify a regular expression while
simultaneously specifying the canonical representatives on the regular
expression's language. With QREs, programmers needn't define the data and
canonization functions separately -- by adding simple annotations to the data,
the canonization functions can be inferred.

Secondly, we use QREs to define a class of quotient lenses called {\em QRE
  lenses} which map betweeen QREs, and show how to synthesize QRE lenses. Using
this tool, a lens programmer need not manually derive the lens herself, she
merely must provide a pair of QREs (describing the formats the lens should map
between), and a suite of examples (demonstrating how the lens should act on
specific instances).  The resulting lens is guaranteed to map between the
languages of the QREs and satisfy the provided examples.

In summary, our main contributions are:
\begin{enumerate}
\item We introduce a novel language of {\em Quotient Regular Expressions} (QREs)
  that provide a compact, convenient notation for regular languages and their
  canonical representatives (Section~\ref{QRE}).
...
QRE lenses to the previously studied
...
to express with the existing Boomerang combinators. 

Furthermore, programming best practices require programmers provide type
annotations to their lenses to make explicit the formats the quotient lenses map
between. This practice prevents nefarious bugs that can result from the lenses
mapping between incorrect formats, but it is also verbose and time consuming.

To address these issues, we propose {\em Quotient Regular Expressions}, or QREs,
which enable the programmer to define a regular expression $R$ and its canonical
elements in the same expression. For example, with QREs we can specify \bibtex{}
records and their canonical elements, as shown in Figure~\ref{fig:example-qre}.
...
\verb+" "+ matches any non-zero number of whitespace characters, with a single
space as the canonical representative. We call this QRE a \textit{projection}
because an entire regular language is projected to a single representative
string during canonization.
...
\saz{Say more about how normalize
wouldn't be used in ``ordinary'' QRE programming?}\sam{I'm not sure what you
mean here exactly\ldots} However, checking that the canonizing function $f$ is
