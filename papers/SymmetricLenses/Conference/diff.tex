...
%DIF DEL main-submitted-icfp.tex   Tue May 14 15:41:59 2019
%DIF ADD main.tex                  Wed May 15 11:23:22 2019
% \newif\ifdraft\draftfalse  % set true to show comments
...
iCalendar, vCalendar, and h-event formats~\cite{calendar-formats}; \DIFdelbegin \DIFdel{US }\DIFdelend \DIFaddbegin \DIFadd{U.S. }\DIFaddend taxation
data can be transmitted via Tax XML (used by the \DIFdelbegin \DIFdel{US }\DIFdelend \DIFaddbegin \DIFadd{U.S. }\DIFaddend government for e-filing) or
...
bijective lens matching the examples, if one exists\DIFdelbegin \DIFdel{. Indeed, it can often find
an appropriate lens in less than a second with }\DIFdelend \DIFaddbegin \DIFadd{, often requiring }\DIFaddend no
examples at all. \DIFaddbegin \DIFadd{While specifying regular expressions can be cumbersome, doing so
is much easier than programming lenses -- regular expressions are widely
understood whereas few programmers are fluent in lenses, and writing lenses
require users reason about how to align two formats at once.
}\DIFaddend %, with just types describing the source and target formats.
...
Elided proofs can be found
\DIFaddbegin \ifappendices \DIFaddend in the appendices\DIFdelbegin %DIFDELCMD < \ifappendices%%%
\DIFdelend \else \DIFdelbegin \DIFdel{included with the supplemental material}\DIFdelend \DIFaddbegin \DIFadd{in the full version of the paper~\mbox{%DIFAUXCMD
\cite{sslfull}}%DIFAUXCMD
}\DIFaddend \fi.
...
\end{lstlisting}
  ...
  \setbox0=\hbox{%
    \begin{minipage}{3.5in}
\begin{lstlisting}
let company = (co_name . ("Co." | "Inc." | "Ltd.")) | "UNK"
...
\end{lstlisting}
  ...
  used by management and HR at a \DIFdelbeginFL \DIFdelFL{US }\DIFdelendFL company to represent employee salaries and health
...
\DIFdelbegin \DIFdel{U.S. }\DIFdelend company. In this company, management and human resources (HR) store
...
salary file creates \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend insurance file with the same employee names in the file,
with
...
\begin{figure}
\begin{lstlisting}
let name_lens = id(name) . id(" ") . id(name) . del(": ") . ins(",")
let employee_lens = name_lens . disconnect(salary,"","unk","") 
                                                    . disconnect("",company,"","UNK")
let employees_lens = ins("\n") . employee_lens . iterate(id("\n") . employee_lens)
let full_lens  :  emp_salaries $\Leftrightarrow$ emp_insurance = ins(header) . employees_lens
\end{lstlisting}
...
\]
The typing  rules \DIFdelbegin \DIFdel{for }\DIFdelend maintain structural similarity
...
assigns higher entropy (or \DIFaddbegin \DIFadd{lower }\DIFaddend likelihood) to lenses where knowing the string on one side 
...
\DIFdelbegin \DIFdel{$\PRegexStar{\lstinline{"c"}}{\Probability}$ generates the empty string with
probability $1-\Probability$, and generates any other string
in the language with
probability $\Probability$. More generally,
$\PRegexStar{\Regex}{\Probability}$ generates strings in }\DIFdelend \DIFaddbegin \DIFadd{$\PRegexStar{\Regex}{\Probability}$ generates strings in }\DIFaddend $\Regex^n$ with
probability $\Probability^n(1-\Probability)$\DIFaddbegin \DIFadd{. For example,
$\PRegexStar{\Regex}{\Probability}$ will generate a string in $\Regex$ with
probability $\Probability(1-\Probability)$ and a string in
$\RegexConcat{\Regex}{\Regex}$ with probability $\Probability^2(1-\Probability)$.}\DIFaddend .
...
we \DIFdelbegin \DIFdel{to }\DIFdelend extend the \emph{star-semiring}~\cite{starsemiring} equivalences (a
\emph{finer} notion of equivalence than semantic equivalence) to
SREs.  

\begin{theorem}
  If $\Regex \SSREquiv \RegexAlt$\DIFaddbegin \DIFadd{, }\DIFaddend then
  \DIFdelbegin \DIFdel{$\ProbabilityOf{\Regex}{\String} =
  \ProbabilityOf{\RegexAlt}{\String}$, for all strings $\String \in \LanguageOf{\Regex}$}\DIFdelend \DIFaddbegin \DIFadd{$\ProbabilityDistOf{\Regex} = \ProbabilityDistOf{\RegexAlt}$}\DIFaddend .
\end{theorem}

\DIFaddbegin \ifappendices\DIFaddend (For the proof, see the appendix 
\DIFdelbegin %DIFDELCMD < \ifappendices
%DIFDELCMD < %%%
\DIFdelend Theorem~\ref{thm:semantics-correctness}\DIFdelbegin %DIFDELCMD < \else{} %%%
\DIFdel{included with the
supplemental material}%DIFDELCMD < \fi%%%
\DIFdel{.) }\DIFdelend \DIFaddbegin \DIFadd{.) }\else\fi  \DIFaddend This theorem will come in handy as we traverse
...
Fortunately, we already require unambiguous regular expressions as input to our
synthesis procedure \DIFdelbegin \DIFdel{for other reasons}\DIFdelend \DIFaddbegin \DIFadd{to guarantee the synthesized lenses are well-typed}\DIFaddend , and we
can easily preprocess empty subexpressions out of SREs that are themselves
nonempty using the star-semiring equivalences (\EG,
...
symmetric; full details can be found in\DIFdelbegin \DIFdel{an appendix
}%DIFDELCMD < \ifappendices
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \ifappendices{} \DIFadd{appendix
}\DIFaddend \S\ref{sec:appendixlenses} \else{} \DIFdelbegin \DIFdel{(in the supplemental materials)}\DIFdelend \DIFaddbegin \DIFadd{the full version of the paper}\DIFaddend \fi. For
...
The \MergeR lens is interesting because it merges data where one \DIFaddbegin \DIFadd{piece of }\DIFaddend data can be in
...
string in $\RegexAlt$, synchronized by $\Lens$. We write \DIFdelbegin \DIFdel{$a(b,c)$ to
mean $(a*b,a*c)$, and $(a,b)+(c,d)$ to mean $(a+c,b+d)$}\DIFdelend \DIFaddbegin \DIFadd{$a[b,c]$ to
mean $[ab,ac]$, and $[a,b]+[c,d]$ to mean $[a+c,b+d]$}\DIFaddend .
\begin{center}
  \begin{tabular}{rcl}
    ...
    & =
    & \DIFdelbegin \DIFdel{$(0,\REntropyOf{\RegexAlt_1 \Given \Lens_1, \Regex}+\REntropyOf{\RegexAlt_2 \Given \Lens_2, \Regex}+1)$}\DIFdelend \DIFaddbegin \DIFadd{$[0,\REntropyOf{\RegexAlt_1 \Given \Lens_1, \Regex}+\REntropyOf{\RegexAlt_2 \Given \Lens_2, \Regex}+1]$}\DIFaddend \\
...
However, this information content is dependent on \DIFdelbegin \DIFdel{the }\DIFdelend how likely the
...
two regular expressions are \DIFdelbegin \DIFdel{equal }\DIFdelend \DIFaddbegin \DIFadd{equivalent }\DIFaddend modulo differences in associativity,
...
intellectually difficult. We elide them here but include them in
\DIFdelbegin \DIFdel{the
appendix }\DIFdelend \ifappendices \DIFaddbegin \DIFadd{appendix }\DIFaddend \S\ref{sec:appendixdnf}.  \else{}\DIFdelbegin \DIFdel{(in the supplemental
materials)}\DIFdelend \DIFaddbegin \DIFadd{the full version of the
paper}\DIFaddend .  \fi \DIFaddbegin \DIFadd{In the original Optician paper~\mbox{%DIFAUXCMD
\cite{optician}}%DIFAUXCMD
, DNF lenses were
proven equivalent in expressiveness to standard lenses. While we conjecture
symmetric DNF lenses are equivalent in expressivity to our standard symmetric
lenses, we have not proven this equivalence.
}\DIFaddend 
...
As an example, consider searching for a lens between \lstinline{"" | name.name$^*$}
and \lstinline{"" | name}. \GreedySynth might first pop \DIFdelbegin \DIFdel{the
sequence pair }\DIFdelend \DIFaddbegin \DIFadd{the
sequence lens between the
sequences }\DIFaddend \lstinline{""} and \lstinline{""}, because \DIFdelbegin \DIFdel{there }\DIFdelend \DIFaddbegin \DIFadd{it }\DIFaddend is a bijective
sequence lens between\DIFdelbegin \DIFdel{them}\DIFdelend . As neither \lstinline{""} is involved in a sequence
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    ...
    \State \DIFdelbegin \DIFdel{$\GreedyState \gets \Call{PQ.Init}{\ALS}$
    }\DIFdelend \DIFaddbegin \DIFadd{$\GreedyState \gets \Call{PQ.Create}{\ALS}$
    }\DIFaddend \State $\SeqLensBuilder \gets \Call{SLensBuilder.Empty}{}$
    \While{$\Call{PQ.IsNonempty}{\GreedyState}$}
...
lens (unless opened). Such regular expressions will automatically be opened. \DIFaddbegin \DIFadd{The
full details of expansion inference are explained in \mbox{%DIFAUXCMD
\citet{optician}}%DIFAUXCMD
.
}\DIFaddend 
...
\item We adapted 8 data cleaning benchmarks from Flash Fill~\cite{flashfill}. Flash Fill data
  ...
  Excel product team. \DIFaddbegin \DIFadd{The 8 we chose are merely the first 8 found in the Flash
  Fill paper. }\DIFaddend Note that our tool produces bidirectional transformations
...
\item We adapted 29 benchmarks from Augeas~\cite{augeas}. Augeas is a utility
  ...
  configuration files and serialized versions of Augeas dictionaries. \DIFaddbegin \DIFadd{We
  selected the first 28 benchmarks from Augeas in alphabetical order, and
  included one additional benchmark we considered particularly difficult,
  }\texttt{\DIFadd{xml\_to\_augeas.boom}}\DIFadd{. }\DIFaddend Because these benchmarks merely transformed the
  information into a structured form, synthesized lenses were all bijective.
...
examples. We \DIFaddbegin \DIFadd{manually programmed the regular expressions. However, when writing
these regular expressions, we did not massage them to be amenable to synthesis.
The synthesizer implements the star-semiring regular expression equivalence
axioms, so that users do not have to worry about the way they write expressions
themselves. We }\DIFaddend are synthesizing transformations between real formats, so these
tasks are can be quite large -- on average, the size of each RE in the
specification is 1637 and the size of the synthesized lens is 3254.
...
\begin{figure}
  ...
  \caption{Number of
    ...
    synthesis it is able to complete \DIFdelbeginFL \DIFdelFL{26. }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{31. }\DIFaddendFL Each benchmark specification includes
...
synthesis, the synthesis algorithm is able to solve \DIFdelbegin \DIFdel{26 }\DIFdelend \DIFaddbegin \DIFadd{31 }\DIFaddend out of 48 problem instances.
...
\DIFaddbegin \DIFadd{Both the bijective synthesis and the symmetric synthesis engines use a pair of
collaborating synthesizers that involves (1) a search for a compatible pair of
regular expressions and (2) search for a lens given those regular expressions.
Bijective synthesis is faster than symmetric synthesis because part (2) is much
faster. Specifically, a bijection must translate all data on the left into data
on the right, and this fact constrains the search. By contrast, a symmetric
synthesis problem has a choice of which data on the left to translate into data
on the right. This gives rise to a large set of possible choices, and symmetric
synthesis must consider all of them.
}
...
complexity of the information-theoretic measure \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend really
...
\DIFdelbegin \DIFdel{the appendix }\DIFdelend \ifappendices \DIFaddbegin \DIFadd{appendix }\DIFaddend \S\ref{sec:appendixforget}. \else{}\DIFdelbegin \DIFdel{included with the supplemental
material}\DIFdelend \DIFaddbegin \DIFadd{the full version of
the paper}\DIFaddend .  \fi We also comment on related
...
\begin{equation}
  ...
  \begin{mathprooftree}
    ...
    \noLine 
    \UnaryInfC{$\Lens.putl(y,c_2) = (\_,c_2')$}
    \DIFdelbegin %DIFDELCMD < \AxiomC{$putr(x,c_1') = (\_,c_1'')$}
%DIFDELCMD <     %%%
\DIFdelend \DIFaddbegin \AxiomC{$\Lens.putr(x,c_1') = (\_,c_1'')$}
    \DIFaddend \def\extraVskip{.5pt}
    \noLine 
    \UnaryInfC{$\Lens.putr(x,c_2') = (\_,c_2'')$}
    \def\extraVskip{2pt}
    \singleLine
...
of much prior attention.  For example, Gulwani\DIFdelbegin \DIFdel{and others }\DIFdelend \DIFaddbegin \DIFadd{'s and others' }\DIFaddend work on FlashFill
...
benchmarks, even in one direction. \DIFaddbegin \DIFadd{A comparison of Optician (on
bijective benchmarks) to these synthesis tools is included in~\mbox{%DIFAUXCMD
\citet{optician}}%DIFAUXCMD
.
}\DIFaddend %% Flashfill, designed
...
extraction and grammatical inference (see \citet{pereira:info-theory}, for an
introduction). Indeed, our work was inspired, in part, by the PADS format
...
principles in type-directed or syntax-guided synthesis of DSL programs. \DIFaddbegin \DIFadd{More
recently, the principles used in \mbox{%DIFAUXCMD
\citet{pads:synthesis} }%DIFAUXCMD
have been applied in
unsupervised learning~\mbox{%DIFAUXCMD
\cite{ULPS}}%DIFAUXCMD
. These works learn compact descriptions of a
single data set. Ideally, such descriptions are compact and information theory
is used as a measure of the compactness of the description learned. In contrast,
we are attempting to learn a translation from data set to another. However,
there are many different candidate translations. To select amongst the candidate
translations, we choose the translation that preserves as much information from
...
%DIFDELCMD < %%%
\DIFdelend % end conclusion

% begin acknowledgements
\begin{acks}
  \DIFaddbeginFL \DIFaddFL{We thank our anonymous reviewers for their useful feedback and discussions and
  Nate Foster and Michael Greenberg for their help integrating Optician into
  Boomerang. This research has been supported in part by DARPA award
  FA8750-17-2-0028 and ONR 568751 (SynCrypt).}\DIFaddendFL
\end{acks}
