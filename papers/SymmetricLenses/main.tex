...
regular expression pairs, each DNF lens is only typed between a single regular
expression pair. The relationship between DNF lens syntax and DNF regular
expression syntax is incredibly tight, permitting an incredibly efficient
...
problem: unless we find a lens with no cost, other lens classes may contain
better lenses. While we may have found a great lens between two regular
expressions, a different regular expression pair may provide an even better
lens. There may be a great lens in the class characterized by
\lstinline{crontab} and \lstinline{scheduled_job}, but it is not the the
intended lens shown in Figure~\ref{fig:example_lens}. However, there is a
tension, we want to find a good lens, but we also want to give quick feedback to
...
%  \end{tabular}
%\end{center}
Inverting is particularly useful when chaining many compositions together, as it
can be used to align the central types.
...
\]
Type equivalence is useful both for addressing type annotations and for
...
concatenation of its subcomponents, and the language of a stochastoc atom is the
...
\paragraph*{Entropy} In the situation where a DNF regular expression is
...
third component, \SSQLens{}, is a sequence lens that maps between the two
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \State $\GreedyState \gets \Call{GreedyState.AddAtom}{\GreedyState,\SAtomLens}$
    \EndIf
    \EndWhile
    \ReturnVal{\Call{GreedyState.ToDNFLens}{\GreedyState}}
    \EndFunction
...
This state consists of two things: an intermediate partial sequence lens and a
priority queue consisting of potential sequence lenses.  On initialization,
\GreedySeqSynth throws all the atom lenses into the priority queue.
...
be projected or be synchronized with exactly one atom in the other sequence.
Certain atoms need to be mapped, for example if a put changes the data of that
field. In such situations, an atom lens is necessary. A useful atom lens is
where adding that lens reduces the overall cost of the partial lens. If adding
that lens requires removing another lens, where the added cost by removing that
other lens is more than is removed by adding the proposed lens. Finally, after
all atoms have been popped, the partial sequence lens is converted into a
symmetric DNF lens. This is only possible if all atoms that need to be mapped,
are: if they are not, \PCF{GreedyState.ToDNFLens} instead returns \None.
