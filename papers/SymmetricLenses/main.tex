...
\end{lstlisting}
  ...
  \resizebox{\columnwidth}{!}{%
    \begin{tabular}{|>{\columncolor{lightyellow}}c|>{\columncolor{lightgreen}}c|}
      ...
      \listingA & \listingB \\
      \hline
      \cellcolor{yellow}\texttt{company.mgmt} regular expression & 
      \cellcolor{green}\texttt{company.hr}  regular expression\\
      \hline
      \cellcolor{yellow}\listingC & \cellcolor{green}\listingD \\
      \hline
    \end{tabular}
  }
  \caption{Example data files and corresponding regular expressions
  used by management and HR, respectively, to represent employee salaries and health
  insurance companies.  
   \ksf{fix colors so regular expressions are just slightly darker
  shade than data file.}}  
...
hypothetical U.S. company.  In this company, human resources and upper
management store information about employees in separate files:
Management records the names of employees and their salaries while
Human Resources (HR) stores the names of employees and their health
insurance providers.  Figure~\ref{fig:minimized-representations} show
two example files and regular expressions describing the formats.
The company wants to keep these two files synchronized:
if management adds a new employee, ``Chris Roe: 32500'', 
the corresponding entry ``Chris Roe, UNK'' should be added to HR's file.
