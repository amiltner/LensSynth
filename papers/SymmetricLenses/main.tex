...
% state of the world
Similar data can come in varieties of formats: bibliography information can be
stored in the BibTeX, EndNote, or ACM reference formats; web service resources
can be provided to clients in legacy or modern formats; calendar events can be
stored in CSV files or ICS files. Oftentimes, we want to synchronize files
across formats: when my collaborator edits her bibliography stored in the ACM
reference format, I want these edits reflected in my EndNote file; when I update
data through a legacy API, I want those changes to be reflected when I retrieve
it through a modern API; when I create a new calendar event in my ICS file, I
want my CSV calendar files to be updated to reflect this change.
...
other: updated APIs can expose new information, or omit legacy information; the
ACM reference format includes article number information, whereas EndNote
includes page number information; ICS files contain a URL field that Google
Calendar CSV files do not expose. We aim to synthesize transformations where the
data fields are not necessarily in bijective correspondence, where each data
format can contain important information not present in the other format.
...
\item We develop a means to traverse stochastic regular expression equivalences,
  ...
  languages~(\S\ref{sec:preliminaries}).
