...
\bcp{I wonder whether the rest of this subsection can be cut... It is both
  ...
  explanation.} \afm{I believe it is necessary for 2.4. I tried giving it a more
terse treatment, but that caused its own set of problems.}
...
the example strings). In general, {\em many} examples
may be 
...
giving
each string in the language equal probablity (not prioritizing one piece of
...
take a long time (for example, if the satisfying lens loses only a little
information,
but searching for a better one will take a long time, the discovered lens is
returned).
...
\bcp{Future work??????????????}\afm{I don't think there's room for it}
