...
\noindent
\dpw{XXXXXX New attempt at intro starts here}
...
Recent research~\cite{optician}
has exploited this fact by demonstrating that it is possible to synthesize relatively
large, complex lenses from specifications.
In particular, given types that describe source and target formats, there are synthesis
algorithms that will find bijective lenses~\cite{optician}
for medium-sized files, such as those housing linux
application configurations.  \ksf{why medium sized?  You can have very
large files with simple format descriptions that are bijective and for
which our previous systems would have worked just fine.} Oftentimes, these algorithms find such lenses in less than
a second, and without the aid of any examples at all, with just types describing the source and
target formats.  \dpw{I would cite the ICFP paper (cite string: maina+:quotient-synthesis) as to appear but I'm not sure how to do that
without possibly violating the spirit of double-blind reviewing. ie, if it hasn't appeared yet,
how could we know the content if we aren't the authors?  Mentioning it in the 3rd person does not
seem to suffice ...} \ksf{The ICFP 2018 website lists the paper; I
think it is okay to cite it.  The reviewer can assume we contacted the
authors for a copy.  We could ask Stephanie for a ruling if we want to
be sure.  One of the principles of double blind as adopted by SIGPLAN
is not to reduce the quality of the paper; omitting this citation
would have that affect.}\bcp{+1}\saz{Stephanie says we should cite it in the third
person, even if that's a bit awkward, but otherwise citing it is OK.}
...
data as inputs. \ksf{This seems magic.  Say something about how it is
possible/under what circumstances it is possible to not need a
complement?
At the price of some expressiveness, our new lenses eliminate the need
for complements; they require only source and target data as inputs.
}
...
from past instances of $S$ or $T$ when making a round trip.\dpw{fix that to make the intuitive
explaination sufficiently accurate}
...
had never before been implemented\bcp{Confusing: we implement only simple
  ones, but it was the earlier full form that had never been implemented}),
...
combinators,\bcp{confusing: they are not exactly ``compatible'' with the
  bijective combinators---they enrich them}
but with a rich set of advanced features including quotient lenses and matching lenses.

The second difficulty with synthesizing symmetric lenses is more fundamental\dpw{better word?}:
...
one a user wants.  In contrast, and to our surprise, our experiments show \dpw{can we have an experiment that
shows this?} that
...
\dpw{Were the bijective lenses chosen using a ``smaller'' criteria?  I
forget... if not edit the prior paragraph.}\dpw{Ideally, we could back up the claim
that there are many symmetric lenses and not many bijective lenses by graphing the
number of such lenses somehow.  eg, in a past synthesis paper with Steve (POPL 2016)
we graphed
the untyped terms of type $\mathsf{int}\rightarrow\mathsf{int}$.  I wonder if we can
...
additional information in order to reconstruct it---if it were\bcp{were
  what?  likely, or unlikely?}, then one is unlikely to
be attempting to synthesize a lens between the two formats\bcp{why? even if
  two big formats only share a little bit of common information, mightn't
  one want a lens between them?}!  In order to implement
...
question.\bcp{Is this implying that, if the annotation were
  different, we'd need something other than one bit to recover the string??}
...
Hence\bcp{?}, a lens is ``likely,'' in a technical sense, if relative to the target SRE $T$,
...
In summary, our main contributions are:\bcp{annotate with section numbers}
\begin{itemize}
...
\noindent
\dpw{XXXXXX New attempt at intro ends here}
...
Doe,Jane,Healthcare Inc.
Public,John,Insurance Co.
\end{lstlisting}
    \end{minipage}
  }
...
\end{lstlisting}
  ...
  \setbox0=\hbox{%
    \begin{minipage}{3.5in}
\begin{lstlisting}
let company = (name . ("Co." | "Inc." | "Ltd.")) | "UNSET" in
let emp_row = name . "," . name "," company in
let header = "firstname,lastname,company" in
let emp_insurances = header . ("\n" . emp_row)*
\end{lstlisting}
    \end{minipage}
  }
  ...
  \caption{Example files used by management and HR, and regular expressions
    describing the formats of these files.}
  \label{fig:minimized-representations}
\end{figure}

\afm{i commented out the previous version, because this one was quickly written
  up to give you something to work off. Feel free to delete it if it isnt
  helpful. } We motivate our problem with an extended example. In our
theoretical company, human resources and upper management both keep information
about employees. HR needs to know the names of the employees as well as their
insurance providers. Management needs to know the names of employees and their
salary. Two example files, and regular expressions describing the formats, are
shown in Figure~\ref{fig:minimized-representations}. These two files should be
synchronized: if management adds a new employee, ``Richard Roe: 32500'', a new
row should be added to HR's file, ``Roe,Richard,UNSET''. Furthermore, if HR
changes ``John Public'' to ``Jon Public'', that name change should be reflected
in both files. However, not all the data should be synchronized. The HR file
should not get altered due to salary changes, and the management file should not
get altered due to insurance company changes. While names should be matched
across the two formats, insurance company and salary information should be left
unmatched. Such a synchronization can be achieved with simple symmetric lenses.
...
one format when provided with data in the other. If a company.mgmt file is
initialized, a synchronized company.hr file should also be created, and vice
versa. Put functions are used to propagate edits from one format to the other.
If a company.hr file is updated, we should be able to combine it with a
non-updated company.mgmt file, and output a new company.mgmt file, synchronized
to the updated company.hr. To have a simple symmetric lens is to have these
required synchronization functions. Formally, a simple symmetric lens between
$X$ and $Y$ comprises 4 functions:
\begin{enumerate}
...
For synchronization, when the original company.hr or company.mgmt file is
...
information from the company.hr file, and knit it together with the information
specific to the company.mgmt file.
...
between \lstinline{emp_salaries} and \lstinline{emp_insurance}.  The finished
lens is shown in Figure~\ref{fig:example_lens}.
...
expected to updated in the other format. The identity lens propagates that
information.
%
\begin{lstlisting}
...
strings are used for the other format. In our extended example, salaries and
insurance companies should not be matched, so a disconnect lens would be used.
We want our default behavior to specify that the value has not been set.
%
\begin{lstlisting}
disc(salary,company,"UNSET","UNSET") : salary $\Leftrightarrow$ company
\end{lstlisting}
%
...
%
\begin{lstlisting}
or(id(""),multi_emp_map) : emp_salaries $\Leftrightarrow$ "" | ("\n" emp_row) . ("\n" emp_row)*
\end{lstlisting}

Data can also be concatenated, and so lens mappings can be concatenated.  This
is achieved with the concat map.
%
\begin{lstlisting}
concat(ins(header),multi_emp_map) : emp_salaries $\Leftrightarrow$ emp_insurance
\end{lstlisting}
%
Two lenses, $\Lens_1$ and $\Lens_2$, can be concatenated as
\lstinline{concat($\Lens_1$,$\Lens_2$)} or as the infix style
\lstinline{$\Lens_1$ . $\Lens_2$}.
...
example, if \lstinline{single_emp_map} synchronizes a single employee's data,
iterating that lens builds a lens that synchronizes a list of tasks.
%
\begin{lstlisting}
...
\begin{figure}
\begin{lstlisting}
let name_map = id(name) . ins(",") . id(name) . del(": ") . ins(",") in
let single_emp_map = name_map . disc(salary,company,"UNSET","UNSET") in
let multi_emp_map = ins("\n") . single_emp_map . iterate(id("\n") . single_emp_map) in
let emp_map = or(id(""),multi_emp_map) in
ins(header) . multi_emp_map : emp_salaries $\Leftrightarrow$ emp_insurance
\end{lstlisting}
...
Using these combinators, we can build up a lens that synchronizes HR and
management employee files. This lens uses one typing rule not previously
...
using the strings in Figure~\ref{fig:minimized-representations} as an
input-output example for \CreateR.
...
\lstinline{emp_salaries} and \lstinline{emp_insurance} is the one in
...
Now that we have a notion of best, we need a way to search for that best lens.
...
\saz{The following is hard to follow} Eventually, we cut the proposal-synthesis
...
\label{sec:ssl}
\sam{Perhaps we shoud introduce stochastic regular expressions first before introducing the lens combinators? This is because the type equivalence combinator mentions regexp equivalence, but this has not been introduced by this point in the paper.}
...
\sam{I think we should explicitly say what sets $A, B$, $SQ, TQ$ and $DS, DT$ range over.}
\begin{center}
...
iterations a .8 chance to continue iterating.\sam{I think we should mention that
.8 is chosen because it works well in practice.}

\paragraph*{Entropy}
\sam{We should define what entropy means and why it is significant}
...
\sam{I found that I understood the semantics easier than the typing
rules. Perhaps we should switch the order that we do them in?}
...
\sam{We should definitely explain the formulae for entropy computing entropy. Also I did not understand $argmin_{\theta}$ terms were being talked about in the 
previous sentence.}
\begin{centering}
  \[
    \begin{array}{rl}
...
\sam{Could it be better to describe the algorithm in English first before giving
pseudo code? I think that putting the algorithm first might cause readers to read the 
pseudo code and get confused whereas if the algorithm is first described in English then they have a better chance of understanding it.}
\begin{algorithm}
  \caption{\GreedySynth}
...
input-output examples. First, \PCF{CannotMap} checks if there is possibly a lens
between the given DNF regular expressions that is consistent with the examples.
\sam{I think we should reword this to say something like CANNOTMAP checks to see if the specification cannot be satisfied. To me it sounds like checking if a lens possibly exists is already going into synthesis.}If there is not, \GreedySynth returns \None early. After this, \GreedySynth
...
\sam{Perhaps a small example on initialization would help? I was wondering what
the partial DNF lens and potential sequence lenses could be initialized as}
...
disconnected.
\sam{I did not understand this section. In particluar, I'm still not sure I
understand a closed regexp vs an open regexp. Perhaps an example would help?}
...
\item Is our information theoretic cost metric useful in finding good lenses.
\end{itemize}
...
timeout of 1 minute, and average the result over 5 runs. Figure~\ref{fig:times}
