...
\paragraph*{Expansion Inference} Distinguishing between open and closed regular
expressions improves the efficiency of \GreedySynth, but forces \Expand to decide
which closed expressions to open.
At the start of synthesis, all regular expressions are closed. \Expand 
rewrites selected closed regular expressions to
open ones (thereby triggering DNF normalization)
when the disconnect lens is the only lens that will ever apply.
...
fast equality checks and saves memory.\dpw{we could probably cut this paragraph.}
...
while our default termination condition quickly finds the correct lens for
...
adjusting $t$. \dpw{should this paragraph be cut and its content left for the
next section?}
% end synthesis
