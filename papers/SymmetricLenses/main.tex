...
%
The recent Optician~\cite{optician} enhancement to Boomerang has
...
find any bijection, it is fairly likely to be the intended one.
...
symmetric lenses are exactly symmetric lenses that satisfy an intuitive
property called \emph{forgetfulness}.  We show they are expressive
enough in practice by adding simple symmetric lenses to the Boomerang
language~\cite{boomerang} and applying them to a range of real-world
applications, showing that simple symmetric lenses integrate
...
We consider ``likely'' lenses to be ones that propagate ``a lot'' of information
from the left data format to the right and vice versa. Conversely, unlikely
lenses require a large amount of additional information to recover
...
and target formats, measuring success using the likelihood measure.
...
tasks taken from Flash Fill~\cite{flashfill}, view-maintenance tasks
...
\item We also extend Boomerang with symmetric lens synthesis specifications:
  including types, examples, and relevance annotations. We evaluate our implementation, the effects of
...
\end{itemize}
Along the
way, we show the {\em star-semiring equivalences} on regular expressions
...
s\ t = t$ and $l.\PutL\ t\ s = s$. We can \emph{recover} $s$ from
...
required to specify an element drawn from $S$, in other words, the
...
the generated lens to retain \lstinline{name} information. In practice, we
define the cost of any lens that loses ``required'' information to be infinite.
%
\begin{center}
  \begin{tabular}{rcl}
...
\item We adapted 8 data cleaning benchmarks from Flash Fill~\cite{flashfill}. Flash Fill data
  ...
  as Flash Fill -- we ignore the other direction.
\item We adapted 29 benchmarks from Augeas~\cite{augeas}. Augeas is a utility
  ...
  configuration files and serialized versions of Augeas dictionaries.
...
examples. These tasks can be quite quite large, as we are synthesizing complex
transformations between real file formats -- some of the lenses and
specifications are over 500 AST nodes \afm{run the experiments and collect the
  data at some point}.
...
the most recently input $x$ and $y$. Such lenses correspond exactly with simple
