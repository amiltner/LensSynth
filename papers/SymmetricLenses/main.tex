...
Recent research~\cite{optician} has exploited this fact by demonstrating that it
...
synthesis algorithms that will find bijective lenses~\cite{optician} for files
...
formats. \dpw{I would cite the ICFP paper (cite string:
...
\afm{From an information theoretic perspective, yes, because that perspective
  ...
  these choices.} On the other hand, consider the language of the SRE \lstinline{c}. It contains
...
\item We define the space of simple symmetric lenses, which lies between
  ...
  they don't have any internal state (\S\ref{sec:overview}). We characterize the
  simple lenses semantically as the subset of full symmetric lenses for which a
  ``forgetfulness'' property holds (\S\ref{sec:related}). We also analyze the
  gap between simple and full symmetric lenses empirically and argue that simple
  ones suffice for many applications (\S\ref{sec:evaluation}).
