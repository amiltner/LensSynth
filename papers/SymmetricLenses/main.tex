...
$\REntropyOf{\Regex \Given \Lens, \RegexAlt}$ to calculate the expected amount
...
\begin{center}
  \begin{tabular}{rcl}
    $\REntropyOf{\Regex \Given \IdentityLensOf{\Regex}, \Regex}$
    & =
    & 0\\
    
    $\REntropyOf{\Regex \Given \DisconnectOf{\Regex}{\RegexAlt}{\String}{\StringAlt}, \RegexAlt}$
    & =
    & \EntropyOf{\RegexAlt}\\

    $\REntropyOf{\PRegexStar{\Regex}{\Probability} \Given \IterateLensOf{\Lens}^, \PRegexStar{\RegexAlt}{\ProbabilityAlt}}$
    & =
    & $\frac{\Probability}{1-\Probability}(\REntropyOf{\Regex \Given \Lens, \RegexAlt})$\\
    
    $\REntropyOf{\Regex_1 \Concat \Regex_2 \Given \ConcatLensOf{\Lens_1}{\Lens_2}, \RegexAlt_1 \Concat \RegexAlt_2}$
    & =
    & $\REntropyOf{\Regex_1 \Given \Lens_1^\leftarrow, \RegexAlt_1} + \REntropyOf{\Regex_2 \Given \Lens_2, \RegexAlt_2}$\\
    
    $\REntropyOf{\Regex_1 \Concat \Regex_2 \Given \SwapLensOf{\Lens_1}{\Lens_2}, \RegexAlt_2 \Concat \RegexAlt_1}$
    & =
    & $\REntropyOf{\Regex_1 \Given \Lens_1^\leftarrow, \RegexAlt_1} + \REntropyOf{\Regex_2 \Given \Lens_2, \RegexAlt_2}$\\
    
    $\REntropyOf{\PRegexOr{\Regex_1}{\Regex_2}{\Probability} \Given \OrLensOf{\Lens_1}{\Lens_2}, \PRegexOr{\RegexAlt_1}{\RegexAlt_2}{\ProbabilityAlt}}$
    & =
    & $\Probability*(\REntropyOf{\Regex_1 \Given \Lens_1, \RegexAlt_1} ) + (1-\Probability)*(\REntropyOf{\Regex_2 \Given \Lens_2, \RegexAlt_2})$\\
    
    $\REntropyOf{\PRegexOr{\Regex_1}{\Regex_2}{\Probability} \Given \MergeROf{\Lens_1}{\Lens_2}, \RegexAlt}$
    & =
    & $\Probability*(\REntropyOf{\Regex \Given \Lens_1, \RegexAlt_1}) + (1-\Probability)*(\REntropyOf{\Regex \Given \Lens_2, \RegexAlt_2})$\\
    
    $\REntropyOf{\Regex \Given \MergeLOf{\Lens_1}{\Lens_2}, \PRegexOr{\RegexAlt_1}{\RegexAlt_2}{\ProbabilityAlt}}$
    & =
    & $\ProbabilityAlt*(\REntropyOf{\Regex_1 \Given \Lens_1, \RegexAlt} - \Log_2(\ProbabilityAlt)) + $\\
    &
    & $(1-\ProbabilityAlt)*(\REntropyOf{\Regex_2 \Given \Lens_2, \RegexAlt} - \Log_2(1 - \ProbabilityAlt))$\\
    
    $\REntropyOf{\RegexAlt \Given \InvertOf{\Lens}, \Regex}$
    & =
    & $\REntropyOf{\RegexAlt \Given \Lens, \Regex}$\\
  \end{tabular}
\end{center}
$\LEntropyOf{\RegexAlt \Given \Lens, \Regex}$ is defined symmetrically.  These
...
\begin{theorem}
  ...
  \begin{enumerate}
  \item $\REntropyOf{\Regex \Given \Lens, \RegexAlt}$ calculates the expected
...
Observe that 
$$
V_{ \MergeROf{\Lens_1}{\Lens_2}}(s) = \{t \in \mathcal{L}(T) \; | \;  \MergeROf{\Lens_1}{\Lens_2}.\ell.\PutROf{s}{t} = t\}
$$
If $s \in \mathcal{L}(S_1)$, then
$$
...
Observe that
\begin{align*}
\sum_{t \in \mathcal{L}(T_1) \cap V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s)}P_{T_1}(t) \log_2 q * P_{T_1}(t) &= \log_2 q * P_{T_1}(V_{\ell_1}(s)) + H(V_{\ell_1}(s))
\end{align*}
Hence
\begin{align*}
&\sum_{t \in \mathcal{L}(T_1) \cap V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s)}\frac{q * P_{T_1}(t)}{P(V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s))} \log_2 \frac{q * P_{T_1}(t)}{P(V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s))} \\
&= \frac{q}{P(V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s))}\sum_{t \in \mathcal{L}(T_1) \cap V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s)}P_{T_1}(t) \log_2 q * P_{T_1}(t)\\
&- \frac{q * \log_2 P(V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s))}{P(V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s))}\sum_{t \in \mathcal{L}(T_1) \cap V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s)}P_{T_1}(t)\\
&= \frac{q \left[P_{T_1}(V_{\ell_1}(s)) \left[\log_2 \frac{q}{P(V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s))} \right] + H(V_{\ell_1}(s))\right]}{P(V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s))}
\end{align*}
Similarly,
\begin{align*}
&\sum_{t \in \mathcal{L}(T_2) \cap V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s)}\frac{(1-q) * P_{T_2}(t)}{P(V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s))} \log_2 \frac{(1-q) * P_{T_2}(t)}{P(V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s))}\\
&= \frac{(1-q)\left[P_{T_2}(V_{\ell_2}(s)) \left[\log_2 \frac{(1-q)}{P(V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s))}\right] + H(V_{\ell_2}(s))\right]}{P(V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s))}
\end{align*}
\end{enumerate}
