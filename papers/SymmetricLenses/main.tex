...
Semantically, a simple symmetric lens (or lens when clear from context) between
...
The notation $\Lens.\PutR$ extracts the \PutR function from the
...
will get back to exactly where we began. For example, consider a simple
symmetric lens
...
and the original salary file will return the original salary file, unmodified. If \KW{put}ting a string $s$ into a string $t$ results in the original string $s$, then $s$ and $t$ are considered \emph{synchronized}.
...
stochastic regular expressions; we mark plain REs with overbars.)
...
For example, we could use the following lens
%
\begin{lstlisting}
...
of employees in the HR format in the left-to-right direction. These combinators
...
Most of the typing rules for lenses are syntax driven, where only a handful of
rules result in a given type.  For example, the type of a
...
\]
With one notable exception, lens typing rules maintain structural similarity
between regular expression types and lens terms; type equivalence does not. Type
equivalence permits a well-typed lens to be well-typed among any equivalent
...
Even for small examples, lenses like the one above take a little getting used
to. For larger and more complex formats, it can become quite difficult to
...
%
\begin{lstlisting}
disconnect(emp_salaries, emp_insurance, ..., ...)
\end{lstlisting}
%
is another well-typed lens that satisfies the example in
Figure~\ref{fig:minimized-representations} (where \lstinline{...} represents
components of the example itself). In general, {\em many} examples may be
required to rule out all possible occurrences of nested \Disconnect lenses,
...
A satisfying lens is ``more likely'' if it u;ses more data from one format to
construct the other. One manifestation of this principle involves choosing the
identity lens (which uses all the data) rather than the disconnect lens (which
uses none of the data). We define the likelihood of a satisfying lens as the
expected number of bits required to recover data in one format from synchronized
data in the other. Higher likelihoods correspond to fewer bits. We say that two
strings $s$ and $t$ are \emph{synchronized according to lens $l$} if $l.\PutR\
s\ t = t$ and $l.\PutL\ t\ s = s$. We say that we can \emph{recover} $s$ from
$t$ using bits $b$ and lens $l$ if we can reconstruct $s$ from $t$, $b$, and
$l$. For example, given the \IdentityLens{} lens, we can recover $s$ from $t$
...
it from $t$ because $t$ contains no useful information. Thus, if both
...
information theory, each element of $s \in S$ has \emph{information content},
the number of bits required to specify that element in a perfect encoding scheme
($-\Log_2P(s)$). The entropy of $S$ calculates the expected number of bits
required to specify a random element pulled from $S$, in other words, the
probability of each element times its information content. Entropy maintains
that if a data source contains many possible elements, where none have
significantly higher probability than others, it will have high information
content. Data sources with few elements that don't have vanishingly small
probabilities have small entropy. When $P$ is clear, we will often use the
shorthand of $\EntropyOf{S}$ for $\EntropyOf{S,P}$.

In our setting, we already have a way of expressing sets of data: regular
expressions. To calculate entropy, what we need is a way to express probability
distributions over that data. To that end, we adopt \emph{stochastic regular
...
stochastic regular expression specifies both a set of strings and a probability
distribution over that set.

We use entropy to guage the relative likelihood of such lenses in our synthesis
...
To obtain SREs from the plain regular expressions that users write, by default
we use a heuristic that attempts to assign probability annotations that give
each string in the language equal probablity. In other words, this heuristic
tries to make each bit of data no more or less likely than any other.
Our synthesis algorithm could equally well take as input a pair of stochastic
regular expression rather than a plain one, permitting users to specify a
data-specific probability distribution (either manually calculated or inferred
from some dataset of interest).

However, sometimes users already know that certain data should or should not be
...
\lstinline{emp_ins} be annotated as required (as they are converted with the
identity lens).
...
type-directed search. In more detail, \GreedySynth first converts the stochastic
...
disjunctions normalized to operating over lists (see \S\ref{subsec:sdnfre}).
...
These restrictions mean that there are only a few simple symmetric n-ary DNF
lenses that are well-typed between each pair of stochastic n-ary DNF regular
expressions, hence \GreedySynth's search space for a given pair of regular
expressions is finite. Finally, \GreedySynth yields a simple symmetric lens by
converting the n-ary forms into the binary forms provided in the surface
language.
...
pass the next set of unrollings to \GreedySynth. The heuristics favor stopping
...
take a long time. With this approach, the algorithm can quickly respond with a
...
which serve both to rule out the previously proposed lens(es) and to reduce the
...
(see \S\ref{subsec:optimizations}).
