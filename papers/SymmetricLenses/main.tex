...
lenses that inhabit a disjunctive normal form that does not include composition.
...
current best lens has cost $c$, then $\Call{Continue}{pq}$ checks whether $c > d
+ \Log_2(n)$. This termination condition is based around two primary principles:
do not search overly deep ($d$), and do not tackle a frontier that would take
too long to process ($\Log_2(n)$). The log of the frontier is included because
the frontier grows much faster than lens costs grow. Lens cost grows with the
log of the expected number of choices in a given lens (due to its information
theoretic basis), so the frontier calculation does too.
...
\[
  ...
  \end{array}
\]
After this conversion has taken place, the sequences are ordered (thereby
normalizing for commutativity differences).
We have proven that this conversion respects languages and probability
distributions.
...
these probabilities provide good distributions of strings---stars are iterated 4
times on average, and no individual choice in a series of disjunctions is chosen
...
shown in Figure~\ref{fig:minimized-representations}. While salary information is
...
transform the salary to information present in \lstinline{emp_insurance}.
...
the other format, but it may well take quite a bit of work to find a
transformation that connects the salary in \lstinline{emp_salaries} to the
salary information in the second format. In such a scenario, early termination
may leave salary information discarded where it should be retained.
...
information appears exclusively in one format or the other or conversely appears
in both formats. By communicating this knowledge to the synthesis algorithm
...
For example, the user can easily recognize that salary information of
\lstinline{emp_salaries} is not present in \lstinline{emp_insurance}. By
annotating the \lstinline{salary} field as \lstinline{skip(salary)}, users can
add this knowledge to the specification to optimize the search. In practice,
we define \Skip by setting the information content of $\Skip(\Regex)$ to zero.
\begin{gather*}
...
add this knowledge to the specification to optimize the search and enforce that
the returned lens that retains \lstinline{name} information. In practice, we
define \SRequire by setting the cost of any lens that loses ``required''
...
The structure of symmetric DNF lenses exactly mirrors that of SDNF REs. A
symmetric DNF lens $sdl$ is a union of symmetric sequence lenses, $ssql_1 \ldots
ssql_p$, a symmetric DNF sequence lens is a concatenation of symmetric atom
lenses, $sal_1 \ldots sal_n$, and a symmetric atom lens $sal$ is an iteration of
a symmetric DNF lens.
