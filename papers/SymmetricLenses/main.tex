...
$Y$ comprises four functions that collectively satisfy four
``round-tripping'' laws:  
...
\end{minipage} 
\vskip 1ex
\bcp{I want less space between the put/create laws and their names.}
\noindent
...
We do not expect programmers to define simple symmetric lenses by
writing four functions by hand and showing they satisfy the round-tripping
laws.  Instead, we provide a set of combinators---a domain-specific
language---for building complex lenses from simpler ones.  Such languages
...
$\LanguageOf{\BRegexAlt}$. (Later, we will use undecorated variables for
...
In contrast, the \Disconnect lens does not propagate any edits
...
Finally, there are a number of combinators that compositionally construct
...
the forward direction (converting from the left format to the right).
...
%let emp_lens = or(id(""),employees_lens) in
  \caption{A lens that synchronizes management and HR employee files\bcp{I'm
    ...
    names should be id(" ") (was ins('','')).  Crossing my fingers that I'm
...
The expected number of bits required to recover data is the well-known
information-theoretic concept of the \emph{entropy} of the
data~\cite{Shannon1948}. Calculating entropy requires a probability distribution
over the space of possible values for the data. Specifically, given a set $S$
and a probability distribution $P \OfType S \rightarrow \Reals$ over $S$, the
entropy $\EntropyOf{S,P}$ is $-\Sigma_{s\in S}P(s)\Log_2P(s)$.
