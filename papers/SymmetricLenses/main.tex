...
Recent work by~\citet{miltner+:bijective-synthesis} has also demonstrated
...
so we use the disconnect lens to relate them\bcp{Potentially confusing to
  ...
  comprehensible, sure.  Anybody else have thoughts on the matter?}\bcp{No,
  I meant why not concatenate two disconnect lenses, one relating salary to
  the empty string and one relating the empty string to company.}.
...
function on them, much like one defines an $apply$ on classical symmetric lenses:
Given a simple symmetric lens and an optional pair of 
source and target strings,
$apply$ is a function from sequences of put objects to sequences of
put objects. If $apply(\Lens,\None,es) = es'$, then starting with no
prior data, after edit $es_i$, the lens \Lens synchronizes the other data\dpw{Is the ``other data'' $es$? or is it something else? If it is $es$ then we can say ($es$) in the sentence.} to
$es_i'$. \dpw{I wasn't sure what a ``put object'' was.  Is it a function? (If so, perhaps
call it a function and give its type?)} If $apply(\Lens,\SomeOf{(x,y)},es) = es'$, then that means, starting with
...
\sam{No; for example the identity lens of a concatenation of two regular expressions is the same as concatenating the two identity lenses on the respective regular expressions}
...
examples~\cite{flashfill,flashextract}.  On the one hand,
