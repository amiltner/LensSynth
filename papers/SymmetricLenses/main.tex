...
\saz{This abstract too long and doesn't get to the point quickly enough. We
  should tighten it up considerably.}
...
Unfortunately, writing such synchronizers is tedious and error
prone.\saz{``tedious and error prone'' is such a PL clich\'{e}, can we do better?}
Bidirectional languages ease the difficulty of writing such synchronizers, because a
...
languages are themselves difficult to program in, often requiring users to permute
...
\emph{Symmetric lenses} \saz{citations?} formalize this richer class of
...
to use in real-world synchronization tasks, because users\saz{lens developers?
  lens programs?}  must maintain the synchronized files as well as the
complements. The complement further complicates synthesis, because input-output
examples no longer suffice; instead edit sequences must be provided.

In this paper, we reformulate \saz{this sounds like they may be of the same
  expressiveness, but perhaps we mean to ``constrain'' symmetric lenses?}
symmetric lenses as \emph{simple symmetric lenses}. This reformulation does not
use a complement, and only uses the files that we aim to synchronize as
state. We identify a property, \emph{forgetfulness}, that identifies the subset
of symmetric lenses that can be written as simple symmetric lenses. We provide
...
formats. The synthesis algorithm uses type-directed synthesis on the two regular
expressions to synthesize a lens that is well-typed between the provided regular
expressions and satisfies the provided input-output examples.

\saz{This paragraph is hard to follow --- maybe introduce $\ell : S
  \Leftrightarrow T$ and use the names $S$ and $T$?}
...
data cleaning tasks. We find that our algorithm is able to synthesize all
\saz{or each?}  of these lenses in under 20 seconds.
...
\saz{Perhaps we should explicitly say here that we are trying to define a lens,
  or describing the functions that a lens provides.}
To synchronize these files, a few functions are needed. Two \emph{creates}
\saz{we haven't introduced ``create'' as a noun yet.} are
...
\end{lstlisting}
  \caption{A lens, written in our language for building simple symmetric lenses
    ...
    operates on the languages of the regular expressions.\saz{Say something
...
\saz{The following is hard to follow}
We address this issue by keeping track of the best lens found thus far, and
...
of rewrite rules performed on the pair, and identifies how many pairs
...
lens is very well synchronized, \RXSearch stops. In this
