...
% BCP: To make sure we don't leave comments on!
\bcp{XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX XXXX }
...
in both directions.
Since their 
introduction by~\citet{Focal2005-long}, lenses have been extensively
...
\noindent
In today's big-data world, similar information is often
...
targets for synthesis: their specialized, sub-Turing-complete syntax
and expressive type systems drastically
reduce the space of possible programs, making search-based synthesis easier. 
%
Recently, Optician~\cite{optician} has exploited this observation, demonstrating that
...
examples of corresponding data, the bijective synthesis algorithm will find a
bijective lens matching the examples, if one exists. Often,
it can find an appropriate lens in less than a second with
no examples at all.
...
One reason that bijective lens synthesis is so effective, even relative to
successful synthesis tools in other domains, is that there are typically not
very many bijections between related data formats. 
Hence, if the synthesis algorithm can
find any bijection at all, it is likely to be the right one.

However, many real-world data formats share just part of their information
content with other representations.  For instance, two related
database tables might share four out of five columns, but each might have a
different fifth column. 
Or, one ad-hoc system configuration file might include some format-specific meta
data, such as a date or a reference number, whereas the same configuration
file on another operating system does not. 
Indeed among the 48 benchmark problems described in Section \ref{sec:evaluation},
all of the benchmarks taken from Flash Fill~\cite{flashfill} and many of the
benchmarks that synchronize between two ad hoc file formats have this
characteristic, 13 benchmarks in total. Consequently, synthesizing only strict
bijections is often not useful in practice.

Could we extend synthesis tools to more kinds of lenses?  In particular,
what about \emph{symmetric lenses}~\cite{symmetric-lenses}, a class that
includes bijective lenses, ``asymmetric'' lenses where the transformation
from $S$ to $T$ can throw away information that must then be restored when
returning from $T$ to $S$, and even more flexibile transformations that
allow each side to retain information not present in the other?
...
However, we are still left with a second difficulty with synthesizing
symmetric lenses: While the number of bijective lenses between two related
formats is typically tiny, the number of simple symmetric lenses is
typically huge.  Thus, if a na\"ive search algorithm selects the first
simple symmetric lens it finds, this will generally \emph{not} be the one we
want.  We need a new principle for identifying ``more likely'' lenses and a
more sophisticated synthesis algorithm that uses this principles to search
the space more intelligently.

For these, we turn to information theory. 
...
of shared information, and so lenses that propagate
more information are preferable to ones that propagate less.
%
...
two formats\bcp{It's not obvious at this point what the {\em shared}
  information content has to do with anything.} and our measure of likelihood.
...
type-directed search will depend on which of the infinitely many possible
representations are chosen for its source and target formats.  Moreover,
there may be {\em no} lens connecting the 
source and target formats selected by the user: in general, the synthesis
...
to \Expand.  The whole synthesis algorithm terminates when comparing the
...
We added this synthesis procedure to the Boomerang implementation and evaluated
...
The ``programmer'' can control the search process via examples and annotations
...
In summary, our contributions are:\bcp{The section references do not proceed
  ...
  Live with it as-is?}\afm{Personally, I don't see it as a problem.  It's
  nice when the contributions mirror the paper, I am not jarred when they
  don't.}\bcp{But then the reader has no birds-eye view of the paper at
  this point.  I tried reorganizing, which I think also helps clarify the
  contributions.} 
\begin{itemize}
%
...
process (\S\ref{sec:synthesis})\bcp{``and prove it correct''???}.  
%
...
remain valid when applied to stochastic regular expressions, preserving
...
entry is added alongside the name to the management file when HR inserts a
...
the other direction will get back to exactly where we began.
