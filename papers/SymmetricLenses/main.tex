...
is a simple symmetric lens between formats described by $\LanguageOf{\Regex}$ and
$\LanguageOf{\RegexAlt}$. 
...
\ksf{This section needs more polishing}
...
\ksf{This paragraph needs to be updated, if nothing else, to reflect
the new example. Also, we haven't mentioned any typing rules so the
first sentence doesn't make sense.  Perhaps the whole topic could be
defered to later where we can explain why such a rule is necessary?}
