...
total).  \bcp{The safer wording is better, but the reader is still left
  wondering ``If they are demonstrating a system for synthesizing
  non-bijective lenses, why did they choose a benchmark suite where 35/48
  examples are bijective??''}
...
target formats. \bcp{There's a disconnect here: The reader was probably not
  thinking about Boomerang's type-equivalence rule (indeed, very likely, did
  not even know there was such a thing).}Moreover, if Boomerang's type-equivalence rule is disallowed,
there may be {\em no} well-typed lens connecting a particular pair of source
and target formats 
...
We evaluated our system on a benchmark suite of 48 problems, including
data-cleaning tasks taken from Flash Fill~\cite{flashfill}, view-maintenance
tasks taken from Augeas~\cite{augeas2}, and ad-hoc synchronization tasks
from a variety of other sources (mostly from the bidirectional programming
literature).  \bcp{``... and we found...''?}  \bcp{Also, this paragraph is
  redundant with the corresponding contribution a bit below.  I think it
  would improve the flow if we made this one a bit longer (and stronger) and
  the contribution paragraph shorter, just summarizing what this one says.}
