...
A {\em lens} is a program that can be run both ``front to back'' and ``back
...
procedure as extensions to the Boomerang language~\cite{Boomerang}. We
evaluate
its performance on 45 benchmark examples drawn from FlashFill, Augeus,
and an online collection of electronic file formats. We find this implementation is
able to synthesize all the lenses of the benchmark suite in under 30 seconds.
\end{abstract}
...
papers will find bijective lenses to convert between data stored
% ~\cite{optician}
...
number, whereas the other does not. Indeed of the 45 benchmark problems
described in Section \ref{sec:evaluation}, all of the benchmarks
taken from Flash Fill~\cite{flashfill} and all of the benchmarks that
synchronize between two ad hoc file formats have this 
characteristic, \afm{num} benchmarks in total. Consequently, tools that search
only for strict bijections will rarely be applicable in practice.
...
this naive approach runs into two difficulties. 
...
that if a naive search algorithm selects the first simple symmetric lens it
...
\begin{figure}
  ...
  \caption{Schematic diagram for the simple symmetric lens synthesis
  algorithm. The user provides 
   regular expressions \BRegex and \BRegexAlt and a set of examples \Examples{}
    as input. \Expand first converts \BRegex and \BRegexAlt to
    stochastic regular expressions \Regex and \RegexAlt with default
    probabilities. It then finds pairs of stochastic regular expressions
    equivalent to \Regex and \RegexAlt and iteratively proposes them to \GreedySynth.
    \GreedySynth finds a lens typed between the supplied SREs.
    When the algorithm determines it has found a likely lens, it
  returns it.  \ksf{Can we change cost to likelihood and l\_likely to l\_best?}
   }
...
earlier work~\cite{maina+:quotient-synthesis}.
...
effectiveness at synthesizing lenses on a 
benchmark suite of 45 problems, including data cleaning tasks taken
from Flash Fill~\cite{gulwani-popl-2014}, view maintenance
tasks taken from Augeas~\cite{Augeas}, and ad hoc format synchronization tasks
taken from a variety of other locations\footnote{The majority of these format
...
\begin{itemize}
\item We define simple symmetric lenses, which have
  the useful property that they do not maintain any hidden state
  (\S\ref{sec:overview}).  
  We give a language for defining such lenses and describe how to
...
\item We show how to synthesize simple symmetric lenses (including all
  ...
  To support that effort, we show that the star semi-ring equivalences on regular
  ...
  (\S\ref{sec:sre}).
\item We extend Boomerang with simple symmetric lenses,
  showing that such lenses can integrate smoothly with 
  other lens extensions such as quotient lenses and matching
  lenses.  We evaluate our
  implementation on a set of 45 lens synthesis benchmarks drawn from
...
our concluding thoughts (\S\ref{sec:conc}).
...
information theory: it is the \emph{entropy} of the data~\cite{Shannon1948}.
Calculating the entropy of data requires a probability distribution over the
space of possible values for the data. Specifically, given a set $S$ and a
probability distribution over that set $P \OfType S \rightarrow \Reals$, the
entropy of $S$ with probability distribution $P$ is $-\Sigma_{s\in
  S}P(s)\Log_2P(s)$.
...
probability annotations. In a slight abuse of notation\saz{Why is this an
  abuse?}\afm{We also use \BRegex{},\BRegexAlt{} as nonterminals}, if $\Regex$
is a stochastic regular expression, $\BRegex$ is the regular expression obtained
by removing probability annotations. The typing judgment $\Lens \OfType \BRegex
...
\item adds the SREs derived from rewriting $S$ and $T$ ($\Expand(S,T)$)
  to the priority queue.  The priority of a SRE pair is the
  number of rewrites needed to derive the pair from the originals.
\end{itemize}
...
This synthesizer comprises \Expand, \PQ, and \Continue. It proposes regular
expressions that are semantically, but not syntactically, equivalent to the
originals by performing the following rewrites:
\begin{center}
  \begin{tabular}{rcl}
...
When $\Call{Continue}{pq}$ returns $\False$, \Expand stops proposing regular
expression pairs, and instead returns to the user the best lens found thus far.
Note this means that if the algorithm finds a bijective lens, which has zero cost, it
...
language. \saz{I find the use of asterisk for the $\times$ or $\cdot$ or maybe
  just juxtaposition slightly jarring.}
\begin{center}
  \begin{tabular}{rcl}
...
calculate bounds of the expected amount of information required to recover a
...
the structure of symmetric DNF lenses and \SDNFREabbrevs. We describe this
synthesis algorithm using the running example of searching for a lens between
\lstinline{"" | name.name*} and \lstinline{"" | name}.
...
regular expressions to DNF form. This can be mitigated by not converting
\emph{all} of the regular expression to DNF form, but only converting
\emph{some} of it, and doing so incrementally. This change is easy
...
At the start of synthesis, all regular expressions are closed. \Expand is
...
\paragraph*{Expansion Inference} Distinguishing open and closed regular
expressions helps \GreedySynth, but doing so means that \Expand has the harder job of
determining when to open closed expressions.  To that end,
expansion inference identifies when a regular expression cannot
possibly map by any means other than the ``trivial'' disconnect lens. Expansion inference
opens those regular expressions to allow for further exploration.
...
with the examples, or if there is an existing lens (which is a composition of predefined
lenses) that already agrees with the examples.

\paragraph*{Termination Customization}
In this work, there are three competing constraints. (1) We want to return
lenses quickly to users, (2) we want to return good lenses, and (3) we don't
want to require users to write a large number of examples in their
...
correct lens.  Users could work around that problem by providing a large number
of examples, but it can be difficult to write a large number of informative
examples for complex formats. We find it simpler to instead let the users
specify that \SynthSymLens should be more restrictive in which lenses it
accepts.  Conversely, if the user's desired transformation has a particularly
...
For these situations, users can provide a number, $t$, which affects
...
one-size-fits all. Complex lenses, or lenses that project a
large amount of data, can be synthesized more easily and quickly by
adjusting $t$.
% end synthesis
...
quotient lenses. We also integrated our synthesis engine into Boomerang. It allows users to write synthesis tasks
...
\begin{enumerate}
\item We adapted \afm{8} data cleaning benchmarks from Flash Fill, though, of course, our tool produces bidirectional transformations rather than one-way transformers like Flash Fill.
...
Recall that compositional synthesis exploits user annotations to break a benchmark into a series of smaller synthesis steps. In our experience, compositional synthesis (SS mode) allows our system to scale to arbitrarily large and complex formats. However, SSNC mode, which uses no such annotations and synthesizes a complete lens all at once provides a useful experimental stress test for the system. Recall from Section
...
able to solve 26 problem instances. Recall from Section~\ref{subsec:optimizations} that users may tune the termination metric. Success on this set of benchmarks requires we tune that termination codition (e.g. for some benchmarks, allowing the system flexibility to spend more time considering more of the search space). In Section~\ref{subsec:effect-of-heuristics}, we will consider the effect of fixing the termination metric.
...
these benchmarks by either providing additional examples termination parameters.
Without these alterations, the symmetric synthesis synthesized a lens that fit
the specification but not the intended lens.
...
\begin{tabulary}{\linewidth}{rL}
  \AnyOpt{}: & We synthesize lenses without use of the information
               theoretic preference metric.  Once \GreedySynth finds a lens, the
  synthesis procedure returns that lens.\\
  \FLOpt{}: &  The first time \GreedySynth finds a lens, the synthesis procedure
              returns that lens. However, \GreedySynth uses the information
              theoretic preference metric to choose a lens.\\
...
those edits, no additional state needed. \fi While classical symmetric lenses
...
hope our information theoretic analysis provides a principle that can be
used to understand the heuristic choices made in related work.
