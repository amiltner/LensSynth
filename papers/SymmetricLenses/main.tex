...
\emph{forgetfulness}, which we define in Section~\ref{sec:relationship}.
...
effectiveness at synthesizing lenses on a range of
benchmarks, including data cleaning tasks taken from Flash Fill~\cite{gulwani-popl-2014}, information
extraction tasks taken from Augeas~\cite{Augeas}, and ad-hoc format synchronization tasks
...
\begin{itemize}
\item We define simple symmetric lenses, which 
  have the useful property that they do not maintain any hidden state
  (\S\ref{sec:overview}).  
  We describe a language for defining them and describe how to
  calculate the likelihood of a  particular lens (\S\ref{sec:ssl}). 
  We show simple symmetric lenses
  fall between asymmetric lenses~\cite{Focal2005-long2} and full
  symmetric lenses~\cite{symmetric-lenses} in expressiveness (\S\ref{sec:related}). 
\item We show how to synthesize simple symmetric lenses (including all
  ...
  particular, we show that the star semi-ring equivalences on regular
