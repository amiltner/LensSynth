...
paper and symmetric versions of the asymmetric extensions
...
\begin{enumerate}
\item Does the synthesis algorithm find the correct lens?
...
\item Which heuristics are useful
for reducing the ``specification 
...
\end{enumerate}
Each benchmark comprises a source and target regular expression, a
collection of examples, and an optional termination condition
parameter. 
...
All three sources for our benchmarks provide realistic problems
suitable for answering the research questions:
Flash Fill  data cleaning tasks are either derived from online help
forums or taken from the Excel product team;  
Augeas is a utility that transforms Linux configuration files into a manageable dictionary
form; and
many of the remaining benchmarks arise from synchronizing real-world
data formats such as BibTeX and EndNote. 

\subsection{Lens correctness}
To determine whether the synthesized lenses were the desired ones,
we ran the symmetric synthesis algorithm on the 48 benchmarks and then
manually inspected the generated lenses.  In all 48 cases, the
synthesized benchmark was correct.

\subsection{Usefulness in Everyday Development}

We measured the run time of our algorithm in two modes:
...
\begin{figure}
  ...
  \caption{Number of benchmarks that can be solved by a given algorithm in a
    ...
    benchmarks in under 30 seconds elapsed total time. In addition, it is able to
    complete a majority of the benchmarks in that timeframe without
  using compositional 
    synthesis.}
  \label{fig:times}
\end{figure}
...
For each benchmark in our benchmark suite, we ran it in both modes with a
...
39 bijective synthesis tasks. We had to slightly alter four of these
...
our 48 benchmarks.  Nine of them need help in the form of a termination parameter
