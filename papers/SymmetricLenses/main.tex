...
The first of these is pragmatic. ``Classical'' symmetric lenses as presented
...
symmetric lenses integrate smoothly with other advanced lens features
provided by Boomerang, including {\em quotient
  lenses}~\cite{quotientlenses,maina+:quotient-synthesis} and {\em matching
  lenses}~\cite{matchinglenses}.\bcp{How does it demonstrate this?  Did we
  actually {\em use} symmetric quotient or matching lenses for anything?  Or
are we just observing that these features can coexist in the same
implementation even if using them together might not make sense?}
...
one of the formats given the other. \bcp{Next two sentences are not very
  persuasive.  And they may be a bit out of date anyway, not that we have
  these annotations.  We can just say that we take ``propagate as much as
  possible'' as the default and use explicit annotations on the source and
  target types to mark information that the user knows should not be propagated.}For many application scenarios, users
...
primarily for these complex scenarios, and so lenses that propagate more
