...
(SREs)~\cite{?}, which simultaneously define a set of strings \`a la
...
\begin{figure}
  ...
  returns it.  \ksf{can we change the subscript on the returned lens
  to ``likely'' instead of ``best''? Also, is it the cost that is
  returned or the likeliness }
   }
