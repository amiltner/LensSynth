...
\item adds all of the SREs derived from rewriting $S$ and $T$ ($\Expand(S,T)$)
  to the priority queue with $\PCF{\PQ.Push}$. The priority of a SRE pair is the
...
originals. In particular, this synthesizer performs the following rewrites:
\begin{center}
  \begin{tabular}{rcl}
...
\[
  ...
  \end{array}
\]
This conversion utilizes operators defined in
...
The greedy algorithm is actually three algorithms, one that greedily finds
...
synthesis algorithm in the context of trying to find a lens between
...
``useful'' ones are kept. An atom lens is useful if adding it will lower the
cost of the generated sequence lens, or if examples show that one of the atoms
must not be disconnected. Each atom can only be part of one lens at a time, so
the algorithm must at times disconnect previously connected atoms to connect an
atom that must not be disconnected.

Finally, after all atoms have been popped, the sequence lens is validated and
returned. The algorithm confirms that all atoms that must not be disconnected
are involved in an atom lens. If they are not, \PCF{SLensBuilder.ToDNFLens}
instead returns \None.
...
Our implementation includes a number of optimizations to this algorithm. We
added ``closed'' annotations to regular expressions \saz{what do these mean? I
  guess ``open'' means ``subject to DNF conversion'' and ``closed'' means
  not?}\afm{I describe in greater detail below, should I add in additional info
  here too?}, an expansion inference algorithm, hash-consing and memoization,
along with compositional synthesis. The implementation also allows an additional
parameter in specifications that customizes the termination condition for the
synthesis procedure.
