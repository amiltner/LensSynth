...
\end{center}
\end{trivlist}
The two \KW{create} functions are used to \proposedelete{propagate shared
...
lens\bcp{In all these examples, I'd like to have one more space on each side
of the ``:''}
%
\begin{lstlisting}
...
\bcp{I wonder whether the rest of this subsection can be cut... It is both
  hard to understand and too brief to really say much, and I wonder whether
  we need it for anything between here and where we give it a real
  explanation.} 

Most of the typing rules for lenses are syntax directed, where only a handful of
rules result in a given type\bcp{syntax directed usually means that the
  conclusion of at most
  {\em one} rule will match a given goal; if that's not the case here, then
  we need to speak more carefully}.  For example, the type of a
