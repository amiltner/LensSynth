...
content. Data sources where most elements have small
probabilities have small entropy. When $P$ is clear from context, we will
...
assigns higher entropy (or cost\bcp{= lower likelihood?  I'm still confused
  by these different ways of speaking about what seems to be the same
  thing.}) to lenses where knowing the string on one side 
...
tries to make each bit of data no more or less likely than any
other.\bcp{huh?} 
Alternatively, our synthesis algorithm could take as input a pair of stochastic
regular expressions rather than plain ones, permitting users to specify a
data-specific probability distribution (either manually calculated or inferred
from some dataset of interest).\bcp{So why didn't we do it this way?}

However\bcp{``However'' means that something from before is being
  contradicted.  What is it in this case?}, sometimes users already know that certain data should or should not be
...
identity lens).  \bcp{That paragraph seems way too brief and high level.
  These annotations were added in response to significant complaints from an
earlier PC, but the paper reads as though they are an afterthought of little
consequence; they deserve more attention and explanation.}
...
algorithm should terminate for no other lens will be more likely\bcp{this
  case is 100\% obvious, which makes it not so interesting as an example.
  The reader is left wondering what happens with pretty likely but not
  maximally likely lenses?}). The
...
(see \S\ref{subsec:optimizations}). \bcp{Again, I feel like we're
  downplaying this aspect too much.}
...
\begin{figure}
  ...
  \vspace{-4ex}
  \caption{\bcp{What are the little tick marks on the top of the figure?
      And why do we need a line for ``benchmark count''?}Number of
