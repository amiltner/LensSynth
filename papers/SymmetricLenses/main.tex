...
In summary, our main contributions are:\bcp{We need to find a way to mention
sections 3 and 4!}
\begin{itemize}
...
\end{itemize}
\bcp{Related work?  Conclusions?  Appendices?}
...
\end{lstlisting}
  ...
  the star operator? Can we simplify?}\afm{It does require the or operator, if
  we don't want it to have trailing or preceding newlines.  Howevever, I
  intentionally had this as not having those to force a discussion about
  rewrites and searching through rewrites.} }  
...
$\BRegex$ and $\BRegexAlt$\footnote{We use nonterminals \BRegex and \BRegexAlt
  ...
  expressions.} is defined by four functions.\bcp{I think a picture would be
  helpful. I can probably make one pretty easily by tweaking the one from the
  original symmetric paper.} \afm{That would be awesome.}
...
data appears on one side but not the other\bcp{confusing wording: the whole point of
symmetric lenses is that data can appear on one side but not the other} (\EG, the ``unk'' salary
...
edits\bcp{I think you mean ``identity edits,'' but even then this is going
  to be very opaque to readers} to one file do
...
We use regular expressions\bcp{At this point, we are just talking about
  simple symmetric lenses in general: we do not have to restrict to regular
  expressions as types.}\bcp{Also, we should say briefly how this definition
differs from full symmetric lenses.} to denote the types of data formats, and we
...
unconnected\bcp{``leaves data
unconnected'' doesn't say much to me}. The disconnect lens takes four arguments: two regular expressions
...
relate them\bcp{Potentially confusing to readers (and, even to me, hacky).
  Why not relate each of them to the constant 
  empty string?}. The defaults are strings that indicate the information is missing.
%
\begin{lstlisting}
...
equivalence, so if a lens is typed between\bcp{``is typed between'' is
  awkward and potentially confusing} two regular expressions, it is also
...
transform data\bcp{formats do not transform anything} in complex and unintuitive ways. To alleviate the difficulty of
...
\OfType S \rightarrow \Reals$, the entropy of the $(S,P)$ pair is $-\Sigma_{s\in
  S}P(s)\Log_2P(s)$. 
\bcp{Wait---is entropy a property of the whole set of values, or of
  individual data values in the set??  I think the former, but some of the
  explanation made me think the latter.}
...
$\Regex$ given $\RegexAlt$ and $\Lens$\bcp{Am I supposed to know what this
  means?} and the conditional entropy of
$\RegexAlt$ given $\Regex$ and $\Lens$.\bcp{Why
  is this the right (or a sensible)  thing to do, besides the fact that it
  ensures symmetry?}
...
it.\bcp{A way to search for likelihood???}
...
by \RXSearch)\bcp{``ring equivalences'' and ``star semiring equivalences''
  are jargon that most readers won't know} to normalize the
stochastic regular expressions provided by \RXSearch into 
\emph{stochastic n-ary DNF regular expressions}\bcp{As a reader, I can
  sort of imagine what a ``DNF lens'' might be, but it would be better to say
  it.  And I have no idea what ``n-ary'' might be.}.  
...
having a string involving $n$ iterations\bcp{exactly n?  n or fewer?}. The semantics of SREs is defined by
...
However, as of yet\bcp{meaning that this is something we invented?  or was
  it present in other treatments of stochastic REs?}, there is no syntactic means to reason about stochastic
regular expression equivalence. We extend the \emph{star-semiring}\bcp{citation?} equivalences
to stochastic regular expressions.\bcp{Why these?}
...
\begin{proof}
  ...
  the auxiliary materials accompanying this submission.\bcp{Not in the appendix?}
\end{proof}
...
when the stochastic regular expression is unambiguous and contains no nonempty\bcp{empty?}
subcomponents.
\begin{center}
  \begin{tabular}{rcl}
    $\EntropyOf{s}$
...
should be $0$.\bcp{This doesn't show that it's not easy---it just shows that
one particular way of computing it is wrong.} Similar issues occur\bcp{Can we at least hint what they are?} when trying to find the entropy when
$\emptyset$ is a subterm, so we do not define the entropy on $\emptyset$.

In our work, we do not find this to be a problem\bcp{Why might one expect
  it to be a problem, rather than just a technicality?}: we require
unambiguous regular 
...
(\EG, \PRegexOr{\emptyset}{\lstinline{"s"}}{.5} \SSREquiv \lstinline{"s"}).
...
simplicity\bcp{Is there any known issue with relaxing this restriction?
  I've often found ti convenient to use id lenses with ambiguous REs...}, we define the typing derivations and semantics only over unambiguous
...
two formats, and one data has only one format. In previous work\bcp{where?},
this was
...
$\REntropyOf{\RegexAlt \Given \Lens, \Regex}$ to bound\bcp{``bound'' or
  ``calculate''?  I.e., is what we calculate correct, or is it only known to
be an upper bound?  If (as it seems) the answer is the latter, then why?}
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \EndFunction
  \end{algorithmic}
\bcp{Why does the RETURN statement not get a number?}
\end{algorithm}
...
lens (a lens of cost 0\bcp{have we proved that the bijective lenses are
  exactly the ones with zero cost?  That would be nice to state clearly someplace...}) it will immediately return.
%
...
expressions into stochastic regular expressions, \ToStochastic\bcp{from the
  description above, I thought this step was optional (because we allowed
  the user to write SREs directly)}. If $\Regex$ is a
...
expressions whose DNF form gives equal probability to all sequences\bcp{Why?}\bcp{Also, what does this mean?  If a given RE denotes an
infinite language, then there is no way to give equal probability to every
string in that language, is there???}, and gives
iterations a .8 chance to continue iterating. In our experience generating
random strings from regular expressions\bcp{what does generating random
  strings have to do with synthesis? i.e., why is this a good thing to measure?}, these probabilities provide good
...
list of symmetric atom lenses\bcp{why are they called atom lenses?}, as well
...
\bcp{The following is all pretty incomprehensibly dense.  A little running example
might help.}
...
\end{align*}
To compute $H(V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s))$, we use the formula 
...
\begin{align*}
&H(V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s)) = H \left(\frac{q * P_{T_1}V_{\ell_1}(s)}{P_{T_1 \; |_q \; T_2}{V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s)}}, \frac{(1-q) * P_{T_2}V_{\ell_2}(s)}{P_{T_1 \; |_q \; T_2}{V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s)}}\right)\\
&= \frac{q * P_{T_1}V_{\ell_1}(s)}{P_{T_1 \; |_q \; T_2}{V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s)}} H(V_{\ell_1}(s)) + \frac{(1-q) * P_{T_2}V_{\ell_2}(s)}{P_{T_1 \; |_q \; T_2}{V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s)}} H(V_{\ell_2}(s))
\end{align*}
...
&= \frac{q * P_{T_1}(V_{\ell_1}(s)) \left[\log_2 \left[\frac{q * P_{T_1}(V_{\ell_1}(s))}{P(V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s))}\right] - H(V_{\ell_1}(s)) \right] }{P(V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s))}\\
&+ \frac{(1-q) * P_{T_2}(V_{\ell_2}(s)) \left[\log_2 \left[\frac{(1-q) * P_{T_2}(V_{\ell_2}(s))}{P(V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s))}\right] - H(V_{\ell_2}(s)) \right] }{P(V_{ \MergeLOf{\Lens_1}{\Lens_2}}(s))}
\end{align*}
\fi
\end{enumerate}
