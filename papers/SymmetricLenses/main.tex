...
Figure~\ref{fig:high-level-algorithm}. Our algorithm consists of two
...
\end{lstlisting}
  ...
  \caption{Hello there this is the thing}
  \label{fig:minimized-representations}
\end{figure}

We motivate our problem with an extended example.  Consider the previously
described example of keeping Linux and Windows scheduled jobs synchronized.
In this example, we describe keeping synchronized an minified version of
crontab and the task scheduler.  These are each formats specifying when to run
recurring tasks.  For simplicity, we are using an minified version of these
configuration files where we can schedule tasks to occur at fixed intervals,
hourly, daily, or weekly.  Each task consists of a command to be run.
...
not always named the same thing.

To synchronize these files, we wish to synchronize how often the commands are
run, but not synchronize the names of the commands. We consider 4 functions to
be necessary to synchronize these two files: a function that makes a new cron
file from a newly created task scheduler, a function that makes a new task
scheduler file from a new cron file, a function that merges edits to the cron
file to the synchronized task scheduler file, and a function that merges edits
on the task scheduler file to the cron file. These are the only four functions
we need to synchronize these two files, so these four functions comprise a
simple symmetric lens between cron files and task scheduler files.  More
formally, a simple symmetric lens between $X$ and $Y$ comprise the following
four functions:
\begin{enumerate}
...
Furthermore, we expect these functions to satisfy certain properties.  For
example, if two files are synchronized by one of these functions, using another
function to resynchronize it will not change the result.  These are viewed as
