...
\begin{abstract}
  ...
  require external state in the form of a \emph{complement}. This statefulness
  makes symmetric lenses both difficult to use and difficult to synthesize.

  In this work, we reformulate symmetric lenses as \emph{simple symmetric
    lenses}. While this formulation is less expressive than symmetric lenses, it
  expresses many symmetric lenses while requiring \emph{no external state}. We
  provide combinators for simple symmetric lenses on string data, and integrate
  these combinators into Boomerang while providing full backwards compatibility.
...
\emph{complement}. These complements makes symmetric lenses difficult to use in
real-world synchronization tasks, as users must maintain the synchronized files
...
to this format and as such should be left unaligned (changes to this data are
not propagated to the other format), or is this data present in both formats and
as such should be aligned (changes to this data change are propagated to the
other format).
...
Both a language and a probability distribution over that language is represented
by a stochastic regular expression, and we develop a way to syntactically infer
the expected information content (or \emph{entropy}) of an unambiguous regular
expression. This entropy is used to calculate the expected information content
to recover a string, when given a string it is synchronized to.
...
data cleaning tasks. We find that our algorithm is able to synthesize all these
lenses in under 20 seconds.
...
<Tasks>
 <Task>
  <Command>c</Command>
  <Time>0</Time><Time>20</Time> <Time>40</Time>
 </Task>
 <Task>
  <Command>c</Command>
  <AllTimes/>
...
\end{lstlisting}
  ...
  \setbox0=\hbox{%
    \begin{minipage}{3.5in}
\begin{lstlisting}
let minute_list = minute . ("," . minute)$^*$ in
let times = "*" | minute_list in
let line = times . " " . command in
let crontab = "" | line . ("\n\n" line)*
\end{lstlisting}
    \end{minipage}
  }
  ...
  \setbox0=\hbox{%
    \begin{minipage}{3.5in}
\begin{lstlisting}
let minute_xml = "<Time>" . minute . "</Time>" in
let minutes_xml = minute_xml . minute_xml$^*$ in
let xml_time = "<AllTimes/>" | minutes_xml in
let command_xml = 
  "<Command>" . command . "</Command>"
in
let xml_task = command_xml . xml_time in
...
\end{lstlisting}
  ...
  \caption{An example crontab and Scheduled Tasks file, and regular expressions
    describing the formats. For simplicity, these are modified versions of the
    real formats.}
...
this example, we describe keeping the file containing cron jobs (crontab) and a
Scheduled Tasks file (scheduled.job) synchronized. These are each formats
...
in Figure~\ref{fig:minimized-representations}. Each file contains a list of
tasks set to run at specific times every hour. In both these formats, users can
either provide specific minutes they want to run the program during or they can
specify they want to run the program at every minute.
...
Windows equvialent for Linux commands, that equivalent command is not always
...
needed; if a crontab is initialized, a synchronized Scheduled Tasks file should
also be created, and vice versa. Furthermore, \emph{puts} are needed; if a
crontab is updated, we should be able to combine it with a non-updated Scheduled
Tasks file, and output a new Scheduled Tasks file, synchronized to the updated
...
examples. In this example, we wish to synthesize a lens from \lstinline{crontab}
and \lstinline{scheduled_job}, using the strings in
Figure~\ref{fig:minimized-representations} as an input-output example for
\CreateR.
...
expected number of bits of a data source is the \emph{entropy} of that data
source.
...
regular expressions), we do not have probability distributions. In this work, we
...
probabilities. In this work, we guess a reasonable probability distribution from
the syntax, though if users manually provided stochastic regular expressions, or
large suite of data in the formats through which we could learn the annotations,
...
given $\Regex$ and $\Lens$. Now that we have a notion of best, we need a way to
...
\emph{stochastic DNF regular expression} pair. Where a stochastic regular
expression pair characterizes a given class, a stochastic DNF regular expression
pair is highly normalized -- it is the \emph{unique} pair that characterizes a
given class. Then, we use the syntax of the DNF regular expression pair to
greedily find a highly normalized version of a simple symmetric lens, a
\emph{symmetric DNF lens}. Finally, this symmetric DNF lens is converted into a
simple symmetric lens.

However, these communicating synthesizers pose a problem: unless we find a lens
with no cost, further search may find a better lens. While we may have found a
great lens between two regular expressions, a different regular expression pair
may provide an even better lens. There may be a great lens in the class
characterized by \lstinline{crontab} and \lstinline{scheduled_job}, but it is
not the the intended lens shown in Figure~\ref{fig:example_lens}. However, there
is a tension, we want to find a good lens, but we also want to give quick
feedback to the user. After finding the lens in Figure~\ref{fig:example_lens},
we dont want the algorithm to spend much longer searching for better lenses.

We address this by keeping running track of the best lens found thus far, and
eventually cutting the proposal-synthesis loop short. Before proposing new
types, the regular expression generator, \RXSearch, evaluates whether it should
continue proposing regular expression pairs, or whether it is content with the
current best. For a given regular expression pair, \RXSearch looks at the number
of rewrite rules performed on the pair, and identifies how many pairs with
involving the same number of rewrites may be proposed. If there are a large
number of pairs that can be proposed with that many rewrites, or the current
lens is very well synchronized, \RXSearch does not propose that pairs. In this
way, \RXSearch will continue searching for better lenses, but does not tackle
huge search spaces unless it needs to. In this way, the algorithm can provide
lenses quickly to the users, which users can refine with additional examples.
% end overview
...
\afm{there isn't much to say here, should I put these in a figure or something?}
...
metrics require a notion of probability distributions \emph{how are probability
  ...
  to different subcomponents and parts of the data}. We annotate
the regular expressions with a reasonable probability distribution, making them
...
user the best lens found thus far.  Note, if \RXSearch finds a bijective lens (a
lens of cost 0), it will immediately return, as would be expected.

\afm{dont forget to add in description of termination}
%

\subsection{\GreedySynth}
\label{subsec:greedy-synth}
The second synthesizer, \GreedySynth, actually generates the lenses. It does
this by first converting the regular expressions into \emph{stochastic DNF
  regular expressions}, intuitively stochastic regular expressions with all
disjunctions distributed over all concatenations. Next, a good DNF lens is
greedily found between these stochastic DNF regular expressions, using the
probability annotations on the DNF regular expressions to find the cost.
Finally, this DNF lens is converted into a simple symmetric lens.

\paragraph*{Stochastic DNF Regular Expressions}
\label{subsec:stoch-dnf-regex}
...
\paragraph*{Symmetric DNF, Sequence, and Atom Lenses}
...
\paragraph*{Sequence Lenses, Typing and Semantics}
...
\paragraph*{Atom Lenses, Typing and Semantics}
...
symmetric DNF lenses and one that greedily finds symmetric sequence lenses.

The algorithm for symmetric DNF lenses is relatively straightforward.  First,
using examples, we identify which atoms in one format \emph{must} be
synchronized to certain atoms in the other.  After that, we iteratively find the
pairs of atoms with lowest cost, where at least one of the atoms is not mapped
to another atom.  The cost of these pairs is not merely the cost of the lens,
but also the cost incurred by adding that lens, if one of the atoms is already
involved in a map, the lens has additional cost to account for the merge cost.
This continues until there are no more atoms that aren't mapped to another atom.
At this point, the creates for each lens is determined.  If a examples dictate
one lens must create to another, that is the create behavior, otherwise atoms
create along the lens of least cost.

The algorithm for symmetric sequence lenses is less straightforward.  Unlike
symmetric DNF lenses, where multiple subcomponents can map to the same
subcomponent, each atom can map to at most one other subcomponent.  This is
complicated by the fact that examples sometimes dictate one atom must map to
another.

To address this, we introduce the \emph{profit} of a adding a mapping between
two subcomponents. The profit is the cost of the lens, minus the information
content of the subcomponents it maps between, plus how much profit would be lost
by freeing those subcomponents to make the subcomponents available. Then, lenses
are only added if they have positive profit, or if one of the atoms requires a
mapping. After there are no more lenses with positive profit or with
subomponents requiring a mapping, the sequence lens is returned.
...
we enhance our rewrites by changing an annotation to from closed to open. This
