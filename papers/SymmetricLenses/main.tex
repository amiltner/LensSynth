...
expressions) and a small number of examples.  \bcp{Why is the next line not indented?}
...
%
\bcp{Also, it would be nice to be able to give a quick example of a truly
  symmetric case---do any of these examples work for that?}\afm{h-event and
  vCalendar are truly symmetric. Even the fields related to location alone
  are truly symmetric (though it is subtle to see this).}
...
principled data transformations.

\textit{Synthesizing} lenses 
can make the task even easier.  Indeed, lens languages are excellent
targets for search-based synthesis, since their specialized, sub-Turing-complete syntax
and expressive type systems drastically
limit the space of possible programs. 
%
In particular, the Optician~\cite{optician} algorithm\bcp{there's a
  potential for confusion between ``Optician'' and ``Boomerang''---are they
  distinct systems?  Is one part of the other?  Is one a system and one just
  an algorithm?  It's not very clear.} has demonstrated that
it is feasible to synthesize quite complex examples, albeit only for a
restricted class of lenses---so-called bijective lenses. Specifically, given 
source and target formats plus a small number of
examples of corresponding data, Optician's synthesis algorithm is guaranteed
to find a bijective lens matching the examples, if one exists. Indeed, 
it can often find an appropriate lens in less than a second with
no examples at all.
...
source structure $S$ to a target $T$ can throw away information that must
then be restored when transferring from $T$ to $S$), and even more flexibile
transformations that allow each side to retain information not present in
the other?

\bcp{We use $S$ and $T$ here but $X$ and $Y$ below.}
...
present in either $S$ or $T$. Logistically, these complements must be stored
...
language~\cite{Boomerang}\bcp{this seems to assume that readers already know what
  Boomerang is} and apply them to a range of real-world
...
target and vice versa. Conversely, an unlikely lens is one that requires a
large amount of additional information to recover the target given a
source.\bcp{I'm nervous about the way we're talking about source and target
  here.  For one thing, even the words ``source'' and ``target'' connote a
  directionality that is misleading here.  And even ignoring that issue, a
  lot of the discussion (like here) sounds as though we are talking about
...
of shared information\bcp{really?  It's hard to be certain without lots of
  practical experience, but a priori it seems equally reasonable for users
  to want to pull little bits of shared information out of huge structures
  and keep them in sync.  But maybe we can weaken the claim and just say
  that we have an intuition that this will at least be {\em one} large class
  of application scenarios, and this is the one we focus on}, and so lenses
that propagate more information are preferable to ones that propagate less.
%
...
With simple symmetric lenses and our SRE-based likelihood measure in
...
The interesting complications arise from the need to deal with
regular-expression equivalences.  There are infinitely many regular
expressions equivalent to a given one, and the lens returned by a
type-directed search will in general depend on which of the possible
representations are chosen for its source and target formats.  Moreover,
there may be {\em no} lens connecting the 
source and target formats selected\bcp{until we get past the colon, it is
  not clear that we are talking here about formats {\em not} modulo
  equivalence.} by the user: in general, the synthesis 
algorithm has to search to find equivalent ones for which a suitable lens
exists.  
To tame this complexity, we divide the synthesis algorithm into two
communicating search procedures (Figure~\ref{fig:high-level-algorithm}),
following~\citet{optician}. 
The first, \Expand, uses rewriting rules to iteratively
propose new pairs of stochastic regular expressions equivalent to the
original pair.
The second, \GreedySynth, takes a pair of SREs as input
and uses a greedy, type-directed algorithm to find a simple symmetric
lens between them, returning the lens and its likelihood score
to \Expand.  The whole synthesis algorithm terminates when a comparison of the
likilihood score of the best lens found so far with 
the complexity\bcp{what does it mean to compare likelihood with complexity?}
of newly generated SREs  
...
The ``programmer''\bcp{user?} can control the search process via examples and annotations
...
%
We
present a collection of combinators for building simple symmetric lenses and
describe how to calculate their likelihoods (\S\ref{sec:ssl}).
\item We give
an algorithm for synthesizing lenses built from these combinators, using a 
novel application of stochastic regular expressions to guide the search
process (\S\ref{sec:synthesis})\bcp{``and prove it correct''!  (right?)}.  
%
...
\ifappendices\else included with the supplemental material\bcp{Right?}\fi.
...
\end{center}
\vskip 1ex
\bcp{Are we going to use the numbers next to the laws??  Seems like not, so
  why have them at all?}
\noindent
The two \KW{create} functions are used to propagate shared information and
fill in default values when introducing new data (\EG, the ``unk'' salary
entry is added\bcp{is added by what?  A create function, I guess, but the
  connection is not clear} alongside the name to the management file when HR inserts a
new employee). The two \KW{put} functions propagate edits from one format to
the other by combining a new value from one with an old value from the
other.  The round-tripping laws guarantee that pushing an ``un-edited''
value from one side (the result of a put or create) back through a lens in
the other direction will get back to exactly where we began.  \bcp{That
  paragraph seems likely to be quite opaque to any reader that is not
  already familiar with (symmetric) lenses.  How about a concrete example?}

This definition differs from classical symmetric lenses in that it does not
involve a \emph{complement}.  We give a detailed comparison in
...
In principle, programmers can define simple symmetric lenses just by
writing four arbitrary functions by hand and showing they satisfy the round-tripping
laws.  But a better idea is to streamline the process by providing a set of
combinators---a domain-specific 
language---for building law-abiding complex lenses from simpler ones.  Such
bidirectional languages
have been given for building lenses over many different kinds of data,
including database relations~\cite{BohannonPierceVaughan} and algebraic data
types~\cite{symmetric-lenses}\bcp{and what else? graphs, etc.?}.  We focus
here on combinators for defining lenses between string data formats
...
\bcp{We don't actually give a nontrivial example of how disconnect works.}
...
%let emp_lens = or(id(""),employees_lens) in
  \caption{A lens that synchronizes management and HR employee files\bcp{I'm
    pretty sure there was an error in name\_lens: the thing between the two
    names should be id(" ") (it was ins('','')).  Crossing my fingers that I'm
    making a correct fix...}}
...
to another, equivalent, regular expression type. \bcp{The next sentence is
  dense.  Just show the rule?}Our typing rules include type
...
equivalent to $\BRegex'$, and $\BRegexAlt$ is equivalent to
$\BRegexAlt'$. \bcp{We're not talking about synthesis yet, so this next
  sentence seems off-track.  Drop it?}The
...
\bcp{Suggest deleting this sentence:}The algorithm
...
likelihoods correspond to fewer bits. Yet more formally, we say that two
strings $s$ and $t$ are \emph{synchronized 
according to lens $l$} if $l.\PutR\ s\ t = t$ and $l.\PutL\ t\ s = s$.\footnote{
The notation $l.\PutR$ extracts the \PutR function from simple
symmetric lens $l$; similarly for \PutL.\bcp{Let's introduce this notation
  earlier when talking about lenses; shoving it in a footnote here is clunky.}}  We say that we
...
information.  Thus, if both \IdentityLens{}
and \Disconnect{} are satisfying lenses for a particular pair of formats,
then \IdentityLens{} will be more likely.

The expected number of bits required to recover a piece of data corresponds
to the well-known information-theoretic concept of
\emph{entropy}~\cite{Shannon1948}. Calculating entropy requires a
probability distribution over the space of possible values for the
data. Specifically, given a set $S$ and a probability distribution
$P \OfType S \rightarrow \Reals$ over $S$, the entropy $\EntropyOf{S,P}$ is
$-\Sigma_{s\in S}P(s)\Log_2P(s)$.  \bcp{That definition requires some more words of
  explanation!  And probably an example or two.  It is fundamental to the
  rest of the paper: if a reader doesn't follow it, they will be lost.  (I
  am a little lost myself... is entropy a property of an individual piece of
  data (as the wording above suggests), or is it a property of the set to
...
\bcp{This paragraph has a funny flow: The first sentence implies that
  entropy is all we need.  The second sentence then says that, actually, we
  need something else!}Entropy is the tool we need to
choose likely satisfying lenses from 
a collection of satisfying lenses. All such lenses must synchronize data between
the same two SREs $\Regex$ and $\RegexAlt$. For any lens $\Lens$, we can
calculate the expected number of bits required to recover a string $t$ in
$\LanguageOf{\RegexAlt}$ from a synchronized string\bcp{Have we defined
  what a ``synchronized string'' is?} $s$ in
$\LanguageOf{\Regex}$.  This expectation is the \emph{conditional entropy of
  ...
  \Regex}\ProbabilityOf{\Regex}{s}\cdot\EntropyOf{\SetOf{t \SuchThat
    \Lens.\PutROf{s}{t} = t}}$.\bcp{It seems H is applied here to one argument.  Are
  we supposed to know what that means.} The likelihood we assign to $\Lens$ is the sum
...
equally well take as input a\bcp{pair of} stochastic regular expression rather than a
...
expressions\bcp{the rest of the sentence feels out of place (and it's
  grammatically confusing, suggesting that there might be an RE that is NOT
  semantically equivalent to infinitely many others)}
where each regular expression is semantically equivalent to infinitely many
others. Following existing work on bidirectional program
synthesis~\cite{maina+:quotient-synthesis}, we 
split our algorithm into two communicating
synthesizers. The first synthesizer\bcp{why
call this algorithm a synthesizer?},
...
synthesizer, \GreedySynth, in rank order with the smallest first.

Abstractly, \GreedySynth looks for highest-likelihood lenses between a given pair of
stochastic regular expressions $\Regex$ and $\RegexAlt$ by performing a
type-directed search. In more detail, \GreedySynth 
first normalizes the stochastic regular expressions provided by \Expand into
\emph{stochastic DNF regular expressions}---a constrained representation of
stochastic regular expressions with disjunctions  
distributed over concatenations and with associativity information\bcp{what
  is ``associativity information''??} 
removed.  \GreedySynth then uses the syntax of these n-ary DNF regular
expressions to find highly normalized\bcp{is ``highly normalized'' different
from ``normalized''?} lenses, in a form we call \emph{simple
...
regular expressions, hence \GreedySynth's search space for a given pair of regular
...
names to first names and last names to last names\bcp{the relationship of
  the parenthesized comment to what comes before is not clear}). Finally, \GreedySynth
yields a simple symmetric lens by converting the n-ary
forms into the binary forms provided in the surface language.

To illustrate the partnership between \Expand and \GreedySynth,
consider the lens shown in Figure~\ref{fig:example_lens}. 
Inferring this lens requires \Expand to use a star-semiring axiom\bcp{are we
assuming readers know what that is?} to
rewrite the regular expression \lstinline{("\n" . emp_salary)$^*$} within
\lstinline{emp_salaries} into 
\lstinline{"" | (("\n" . emp_salary). ("\n" . emp_salary)$^*$)} 
so that \GreedySynth can find the appropriate lens.  \bcp{This example would
be more helpful if it were spelled out in full.  What is the path from the
start of the search to the point where we find this solution?}

This architecture of communicating synthesizers gives us a way to
...
problem: when do we stop?\bcp{This question would arise independent of the
  architecture of the search algorithm!} We may have found a promising lens
...
acceptable to the users. \bcp{How would the algorithm know this?}

The search algorithm must resolve a three-way tension among the quality of
...
searching for an even better lens.\bcp{How does this example illustrate a
  three-way tension?}
...
satisfying lens has relatively high\bcp{high compared to what??} likelihood,
indicating the lens is very 
promising. The heuristics also favor stopping if \Expand has delivered to
\GreedySynth all pairs of stochastic regular expressions at a given rank and
there is a large number of pairs at the next rank, because searching through all
such pairs will take a long time. With this approach, the algorithm can quickly
respond with a satisfying lens with relatively high likelihood. If the user is
unhappy with the result, they can either refine the search by supplying
additional examples, which serve both to rule out the previously proposed
lens(es) and to reduce the size of the search space by cutting down on the
number of satisfying lenses, or they can supply annotations on the source
and target SREs indicating that certain information
either must be retained or must be discarded by the lens (see
\S\ref{subsec:optimizations}\bcp{this reference to the later section with
  details is nice, but why don't we have similar references throughout this
  overview??}). 
...
needed to recover a string in one data source from a synchronized\bcp{again,
needs a definition} string in the
...
\begin{center}
  ...
  \end{tabular}
\end{center}
In the stochastic regular expression\bcp{abbreviate to SRE here and in many
  places elsewhere}
$\PRegexOr{\Regex_1}{\Regex_2}{\Probability}$, the annotation $\Probability$
represents the likelihood that the string is in $\Regex_1$\bcp{That seems
  both confusing (what is ``the string''??) and probably inaccurate (what if
  $S_1$ and $S_2$ are the same?)}. In the stochastic regular
...
having a string involving exactly $n$ iterations. \bcp{Last sentence needs
  an example.  Probably the first sentence also.}
...
However, existing work does not provide a means of reasoning about stochastic
regular expression equivalence\bcp{Does not provide a means of {\em
    reasoning} about it, or does not even define
  it?}. Figure~\ref{fig:ssr-sre} shows how we to extend the
\emph{star-semiring}~\cite{Droste2009} equivalences\bcp{again, we seem to be
assuming that all readers will already know what they are...} to stochastic regular
expressions.\bcp{Notation: I don't see any reason for the $s$ superscript in
$\SSREquiv$}
...
material\bcp{Right?}\fi.  \bcp{It took me quite a while to find it.  We
...
describe a random element in $S$ (formally $-\Sigma_{s\in S}\,P(s)\cdot \Log_2P(s)$). The
entropy of a stochastic regular expression can be computed directly from its
syntax, when it is unambiguous\bcp{undefined term} and contains no
empty subcomponents.
\begin{center}
  \begin{tabular}{rcl}
    $\EntropyOf{s}$
  ...
  \end{tabular}
\end{center}
\bcp{This sentence says exactly what the previous one does.  Drop.}We have proven that this syntactic computation corresponds to the entropy of the
stochastic regular expression when the stochastic regular expression is
unambiguous (parses strings uniquely).

\bcp{Again, this is a critical definition for understanding the paper, and
  it is pretty gnarly.  Examples, please!}

\begin{theorem}
  \label{thm:correct_entropy}
  If $\Regex$ is unambiguous and does not contain $\emptyset$ as a subterm,
  $\EntropyOf{\Regex}$ is the entropy of $\Regex$.\bcp{Should it not be $-\EntropyOf{\Regex}$??}
\end{theorem}
...
probability annotations. In a slight abuse of notation\bcp{why is this an
  abuse of notation??}, if $\Regex$ is a
...
$\LanguageOf{\BRegex}$ and $\LanguageOf{\BRegexAlt}$.  \bcp{Ugly typesetting:}
...
We now give typing rules and semantics for these combinators. We do not
include the semantics for combinators equivalent to those presented in prior
work~\cite{symmetric-lenses,Boomerang}\bcp{Huh?}. Furthermore, we present
...
everywhere, even though some lenses can actually be defined over ambiguous
regular expressions (for example, the identity lens doesn't need any
ambiguity constraints\bcp{Is this the only example?}); we believe relaxing
...
lost.  \bcp{We should highlight here the fact that this is a significant
  loss of power for some examples, compared to full symmetric lenses.}
%\begin{center}
%  \begin{tabular}{@{}r@{\ }c@{\ }l@{}}
...
calculate the conditional entropy, were it not for \MergeL\bcp{and \MergeR??}. If
...
dramatically and they do not contain instances of composition.\bcp{The last
  sentence basically repeats what the rest of the paragraph already says.}
...
Conversely, the termination heuristic is satisfied algorithm cuts
short\bcp{huh??}: if it 
...
If synthesizing a lens seems to be taking a long time, it is\bcp{always?} spending time
checking it cannot find a lens that projects less information. By ``skipping''
portions of the SRE that \emph{should} be projected, the algorithm will not
continue searching for a better lens when it has already found the correct one.
In practice, we implement \Skip by setting the information content of
$\Skip(\Regex)$ to zero.

If the algorithm returns a lens with too disconnects\bcp{??} and merges, one could
...
\bcp{This discussion seems both too brief---maybe some concrete examples
  would help?---and too informal: E..g., What do skip and require have to do
  with probability distributions and entropy?}
...
We have implemented symmetric DNF lenses as an extension to
Boomerang. Specifically, we added symmetric lens combinators to
Boomerang\bcp{that's exactly what the previous sentence said!}, and
implemented all of Boomerang's asymmetric lens 
combinators  using a combination of the symmetric lens combinators presented in this
paper and symmetric versions of the asymmetric extensions (like matching lenses~\cite{matchinglenses} and quotient lenses\cite{quotientlenses})
already present in Boomerang.
We also integrated our synthesis engine into Boomerang. This
integration allows users to write synthesis tasks 
alongside lens combinators, incorporate synthesis results into 
manually-written lenses, and reference previously defined lenses in
the synthesis of new lenses.
...
whether this additional expressiveness is useful in practice.\bcp{That's a
  total cop-out! We have much more to say about this point, and we {\em
    should} have even more to say than we do.} In our judgement,
...
\paragraph*{Other Lens Formulations}\bcp{This paragraph doesn't add much}
