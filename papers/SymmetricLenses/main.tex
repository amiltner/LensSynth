...
the string was already determined. Using this foundation, a lens $\Lens \OfType
\Regex \Leftrightarrow \RegexAlt$ is ``likely,''
...
corresponding synthesis algorithm using an example drawn from a hypothetical
U.S. company. In this company, management and human resources (HR) store
information about employees in separate files: management records the names of
...
examples of the two types of files and regular expressions describing the two
formats. The company uses simple symmetric lenses to keep these files
...
\lstinline{company} fields are independent, so we use the disconnect lens to relate them. The defaults are strings that indicate the information is missing.
%
\begin{lstlisting}
...
To that end, we adopt \emph{stochastic regular expressions}\cite{?}, 
...
To obtain stochastic regular expressions from the plain regular expressions that
...
\bcp{This is a POPL paper: Proofs of important theorems should
be included.  (This one might be easy enough that we can just label it as a
``Fact'' and omit the proof, or better yet we can say that the proof is straightforward.}

\subsection{Stochastic Regular Expression Entropy}
The {\em entropy} of a stochastic regular expression can be computed
directly from its syntax, when the stochastic regular expression is
unambiguous and contains only nonempty subcomponents.\bcp{What is an empty
  subcomponent?} 
\begin{center}
  \begin{tabular}{rcl}
    $\EntropyOf{s}$
  ...
  \end{tabular}
\end{center}
We have proven\bcp{Again, some kind of proof is needed.} that this syntactic
computation corresponds to the entropy of the stochastic regular expression.
...
$\REntropyOf{\RegexAlt \Given \Lens, \Regex}$ to bound the expected amount of
...
conditional entropy to recover the data.\bcp{Again, the proofs should be
  included.}
...
\begin{theorem}
  ...
  \begin{enumerate}
  \item $\REntropyOf{\RegexAlt \Given \Lens, \Regex}$ bounds the expected
    information content of $\SetOf{t \SuchThat t \in \LanguageOf{\RegexAlt}}$,
    given $\SetOf{s \SuchThat s \in \LanguageOf{\Regex} \BooleanAnd
      \Lens.\PutROf{s}{t} = t}$
  \item $\LEntropyOf{\Regex \Given \Lens, \RegexAlt}$ bounds the expected
...
expressions whose DNF form gives equal probability to all sequences\bcp{Why?}, and gives
iterations a .8 chance to continue iterating, as that gives a reasonable
distribution of strings when generating them from the regular
expressions\bcp{What evaluation did we do to
  determine that it is the right value?}.
...
We have extended Optician\bcp{this is the first mention of the word
  Optician!} with our symmetric synthesis algorithm. Users can 
...
\item Is our synthesis procedure efficient enough to be used in everyday
  ...
  ``specification overhead'' for users?  \bcp{Maybe it would be better to
    ask {\em which} heuristics are the most useful.  Also, are we talking
    about reducing the number of examples needed?  It's not clear.  (And:
    Don't the heuristics also make it faster??)}
\end{enumerate}

\subsection{Benchmark Suite}
Our benchmarks are drawn from four different sources.\bcp{What makes this a good
  set of benchmarks for present purposes?}
\begin{enumerate}
\item We adapted \afm{8} data cleaning benchmarks from Flash Fill. In adopting
  these benchmarks, we go beyond the data cleaning tasks\bcp{confusing: you
    say these are data cleaning tasks, then immediately say we go beyond
    data cleaning tasks.  (I know the next bit of the sentence makes it
    (somewhat) clearer, but the reader is already lost right here.)}, and
  synthesize 
  synchronization functions\bcp{what is a ``synchronization function''}
  ...
  be in bijective form\bcp{Mysterious}. With our symmetric enhancement, we
  \bcp{words missing?} Optician's four 
...
\bcp{Why is this a good set of benchmarks for the purposes described earlier
in the section?}
...
lenses.\bcp{Note that this comparison doesn't, directly answer the question
  ``Is it fast enough.''}
...
\begin{figure}
  ...
  \caption{Number of benchmarks that can be solved by a given algorithm in a
    ...
    synthesis. \bcp{The interesting part of the graph is all squished
      against the Y axis.  Can we make the X axis logarithmic?}}
  \label{fig:times}
\end{figure}
...
\subsection{The Effect of Our Heuristic}
