...
The entropy of a data source $S$ is $-\Sigma_{s\in S}P(s)\Log_2P(s)$, the
expected number of bits required to describe a random element in $S$. The
entropy of a stochastic regular expression can be computed directly from its
syntax, when the stochastic regular expression is unambiguous.
\begin{center}
  \begin{tabular}{rcl}
    $\EntropyOf{s}$
    & =
