...
\end{theorem*}
\begin{proof}
Given $s \in S$, let $V_{\ell}(s) = \{t \in \mathcal{L}(T) \; | \; \ell.\PutLOf{t}{s} = s\}$. We want to show that
$$\sum_{s \in \mathcal{L}(S)}P_{\mathcal{L}(S)}H(V_{\ell}(s)) = \mathbb{H}^{\leftarrow}(S \; | \; \ell, T)$$
...
Observe that $V_{\IdentityLensOf{S}}(s) = \{t \in \mathcal{L}(S) \; | \; \IdentityLensOf{S}.\PutLOf{t}{s} = s\} = \{t \in \mathcal{L}(S) \; | \; t = s\} = \{s\}$.

Consequently, $H(V_{\IdentityLensOf{S}}(s)) = H(\{s\}) = 0$, from which it follows that 
$$\sum_{s \in \mathcal{L}(S)} P_{\mathcal{L}(S)}(s) * H(V_{\IdentityLensOf{S}}(s)) = 0 = \mathbb{H}^{\leftarrow}(S \; | \; \IdentityLensOf{S}, T)$$
\item
($\ell = \DisconnectOf{\Regex}{\RegexAlt}{\String}{\StringAlt}$)

Observe that 
$$V_{\DisconnectOf{\Regex}{\RegexAlt}{\String}{\StringAlt}}(s) = \{t \in \mathcal{L}(T) \; | \; \DisconnectOf{\Regex}{\RegexAlt}{\String}{\StringAlt}.\PutLOf{t}{s} = s\} = \{t \in \mathcal{L}(T) \; | \; s = s\} = \mathcal{L}(T)$$
Consequently
$$\sum_{s \in \mathcal{L}(S)}P_{S}(s)H(V_{\DisconnectOf{\Regex}{\RegexAlt}{\String}{\StringAlt}}(s)) = \sum_{s \in \mathcal{L}(S)}P_{\mathcal{S}}(s)H(\mathcal{L}(T)) = H(\mathcal{L}(T)) = \mathbb{H}(T) = \mathbb{H}^{\leftarrow}(S \; | \; \DisconnectOf{\Regex}{\RegexAlt}{\String}{\StringAlt}, T)$$
\item
($\ell = \IterateLensOf{\Lens} : S^{*p} \Leftrightarrow T^{*q}$)

Observe that 
\begin{align*}
V_{\IterateLensOf{\Lens}}(s_1 \cdot \ldots \cdot s_n) &= \{t_1 \cdot \ldots \cdot t_m \; | \; \IterateLensOf{\Lens}.\PutLOf{(t_1 \cdot \ldots \cdot t_m)}{(s_1 \cdot \ldots \cdot s_n)} = s_1 \cdot \ldots \cdot s_n\}\\
&= \{t_1 \cdot \ldots \cdot t_n \; | \; \ell.\PutLOf{t_i}{s_i} = s_i \text{ for }1 \leq i \leq n\}\\
...
&= \mathbb{H}^{\leftarrow}(S \; | \; \ell, T) + \sum_{j=1}^{n-1}H(V_{\ell}(s_j))
\end{align*}
Consequently,
\begin{align*}
\sum_{s \in \mathcal{L}(S^{*p})}P_{S^{*p}}(s) H(V_{\IterateLensOf{\Lens}}(s))
&= (1-p)\sum_n p^n \sum_{\substack{(s_1, \ldots s_{n-1}) \\ s_i \in \mathcal{L}(S)}} \prod_{i=1}^{n-1}P_S(s_i) \left( \mathbb{H}^{\leftarrow}(S \; | \; \ell, T) + \sum_{j=1}^{n-1}H(V_{\ell}(s_j))\right)\\
&= (1-p)\sum_n p^n \sum_{\substack{(s_1, \ldots, s_{n-1})\\s_i \in \mathcal{L}(S)}} \prod_{i=1}^{n-1}P_S(s_i) \mathbb{H}^{\leftarrow}(S \; | \; \ell, T)\\
...
&= \mathbb{H}^{\leftarrow}(S \; | \; \ell, T) * (1-p)\sum_n p^n \\
&+(1-p)\sum_n p^n \sum_{\substack{(s_1, \ldots, s_{n-1}) \\ s_i \in \mathcal{L}(S)}} \prod_{i=1}^{n-1}P_S(s_i) \left(\sum_{j=1}^{n-1}H(V_{\ell}(s_j))\right)
\end{align*}
Unrolling the term 
...
$$\sum_{s \in \mathcal{L}(S^{*p})}P_{S^{*p}}(s) H(V_{\IterateLensOf{\Lens}}(s))
= \mathbb{H}^{\leftarrow}(S \; | \; \ell, T) * (1-p)\sum_n n * p^n  = \frac{p}{1-p} * \mathbb{H}^{\leftarrow}(S \; | \; \ell, T)$$
which is what we wanted to show.
\end{enumerate}
