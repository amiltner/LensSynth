...
regular expression equivalence\bcp{Does not provide a means of {\em
    reasoning} about it, or does not even define it?}. We extend the
...
\begin{proof}
  ...
  the appendix presented in the full version of the paper accompanying this
  submission.
\end{proof}
...
describe a random element in $S$ (formally $-\Sigma_{s\in S}P(s)\Log_2P(s)$). The
entropy of a stochastic regular expression can be computed directly from its
syntax, when the stochastic regular expression is unambiguous and contains no
empty subcomponents.
\begin{center}
  \begin{tabular}{rcl}
    $\EntropyOf{s}$
...
Without unambiguity constraints it is not easy to calculate entropy (and the
method we have given is certainly incorrect).  For example, consider the SRE
$\PRegexOr{\lstinline{"a"}}{\lstinline{"a"}}{.5}$. Using the above computation,
...
subtle ways and the algorithm must be correct when only some of the two languages
overlap. Similar issues occur when trying to find the entropy when $\emptyset$
is a subterm (what should the entropy of $\PRegexStar{\emptyset}{.5}$ be?),
so we do not define entropy on $\emptyset$.
...
emptysets out of the stochastic regular expressions that are themselves
nonempty by traversing the appropriate star-semiring equivalences (\EG,
...
We now provide the typing rules and semantics for these combinators. We do
not include the semantics for combinators equivalent to those presented in
prior work~\cite{symmetric-lenses,Boomerang}. Furthermore, we only present
\CreateR and \PutR when \CreateL and \PutL are symmetric; full details can
be found in an appendix\ifappendices\else{} (in the supplemental
materials)\fi. For simplicity of presentation, we use unambiguous regular
expressions everywhere, even though some lenses can actually be defined over
ambiguous regular expressions (for example, the identity lens doesn't need
any ambiguity constraints\bcp{Is this the only example?}); we believe
relaxing this restriction is unproblematic.
...
string in $\RegexAlt$, synchronized by $\Lens$. We use the notation $a(b,c)$ to
mean $(a*b,a*c)$, and the notation $(a,b)+(c,d)$ to mean $(a+c,b+d)$.
\begin{center}
  \begin{tabular}{rcl}
...
\paragraph*{Relevance Annotations}
...
Conversely, the termination heuristic is satisfied algorithm cuts short: if it
...
If synthesizing a lens seems to be taking a long time, it is spending time
...
If the algorithm returns a lens with too disconnects and merges, one could
...
Boomerang, and implemented all of Boomerang's asymmetric lens
...
\paragraph*{Relation with Simple Symmetric Lenses} To compare classical and simple
