...
%
Recent work~\cite{optician} has exploited this observation, demonstrating that
...
Indeed of the 48 benchmark problems described in Section \ref{sec:evaluation},
...
type-directed search will depend which of the infinitely many possible
...
This definition differs from classic symmetric lenses in that it does not
...
clear from context, we shoren ``simple symmetric lens'' to just
``lens.''
...
We now provide the typing derivations and semantics for these lenses. We do
...
through possible regular expressions harder. Our algorithm 
identifies when certain closed regular expressions can \emph{only} be
...
paper and symmetric versions of the asymmetric extensions (like matching lenses and quotient lenses)
...
zero or one examples, runs the tools, determines whether the output is the
...
primary concern is whether it is usable to generate synthesize correct lenses.

To determine whether the system can synthesize the desired lenses, we ran it
...
it shows of the responsiveness of the system when used as
