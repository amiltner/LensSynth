...
In summary, our contributions are:\bcp{The section references do not proceed
  ...
  don't.}\bcp{But this way, the reader has no birds-eye view of the paper at
  this point...}
\begin{itemize}
%
\item We define {\em simple symmetric lenses}, a subclass of symmetric
lenses that do not maintain any hidden state (\S\ref{sec:overview}).  We
...
lenses~\cite{symmetric-lenses} in expressiveness.  We further show that the
...
(\S\ref{sec:related}).
%
\item We give an algorithm for synthesizing simple symmetric lenses, using a
novel application of stochastic regular expressions to guide the search
process (\S\ref{sec:synthesis})\bcp{``and prove it correct''???}.  Along the
way, we show that {\em star semi-ring equivalences} on regular expressions
