...
formats. 
\ksf{I don't understand the above.  We shouldn't explain the issue in
...
Given a lens $\Lens$ between two stochastic regular expressions $\Regex$ and
$\RegexAlt$, we can calculate the expected number of bits required
to recover a string $s$ in $\LanguageOf{\Regex}$ from a synchronized
string $t$ in $\LanguageOf{\RegexAlt}$. 
This expected value is just the conditional entropy of the $\Regex$ given
$\RegexAlt$ and $\Lens$. 
To ensure symmetry, we seek to find the lens that
minimizes the sum of that entropy with the conditional entropy of $\RegexAlt$
given $\Regex$ and $\Lens$. 

\ksf{This piece might better go somewhere deeper in the paper.}
Our algorithm guesses a reasonable probability distribution from
