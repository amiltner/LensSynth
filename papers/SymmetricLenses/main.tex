...
evaluate
its performance on 48 benchmark examples drawn from FlashFill~\bcp{cite}, Augeus~\bcp{cite},
and an online collection of electronic file formats. Our
implementation can synthesize all these lenses in under
30 seconds. 
\end{abstract}
...
vCalendar, and h-event formats~\cite{calendar-formats}; US taxation data can
be transmitted via Tax XML (used by the US government for e-filing) or TXF
(used by TurboTax)~\cite{tax-formats}; and sewing machines can use
embroidery files in formats with names like PES, DST, and several
others~\cite{embroidery-formats}.  \bcp{It would be nice to be able to give
  a quick example of a truly symmetric case---do any of these examples work
  for that?}
...
earlier work~\cite{maina+:quotient-synthesis}\bcp{Is this the right citation?}.
...
\item We extend the Boomerang implementation with simple symmetric lenses,
  ...
  other lens extensions such as quotient lenses~\bcp{citation} and matching
  lenses~\bcp{citation}.  We evaluate our
...
work~\cite{maina+:quotient-synthesis}\bcp{Some of this text seems to be
  identical to text in the intro.  That will annoy careful readers.}. The
first synthesizer, \Expand, navigates 
...
\begin{figure}
  ...
  \end{tabular}
  \caption{Stochastic Regular Expression Star-Semiring Equivalence\bcp{Can
      we make the skips between the separate sections of the definition a
      bit larger, and increase the margin on the left?  It looks (even)
      denser than it is, at the moment.}}
  \label{fig:ssr-sre}
\end{figure}
...
$\ToDNFRegexOf{\Regex}$, is defined below.\bcp{Could put this in two columns
to save space.}
\[
  \begin{array}{rcl}
...
\begin{theorem}
  ...
  where:\bcp{This could be set in two columns.}
  \begin{center}
    \begin{tabular}{rcl}
