...
to front,'' allowing updates to both input and output data to be transferred
in both directions.
Since their introduction by~\citet{Focal2005-long}, lenses have been extensively
studied, extended, and applied.  Recent work~\cite{optician} has also demonstrated how
...
expressions) and a small number of examples.  \bcp{Why is the next line not
  indented?} \afm{I have truly no idea, deferring until I get a stackoverflow answer.}

We show how to extend this synthesis algorithm to a much wider class of
lenses, which we call {\em simple symmetric lenses}, including not only all
of the bijective lenses and the more widely used ``asymmetric''
lenses~\cite{Focal2005-long}, but also a rich subset of the full-blown
``symmetric lenses'' proposed by \citet{symmetric-lenses}.  
...
theory to estimate the amount of information propagated by a candidate lens,
in conjunction with user annotations identifying parts of the source and
target data structures as either {\em irrelevant} or {\em essential}.

Simple symmetric lenses are also of independent theoretical interest, being
...
\noindent
In today's data-rich world, similar information is often
stored in different places and in different formats. For instance, electronic
calendars come in iCalendar, vCalendar, and h-event
formats~\cite{calendar-formats}; US taxation data can be transmitted via Tax
XML (used by the US government for e-filing) or TXF (used by
TurboTax)~\cite{tax-formats}; and sewing machines can use embroidery files
in formats with names like PES, DST, and several
others~\cite{embroidery-formats}.
%
\bcp{I'm not satisfied with the embroidery example: It doesn't obviously
  correspond to a bidirectional situation (the sewing machine does not need
  to send back the embroidery file to anybody).  The tax example could be
  criticized on the same grounds.}
\afm{I still think that there are situations where the embroidery file makes
  sense (I'm sharing my design with a friend, who then tweaks it and sends it
  back. There may also need to file-specific information, where one contains
  parameters like how fast to embroider and some doesn't, to be honest I don't
  know much about embroidery though. Thinking about other examples, however.)}
...
Domain-specific languages for writing lenses can facilitate building
...
languages, like Boomerang, are excellent targets for search-based synthesis, since their
specialized, sub-Turing-complete syntax and expressive type systems drastically
limit the space of possible programs.
%
In particular, the recent Optician~\cite{optician} enhancement to Boomerang has
demonstrated that it is feasible to synthesize quite complex examples, albeit
only for a restricted class of lenses---so-called bijective lenses.
Specifically, given source and target formats plus a small number of examples of
...
If the synthesis algorithm can
find any bijection at all, it is fairly likely to be the intended one.

However, the set of real-world use cases for bijective lenses is also
somewhat limited.  A more common situation is where two real-world data
formats share just {\em some} of their information content.  For instance,
two related database tables might share four out of five columns, but each
might have a different fifth column.  Or, one ad-hoc system configuration
file might include some format-specific meta data, such as a date or a
reference number, whereas the same configuration file on another operating
system does not.  Indeed among the 48 benchmark problems described in
Section \ref{sec:evaluation} below, all of the benchmarks taken from Flash
Fill~\cite{flashfill} and many of the benchmarks that synchronize between
two ad hoc file formats have this characteristic, 13 benchmarks in
total. 

Can Optician's basic synthesis procedure be extended to a richer class of
lenses? In particular, what about \emph{symmetric
...
then be restored when transferring from target to source), and even more flexibile
transformations that allow each side to retain information not present in
the other?
...
One might hope that extending the algorithms from \citet{optician} to
...
over three structures: the ``left'' structure $X$, the ``right'' $Y$ and a complement
$C$ that contains the information not
present in either $X$ or $Y$. Logistically, these complements must be stored
and managed somehow.  More fundamentially, complements complicate
...
restricted variant 
of symmetric lenses, which we call \emph{simple symmetric lenses}. 
...
To characterize their expressive power formally, we show that simple
symmetric lenses are exactly the symmetric lenses that satisfy an intuitive
property called \emph{forgetfulness}.  To check that they are expressive
enough in practice, we add simple symmetric lenses to the Boomerang
language~\cite{Boomerang} and apply them to a range of real-world
applications, showing in the process that simple symmetric lenses integrate
smoothly with other advanced lens features provided by Boomerang, including
quotient lenses~\cite{quotientlenses,maina+:quotient-synthesis} and matching
lenses~\cite{matchinglenses}.

This leaves us with just the second difficulty with synthesizing symmetric
lenses: While the number of bijective lenses between two given formats is
...
it finds, this will generally \emph{not} be the one the user wanted.  We
...
space more intelligently. For these, we turn to information theory. 

We consider a ``likely'' lens to be one that propagates ``a lot'' of information
from the left data format to the right and vice versa. Conversely, an unlikely
lens is one that requires a large amount of additional information to recover
either of the formats given the other. For many application scenarios, users
will want to synchronise formats with lots of shared information (and lenses
with little shared information are typically easier to write). Our tool is built
primarily for these complex scnearios, and so lenses that propagate more
information are preferable in our context to ones that propagate less.
%
This intuition is formalized using \emph{stochastic regular expressions}
...
probability distribution, we can calculate the shared information content of
two formats and our measure of likelihood.
...
\begin{figure}
  ...
  \caption{Schematic diagram for the simple symmetric lens synthesis algorithm.
    ...
    When the algorithm finds a likely lens, it returns it.\bcp{Later: We
      could save significant space if we put the inputs and outputs on the
      sides instead of top and bottom}}
  \label{fig:high-level-algorithm}
\end{figure}

With simple symmetric lenses and our SRE-based likelihood measure in hand, we
develop a new algorithm for synthesizing likely lenses. At its core, the
algorithm performs a type-directed search between the descriptions of the source
and target formats, measuring success using the likelihood measure. The
...
target formats. Moreover, if Boomerang's type-equivalence rule is disallowed,
there may be {\em no} well-typed lens connecting the source and target formats
selected by the user: in general, the synthesis algorithm has to search through
equivalent regular expression types for which a suitable lens exists. To tame
...
The first, \Expand, uses rewriting rules to iteratively propose new pairs of
stochastic regular expressions equivalent to the original pair. The second,
\GreedySynth, takes a pair of SREs as input and uses a greedy, type-directed
algorithm to find a simple symmetric lens between them, returning the lens and
its likelihood score to \Expand. The whole synthesis algorithm heuristically
terminates when the most likely lens is no longer unlikely enough to justify
the time cost of searching through newly generated SREs.
...
We have added this synthesis procedure to the Boomerang system and evaluated
its effectiveness on a benchmark suite of 48 problems, including data-cleaning
tasks taken from Flash Fill~\cite{gulwani-popl-2014}, view-maintenance tasks
taken from Augeas~\cite{augeas2}, and ad-hoc synchronization tasks from a
variety of other sources (mostly from the bidirectional programming literature).
The user can control the search process via examples and annotations
that informs the synthesis algorithm as to whether certain bits of information
are irrelevant, or must maintained.

In summary, our contributions are:
\begin{itemize}
%
\item We define {\em simple symmetric lenses}, a subclass of symmetric
lenses that do not maintain any hidden state (\S\ref{sec:overview}).  
%
We present a collection of combinators for building simple symmetric lenses and
describe how to calculate their likelihoods (\S\ref{sec:ssl}).
\item We give an algorithm for synthesizing lenses built from these combinators,
  ...
  search process (\S\ref{sec:synthesis}) and prove that our metrics have deep
  semantic meaning.
%
\item We extend the Boomerang implementation with simple symmetric lenses,
  demonstrating that they integrate smoothly with other lens extensions such as
  quotient lenses~\cite{quotientlenses} and matching
  lenses~\cite{matchinglenses}. We evaluate our implementation, the effects of
  various optimizations, and our inclusion of relevance annotations on a set of
...
%
\item We show that simple symmetric lenses fall between asymmetric
...
\end{itemize}
Along the
way, we show that {\em star-semiring equivalences} on regular expressions
...
Elided proofs can be found in the appendices\bcp{The appendices are pretty
  hideous and hard to navigate!!}
...
\end{lstlisting}
  ...
  \resizebox{\columnwidth}{!}{%
    \begin{tabular}{|>{\columncolor{vlightyellow}}c|>{\columncolor{vlightgreen}}c|}
      ...
      \cellcolor{lightgreen} HR's type\\
      \hline
  ...
  insurance providers, respectively. \bcp{The colors are a bit loud,
    especially since there is so little color elsewhere.}
  %
  \bcp{Suggest replacing ``Name,Company'' with ``First Last, Company'' (too
    many ``name''s flying around)}
  %
  \bcp{More substantially, thinking about this example makes me wonder about
    a whole different way of specifying simple symmetric lenses: rather than
    just giving corresponding source and target pairs, could we give triples
    of the form ``new source / old target / new target'' and ``new target /
    old source / new source''?  I know these would be a bit more work to
    write, but they would also give the synthesis procedure way more
    information to work with---it seems to me it should be able to do much
    less guessing about where things are disconnected.  Would this allow us
    to drop all the information theory?  Or would we still need it?  (In
    retrospect, this seems like a fairly obvious thing to try, so it seems
    important to try it and report on how it performs relative to what we
    propose here...)}\afm{Oh sorry, yes we can write such examples.  I personally found that typically the same information could be provided through a series of 2 creates (aka: if we want a disconnect: creater a = b; creater a' = b:  if we want an identity: creater a = a; creater a' = a').  However, we also permit putr and putl examples of the proposed form in the tool 
Much like other aspects of the work, any situation with examples ultimately only constrains the subcomponents of the lens corresponding to that specific parse tree (aka: if I give an example about A in A|B, it only constrains "A").  I don't think it removes the need for the information theory, as all the other non-constrained subcomponents of the lens can still be "anything."
}  }
...
Semantically, a simple symmetric lens between sets $X$ and
$Y$ comprises four functions subject to four
``round-tripping'' laws:  
...
\vskip 1ex
\noindent
The two \KW{create} functions are used to propagate shared information and fill
...
The notation $\Lens.\PutR$ extracts the \PutR function from simple symmetric
lens $\Lens$; similarly for \PutL. These four functions can be used to keep the
common information between two file types in synch. For example, if a new file
of the left-hand format is created, the \CreateR function will a new file in the
right-hand format. If the right-hand format is edited, the \PutL function can
update the left-hand format with the changed information.

The round-tripping laws guarantee that pushing an ``un-edited'' value from one
side (the result of a put or create) back through a lens in the other direction
will get back to exactly where we began. For example, consider,
\lstinline{emp_lens}, a lens between
\lstinline{emp_salaries} and
\lstinline{emp_insurance}. Applying
\lstinline{emp_lens}.\CreateR to a salary file creates a insurance file with the
same employee names in the file, with \lstinline{"UNK"} for each insurance
company. The round-tripping laws guarantee that using \lstinline{emp_lens}.\PutL
to put the generated insurance file into the original salary file will return
the original salary file, unmodified. If \KW{put}ting a string $s$ into a string
$t$ results in the original string $s$, then $s$ and $t$ are considered
\emph{synchronized}.

This simple symmetric lens definition differs from classical symmetric lenses in
that it does not involve a \emph{complement}. We give a detailed comparison in
...
In principle, programmers can define simple symmetric lenses just by writing
four arbitrary functions by hand and showing they satisfy the round-tripping
laws. But a better idea is to streamline the process by providing a set of
combinators---a domain-specific language---for building law-abiding complex
lenses from simpler ones. Such bidirectional languages have been given for
building lenses over many different kinds of data, including database
...
stochastic regular expressions; we mark plain REs with overbars.)  When
clear from context, we shorten ``simple symmetric lens'' to just
``lens.''

We illustrate some of these
combinators by defining a lens between the two employee data formats
in Figure~\ref{fig:minimized-representations}. The finished lens, which appears
in Figure~\ref{fig:example_lens}, has type
\lstinline{emp_salaries} $\Leftrightarrow$
\lstinline{emp_insurance}.
...
\end{lstlisting}
%
The identity lens fully connects the corresponding data in the two formats:
when one is updated, the other gets the exact same value. Both the \CreateR
and \CreateL functions are the identity function ($\CreateR \App s = s$),
and the put functions merely return the first argument
($\PutROf{\String}{\StringAlt} = \String$).
%
In our example, when a \lstinline{name} is updated in one file, the identity
lens ensures it is also updated in the other file.

In contrast, the \Disconnect lens does not propagate any edits at all
from one format to the other.  It takes four arguments: two
...
HR file.
For example, with the above lens, $\PutL \App \lstinline{""} \App 60000$ will return 60000,
...
For example, the composite lens \lstinline{name_lens} in the running
example
%
\begin{lstlisting}
...
\end{lstlisting}
%
will transform ``\lstinline{Jane Doe: }'' to ``\lstinline{Jane Doe,}'' in
...
We provide typing rules for the simple symmetric combinators, most of
them syntax directed. However, regular expressions have a number of
equivalences on them, and sometimes we must convert one regular expression type
to another, equivalent, regular expression type.
%
\[
  \centering
  \inferrule*
...
Lens expressions for small examples like the one above are pretty easy to
write.  But for larger and more complex formats, it can become quite difficult to
write lenses by hand.  Instead, we wish to synthesize them from examples.

That is, given a pair of regular expressions and a set of input-output examples, we want
to find a simple symmetric lens that is typed by the regular expression pair and
satisfies all the input/output examples. We call such a lens a \emph{satisfying
...
example the data in Figure~\ref{fig:minimized-representations}.
...
\end{lstlisting}
%
is another well-typed lens (where \lstinline{rep($\BRegex$)} denotes some string in the
language of $\BRegex$).  Adding the input-output example in
Figure~\ref{fig:minimized-representations} 
helps, but only a little: In general, {\em many} examples may be required to rule
...
A satisfying lens is ``more likely'' if it ``connects'' more data between the
two formats, \EG, by using an identity rather than a disconnect lens. The
likeliest satisfying lenses would connect as much data as possible. More
formally, we define the likelihood of a satisfying lens as the expected number
of bits required to recover data in one format from synchronized data in the
other. Higher likelihoods correspond to fewer bits. Yet more formally, we say
that two strings $s$ and $t$ are \emph{synchronized according to lens $l$} if
$l.\PutR\ s\ t = t$ and $l.\PutL\ t\ s = s$. We say that we can \emph{recover}
$s$ from $t$ using bits $b$ and lens $l$ if we can reconstruct $s$ from $t$,
$b$, and $l$. For example, given the \IdentityLens{} lens, we can recover $s$
from $t$ using no bits because, in this case, $s$ is just $t$. In contrast,
given the \Disconnect{} lens, we need enough bits to fully encode $s$ in order
to recover it from $t$ because $t$ contains no useful information. Thus, if both
\IdentityLens{} and \Disconnect{} are satisfying lenses for a particular pair of
formats, then \IdentityLens{} will be more likely.
...
information theory, each element of $S$ has \emph{information content}, the
number of bits required to specify that element. The information content of an
element $s$ is $-\Log_2P(s)$. For example, let $S = \SetOf{a,b_1,b_2}$, where
$P(a)=.5$ and $P(b_1) = .25 = P(b_2)$. The information content of $a$ is $1$,
and the information content of each $b_i$ is 2. The entropy of $S$ calculates
the expected number of bits required to specify a random element pulled from
$S$, in other words, the probability of each element times its information
content. In our example, the entropy of $S$ is $1.5$. When $P$ is clear, we will
...
probability (see \S\ref{sec:sre}).  A stochastic regular expression
specifies both a set of strings and a probability
distribution over that set, enabling us to calculate the
entropy of the SRE.

In the synthesis setting, there are typically an infinite number of possible
lenses that satisfy the specification. There are many choices on which
components of the data should be synchronized vs which should be left
unsynchronized. We use entropy to guage the relative likelihood of such lenses
in our synthesis algorithm (see \S\ref{subsec:lenscosts}). For any lens $\Lens$, we can calculate the expected
number of bits required to recover a string $t$ in $\LanguageOf{\RegexAlt}$ from
a synchronized string $s$ in $\LanguageOf{\Regex}$. This expectation is the
\emph{conditional entropy of $\Regex$ given $\RegexAlt$ and $\Lens$}, formally
$\sum_{s \in \Regex}\ProbabilityOf{\Regex}{s}\cdot\EntropyOf{\SetOf{t \SuchThat
...
assigns higher entropy (or cost) to lenses where knowing the string on one side
...
To obtain SREs from the plain regular expressions that users write, we use a
heuristic that attempts to assign probability annotations that give each
string in the language equal probablity. Our synthesis algorithm could
equally well take as input a pair of stochastic regular expression rather than a
plain one, permitting users to specify a data-specific probability
distribution (either manually calculated or inferred from some dataset of
interest). However, we have found that our heuristic works well in practice,
so we have not explored these options.
...
likely lenses. Previous work~\cite{augustsson-2004, osera+:pldi15,
  feser-pldi-2015, frankle+:popl16} has shown that we can use types to
...
expressions. The fact that each regular expression is semantically equivalent to
infinitely many others complicates the problem.
...
searches. The first search,
\Expand, navigates the space of semantically equivalent regular expressions
by applying rewrite rules that preserve both the semantics and the
probability distributions of those stochastic regular expressions. This
...
Abstractly, \GreedySynth looks for highest-likelihood lenses between a given
pair of stochastic regular expressions $\Regex$ and $\RegexAlt$ by performing a
type-directed search. In more detail, \GreedySynth first normalizes the
stochastic regular expressions provided by \Expand into \emph{stochastic DNF
  regular expressions}---a constrained representation of stochastic regular
expressions with disjunctions distributed over concatenations and with
concatenations and disjunctions normalized to operating over lists (see \S\ref{subsec:sdnfre}).
\GreedySynth then uses the syntax of these n-ary DNF regular expressions to find
normalized lenses, in a form we call \emph{simple symmetric n-ary DNF lenses}.
These lenses include neither a composition operator nor a type equivalence rule.
These restrictions mean that each simple symmetric n-ary DNF lens is typed by a
single pair of stochastic n-ary DNF regular expressions, hence \GreedySynth's
search space for a given pair of regular expressions is finite. The variations
that \GreedySynth considers include choosing between \IdentityLens{} and
\Disconnect lenses and deciding how to match multiple occurrences of the same
kind of data, \EG, the two occurrences of \lstinline{name} in the management and
HR data formats (should the algorithm match the first instance of
\lstinline{name} on the left with the first instance of \lstinline{name} on the
right, or to the second). Finally, \GreedySynth yields a simple symmetric lens
by converting the n-ary forms into the binary forms provided in the surface
language.

To illustrate the partnership between \Expand and \GreedySynth, consider
synthesizing the lens shown in Figure~\ref{fig:example_lens}. When \GreedySynth
initially tries to find a lens between \lstinline{emp_salaries} and
\lstinline{emp_insurance}, it hits some snags: there are complex disjunctions
and concatenations in \lstinline{emp_salaries} outside the iteration, but no
such complexity in \lstinline{emp_insurance}. Because of this disparity between
the formats, \GreedySynth finds an unlikely lens (involving undesired
\KW{merge}s and \KW{disconnect}s), and the search continues. However, after
\Expand applies a star-semiring axiom (presented in Figure~\ref{fig:ssr-sre}) to
rewrite the regular expression \lstinline{("\n" . emp_salary)$^*$} within
\lstinline{emp_salaries} into
\lstinline{"" | (("\n" . emp_salary) . ("\n" . emp_salary)$^*$)}, the
subcomponents line up better, and the more likely correct lens is instead found.
...
new SRE pairs? We may have found a promising lens between a pair of stochastic
regular expressions, but a different pair we haven't yet discovered may give
rise to an even better lens.

The search algorithm must resolve a tension among the quality of
the inferred lens and the amount of time it takes to return a
...
To resolve this tension, the algorithm uses heuristics to judge whether to
return the current best satisfying lens to the user or to pass the next set of
unrollings to \GreedySynth. The heuristics favor stopping if the current best
satisfying lens is very likely, indicating the lens is very promising (for
example, if the satisfying lens loses no information, the algorithm should
terminate for no other lens will be more likely). The heuristics also favor
stopping if \Expand has delivered to \GreedySynth all pairs of stochastic
regular expressions at a given rank and there is a large number of pairs at the
next rank, because searching through all such pairs will take a long time. With
this approach, the algorithm can quickly respond with a satisfying lens with
relatively high likelihood. If the user is unhappy with the result, they can
either refine the search by supplying additional examples, which serve both to
rule out the previously proposed lens(es) and to reduce the size of the search
space by cutting down on the number of satisfying lenses, or they can supply
annotations on the source and target SREs indicating that certain information
either must be retained or must be discarded by the lens (see
\S\ref{subsec:optimizations}).
...
other data source. To do this, we must first develop a probabilistic 
model for our language. We do this with \emph{stochastic regular
  expressions} (SREs)---regular expressions annotated with probability
information~\cite{stoch-rnn,stoch-def}---which 
...
\begin{center}
  ...
  \end{tabular}
\end{center}
In the SRE $\PRegexOr{\Regex_1}{\Regex_2}{\Probability}$, the annotation
$\Probability$ represent that, if generating arbitrary strings from
$\PRegexOr{\Regex_1}{\Regex_2}{\Probability}$, strings should be pulled from
$\Regex_1$ with probability $\Probability$ (\EG pulling a string from
$\PRegexOr{\lstinline{"a"}}{\lstinline{"b"}}{.2}$ would retrieve \lstinline{"a"}
20\% of the time). In the SRE
$\PRegexStar{\Regex}{\Probability}$, the annotation \Probability represents how
much more likely having a string involving more than $n$ iterations is than
having a string involving exactly $n$ iterations (\EG pulling a random string
from $\PRegexStar{\lstinline{"c"}}{.8}$ would give \lstinline{""} 20\% of the
time, \lstinline{"c"} 16\% of the time, and \lstinline{"cc"} 12.8\% of the time).
...
The $\Expand$ algorithm needs to be able to enumerate SREs that are
...
we to extend the \emph{star-semiring}~\cite{Droste2009} equivalences (a
...
\begin{proof}
  See the appendix%
\ifappendices\else{} included with the supplemental
material\fi.  \bcp{It took me quite a while to find it.  We
  should give an actual pointer to a specific theorem in the appendix.}
\end{proof}

This theorem will come in handy as we traverse (with \Expand) or normalize
across (as part of \GreedySynth) star-semiring equivalences.
...
describe a random element in $S$ (formally $-\Sigma_{s\in S}\,P(s)\cdot
...
For example, the entropy of $\PRegexOr{\lstinline{"a"}}{(\lstinline{"b"})}{.5}$
is 1. This is expected because on average 1 bit is required to encode whether a
randomly pulled string is \lstinline{"a"} or \lstinline{"b"}. As an additional
example, a fixed string has no information content, and so has no entropy.
...
Without unambiguity constraints, the method above for calculating entropy does
not work, and it is not easy to see how to generalize it.  For example,
consider the SRE $\PRegexOr{\lstinline{"a"}}{\lstinline{"a"}}{.5}$. Using
the above computation, this SRE has an entropy of $1$, but the entropy
should be $0$. These difficulties become particularly complex when the
languages are ambiguous in subtle ways and the algorithm must be correct
when only some of the two languages overlap. Similar issues occur when
trying to find the entropy when $\emptyset$ is a subterm (what should the
entropy of $\PRegexStar{\emptyset}{.5}$ be?), so we do not define entropy
on~$\emptyset$. 
...
probability annotations. In a slight abuse of notation (as we are also using
these symbols as nonterminals), if $\Regex$ is a
...
the language of \BRegex and the language of \BRegexAlt.
...
Briefly (full definitions are below), the compose lens $\Lens_1 ; \Lens_2$
...
We now give typing rules and semantics for (some of) these combinators. We do not
include the semantics for combinators equivalent to those presented in prior
work~\cite{Boomerang}. Furthermore, we present
just \CreateR and \PutR in cases where \CreateL and \PutL are symmetric;
full details can be found in an appendix\bcp{where,
  exactly??}\ifappendices\else{} (in the supplemental materials)\fi. For
simplicity of presentation, we use unambiguous regular expressions
everywhere, even though the identity and disconnect lenses can actually be defined over ambiguous
regular expressions; we believe relaxing
this restriction is unproblematic.
...
uncommon in practice; only 2 of the lenses sythesized in our benchmarks
include merges.

The cost of a lens between two SREs $cost(\Lens, \Regex, \RegexAlt) =
\MaxOf{\LEntropyOf{\Regex \Given \Lens, \RegexAlt}} +
\MaxOf{\REntropyOf{\RegexAlt \Given \Lens, \Regex}}$ is the sum of the maximum
expected number of bits to
recover the left format from the right, and the right from the left. We have
...
lenses that inhabit a disjunctive normal form.
...
current best lens has cost $c$, then
  $\Call{Continue}{pq}$ checks whether $c > d + \Log_2(n)$.
...
an iteration of a symmetric DNF lens. Note that the structure of symmetric DNF
lenses exactly mirrors that of SDNF REs.
...
intellectually difficult. We elide them here but include them in the appendix.\bcp{Where?}
...
\subsection{Optimizations and Relevance Annotations}
...
hash-consing and memoization; and compositional synthesis. The implementation
also provides for additional specifications in the form of annotations that help
describe which information is irrelevant to the lens. The optimizations make the
system performant enough for interactive use, and the annotations help ensure
the desired information is projected.
...
\paragraph*{Relevance Annotations}\bcp{Wonder whether this should be its own
top-level section...?}\afm{I'm not sure, any other thoughts? To me it is
inherently an optimization, though it's a more important and interesting one
than others}\bcp{My impression was that, before we invented these
annotations, things were really not working very well.  So they may be
``just an optimization,'' but they are a super-important one.  Moreover,
they strongly impact the way the user interacts with the system.  I think
they should figure prominently in the way we explain what we've done.}
...
In particular, our system works hard to find lenses that project \emph{as little
  information as possible}. If the desired lens requires projecting a large
amount of information, the termination condition will be satisfied ``too late.''
While \GreedySynth may have found what the user knows to be the desired lens,
the full algorithm will continue trying to find lenses that project less
information.

Conversely, sometimes the termination heuristic is overly permissive and algorithm cuts
short: if it 
had continued searching for lenses it would have found the desired one, but the
termination heuristic was satisfied ``too early.'' We wish for our algorithm to
provide feedback in a timely manner, but sometimes this prohibits finding the
desired lens.

To address these issues, we allow annotations on the regular expressions
dictating relevance. The first, $\SkipOf{\Regex}$, says the information
content of $\Regex$ is irrelevant to the desired lens, and can the data can be
safely projected. The second, $\SRequireOf{\Regex}$, says the information of
$\Regex$ is critical to the desired lens, and none of the information can be
lost when converting between the formats.

If synthesizing a lens seems to be taking a long time, it is typically spending
time checking it cannot find a lens that projects less information. By
``skipping'' portions of the SRE that \emph{should} be projected, the algorithm
will not continue searching for a better lens when it has already found the
correct one. In practice, we implement \Skip by setting the information content
of $\Skip(\Regex)$ to zero.
%
\begin{gather*}
  \EntropyOf{\Skip(\Regex)} = 0
\end{gather*}
%
If synthesizing a lens between \lstinline{emp_salaries} and
\lstinline{emp_insurance} from Figure~\ref{fig:minimized-representations} is
taking overly long, the synthesis procedure is uncomfortable disconnecting
\lstinline{company} and \lstinline{salary} information. Redefining
\lstinline{company} and \lstinline{salary} as $\SkipOf{\lstinline{company}}$ and
$\SkipOf{\lstinline{salary}}$ would set their information content to zero,
alleviating such issues.

If the algorithm returns a lens with too many disconnects and merges, one could
provide additional examples to constrain the lens more. However, for large
formats, writing such examples is time consuming and error-prone. Instead, if
one recognizes that information from $\Regex$ is incorrectly lost when converted
between formats, they can prevent that information from being lost by annotating
it $\SRequireOf{\Regex}$. One can guarantee a lens between \Regex and \RegexAlt
is bijective by annotating them as $\SRequireOf{\Regex}$ and
$\SRequireOf{\RegexAlt}$. In practice, we implement \SRequire by setting the
cost of any lens that loses ``required'' information to infinity.
%
\begin{center}
  \begin{tabular}{rcl}
  ...
  \end{cases}$\\
    ...
    & =
    & $\begin{cases}
      \infty & \vspace*{-.3em}\text{if $\RegexAlt$ contains $\SRequireOf{\RegexAlt'}$ as a subexpression,}\\
      & \text{where $\EntropyOf{\RegexAlt'} \neq 0$}\\
...
%
If synthesizing a lens between \lstinline{emp_salaries} and
\lstinline{emp_insurance} from Figure~\ref{fig:minimized-representations}
quickly returns a lens with too many disconnects and merges (perhaps
disconnecting instances of \lstinline{name} in \lstinline{emp_salary} and
\lstinline{emp_ins}), the termination heuristic is being overly permissive. By
replacing \lstinline{name} with $\SRequireOf{\lstinline{name}}$ in
\lstinline{emp_salary} and \lstinline{emp_ins}, name information can no longer
be lost, and future synthesis calls would continue until a more likely lens is
found.
% end synthesis
...
We have implemented symmetric DNF lenses as an extension to Boomerang.
We have
implemented all of Boomerang's asymmetric lens combinators using a
combination of the symmetric lens combinators presented in this paper and
symmetric versions of the asymmetric extensions (like matching
lenses~\cite{matchinglenses} and quotient lenses\cite{quotientlenses}) already
present in Boomerang. We also integrated our synthesis engine into Boomerang.
This integration allows users to write synthesis tasks alongside lens
combinators, incorporate synthesis results into manually-written lenses, and
reference previously defined lenses in the synthesis of new lenses.
...
\begin{enumerate}
\item We adapted 8 data cleaning benchmarks from Flash Fill. Note
that our tool produces bidirectional transformations rather
than one-way transformers like Flash Fill. 
...
All three sources provide realistic problems suitable for
answering the research questions: Flash Fill data cleaning tasks are either
derived from online help forums or taken from the Excel product team; Augeas is
a utility that transforms Linux configuration files into a manageable dictionary
form; and many of the remaining benchmarks arise from synchronizing real-world
data formats such as BibTeX and EndNote.  Each task consists of a source and
target regular expression.\saz{Do we want to say anything about the sizes of these?}

\subsection{Synthesizing Correct Lenses}

Because our tool is intended to be interactive---the user typically starts with
zero or one examples, runs the tool, determines whether the output is the
desired one and, if not, either adds an example or relevance annotations and
then tries again---it is hard to cleanly evaluate the net user experience. The
primary concern is whether it is usable to synthesize correct lenses.
...
more than three examples in order to synthesize the desired lens.\footnote{In
...
We verified that \SSOpt{} generated the correct lenses the way programmers often
...
\begin{tabulary}{\linewidth}{rL}
  ...
  \SSNCOpt{}: & The symmetric synthesis algorithm, with no compositional synthesis enabled.\\
\end{tabulary}\\
...
synthesis, the synthesis algorithm is able to solve 26 problem instances.
...
termination heuristic.  To this end, we run our program in several different modes:\\[1ex]
\begin{tabulary}{\linewidth}{rL}
  \AnyOpt{}: & We ignore the information-theoretic preference metric entirely,
               returning the first lens \GreedySynth finds without using this information. \\ 
  \FLOpt{}: &  \GreedySynth uses the information-theoretic preference
              metric to search, but returns immediately upon finding a
              satisfying lens, instead of returning based on \Continue.  \\
  \CCOpt{}: & We replace our information-theoretic cost metric with one where
              ...
              of merges.\\
  \NSOpt{}: & We ignore all \Skip annotations in the SRE specifications. \\
  \NROpt{}: & We ignore all \SRequire annotations in the SRE specifications. \\
\end{tabulary}
...
measures such as ``avoid constants'' which align with, but are simpler
...
the algorithm only discovers 5 lenses, which shows that the first cluster that
contains a satisfying lens is rarely the correct cluster. However, our algorithm
...
While classical symmetric lenses are technically more expressive than simple
symmetric lenses, we still believe simple symmetric lenses are useful in
practice. Indeed, all asymmetric lenses (we later show) are expressible as
simple symmetric lenses. The primary loss is the loss of ``memory'' within the
complement. In classical symmetric lenses, \OrLens lenses retain information, so
users don't lose information when swapping from one disjunct to the other, the
information is stored in the complement. Simple symmetric lenses have no such
complement, so they mimic the forgetful \OrLens lens of classical symmetric
lenses.

In our judgement, the complications they introduce into synthesis did
not justify the potential additional expressiveness: because each lens has a
...
with edit sequentials; another would be to specify the structure of complements
...
\paragraph*{Other Lens Formulations}\bcp{This paragraph doesn't add
  much}\afm{the POPL reviewers complained we didn't have this type of
  paragraph, I personally agree that it doesn't add much and feels unnecessary}
Bidirectional programming has an extensive literature, with many extensions onto
basic lens formulations, like quotient lenses\cite{quotientlenses} and matching
lenses\cite{matchinglenses}. Interested readers can consult
\citet{DBLP:conf/icmt/CzarneckiFHLST09} for a slightly dated survey of lenses
and other lens-like structures.
...
Since $S$ is unambigous, then $S_1$ and $S_2$ are also unambiguous, and for each $s \in \mathcal{L}(S_1 \cdot S_2)$ there exists a unique $s_1 \in \mathcal{L}(S_1)$ and a unique $s_2 \in \mathcal{L}(S_2)$ such that $s = s_1 \cdot s_2$. By the induction hypothesis, $H(\mathcal{L}(S_1)) = \mathbb{H}(S_1)$ and $H(\mathcal{L}(S_2)) = \mathbb{H}(S_2)$, hence\bcp{Don't understand what's going on with the $-$
  signs in the last few lines here.}
\begin{align*}
...
\bcp{``Since...then...'' is not english.}
Since $S$ is unambiguous, then $S_1$ and $S_2$ are also unambiguous, and
...
\mathbb{H}(S_2)$. Then\bcp{Again, don't understand what's going on with the $-$
  signs.}
\begin{align*}
