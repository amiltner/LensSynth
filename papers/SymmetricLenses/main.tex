...
has gone into Optician, a tool for automatically synthesizing such conversion
functions from input-output examples and formal descriptions of the data
...
Google Calendar CSV files do not expose.
...
synchronization tasks, as we must maintain the files we synchronize as well as
maintaining the complements. The complement further complicates synthesis, as
...
We enhance Optician with simple symmetric lens synthesis capabilities. This
extension synthesizes symmetric lenses from a suite of input-output examples and
a pair of regular expressions describing the data formats. The synthesis
algorithm uses type-directed synthesis on the two regular expressions to
synthesize a lens that is well-typed between the provided regular expressions
and satisfies the provided input-output examples.
...
created, and vice versa. Furthermore, we require \emph{puts}, functions that
...
We utilize this same framework, except instead of having the second synthesizer
find \emph{any} lens between the types, \GreedySynth instead uses a greedy
algorithm to try finding the \emph{best} lens between the types.
