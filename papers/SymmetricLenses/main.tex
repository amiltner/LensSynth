...
presented an element of the set. 
...
parameters\dpw{What does that mean? You had to alter it to get the system to work?}, as we had to alter the original specifications to generate the
correct lens.
...
\begin{tabulary}{\linewidth}{rL}
  \AnyOpt{}: & We synthesize any lens \dpw{Did you mean the following? But is the order truly arbitrary? Is size or anything else a driver?:] We synthesize lenses in an arbitrary order (i.e. without use of the information theoretic preference metric)} that satisfies the specification, with no
               notion of cost.\\
  \FLOpt{}: &  The first time \GreedySynth finds a lens, the synthesis procedure
              returns that lens.\dpw{What controls the ``greediness''? Say a little something else to more clearly differentiate from Any.}\\
