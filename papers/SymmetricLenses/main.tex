...
so we use the disconnect lens to relate them\bcp{Potentially confusing to
  readers (and, even to me, hacky). Why not relate each of them to the constant
  empty string?}\afm{I'm fine with the empty string.  I've seen sentinel values
  like UNK used before in practice, but if it makes the example cleaner and more
  comprehensible, sure.  Anybody else have thoughts on the matter?}.
...
$\Regex$ given $\RegexAlt$ and $\Lens$, \bcp{Am I supposed to know what this
  means?} and the conditional entropy of $\RegexAlt$ given $\Regex$ and
$\Lens$.\bcp{Why is this the right (or a sensible) thing to do, besides the fact
  that it ensures symmetry?}
...
stochastic regular expressions.\bcp{Why these?}
...
\begin{center}
  \begin{tabular}{rcl}
    ...
    & =
    & $(0,1+\REntropyOf{\RegexAlt \Given \Lens_1, \Regex_1}+\REntropyOf{\RegexAlt \Given \Lens_2, \Regex_2})$\\
...
\section{Synthesis}
\label{sec:synthesis}
Algorithm~\ref{alg:synth-sym-lens} presents our synthesis algorithm at a
high level of abstraction. The
input regular expressions are first converted into stochastic regular
expressions with \ToStochastic. Once that conversion has taken place,
the $\RXSearch$ algorithm executes.
This algorithm itself is defined via
five functions:
(1) $\PCF{\RXSearch.Create}$,
(2) $\PCF{\RXSearch.Continue}$,
(3) $\PCF{\RXSearch.Propose}$,
(4) $\PCF{\RXSearch.Enqueue}$, and
(5) $\PCF{\RXSearch.Best}$.
First, $\PCF{\RXSearch.Create}$ creates a priority queue ($rxs$)
containing a single element:  the initial pair of stochastic regular
expressions.  Next, 
$\RXSearch$ enters a loop that searches for likely lenses.  The loop
terminates when the algorithm believes it is unlikely
to find a better lens than the best one it has found so far (a
termination condition defined by $\PCF{\RXSearch.Continue}$).
Within each iteration of the loop, it:
\begin{itemize}
\item generates the next class ($S$, $T$) of lenses to
search ($\PCF{\RXSearch.Propose}$),
...
\item adds this best lens to the set of possibilities
under consideration $\PCF{\RXSearch.Add}$
\end{itemize}
...
\dpw{Note: I added overbars to regexps in Algorithm 1 below ... double check me
for correctness.}
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \State $\RXSearchState \gets \Call{\RXSearch.Create}{\Regex,\RegexAlt}$
    ...
    \State $\RXSearchState \gets \Call{\RXSearch.Add}{\RXSearchState,\LCO}$
    \EndWhile
    \State \ReturnVal{\Call{RXSearch.Best}{\RXSearchState}}
    \EndFunction
  \end{algorithmic}
\end{algorithm}
...
\subsection{\PCF{\RXSearch.Propose}}
...
pairs with the smallest number of rewrites first. When there are no more
proposed regular expressions derived from $n$ rewrites, \RXSearch proposes
regular expressions derived from $n+1$ rewrites.
...
specified stochastic regular expressions themselves, this step could be skipped,
though our integration into Boomerang does not permit users to provide
...
We describe this synthesis algorithm in the context of trying to find a lens
between \lstinline{"" | name.name*} and \lstinline{"" | name}, with the example
\lstinline{creater "John Doe" = "John"}.
...
First, \PCF{CannotMap} checks if there could possibly be a lens satisfying the
...
right DNF regular expressions, given the examples that match them. An internal
state for the algorithm is initialized from these. This state consists of two
things: an intermediate partial DNF Lens and a priority queue consisting of
potential sequence lenses.
...
not, \PCF{GreedyState.ToDNFLens} instead returns \None.
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \EndIf
    \EndWhile
    \State \ReturnVal{\Call{GreedyState.ToDNFLens}{\GreedyState}}
    \EndFunction
  \end{algorithmic}
\end{algorithm}
...
This algorithm uses \PCF{AtomSynth}, a simple function that synthesizes atom
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \EndIf
    \EndWhile
    \State \ReturnVal{\Call{GreedySeqState.ToDNFLens}{\GreedyState}}
    \EndFunction
  \end{algorithmic}
\end{algorithm}
