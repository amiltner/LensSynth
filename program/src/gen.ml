	...
	: dnf_lens option =
		...
		: dnf_lens option =
			begin match PQ.pop queue with
				...
				(* print_endline "mytest"; print_endline (regex_to_string r1);     *)
...
let rec whole (r: regex) (rc: RegexContext.t) : regex * RegexContext.t =
	match r with
	| RegExEmpty -> RegExEmpty, rc
	| RegExBase s -> RegExBase s, rc
	| RegExConcat (r1, r2) ->
			let r1, rc = whole r1 rc in
			let r2, rc = whole r2 rc in
	...
	| RegExOr (r1, r2) ->
			...
			RegExOr (r1, r2), rc
	| RegExStar r ->
			let r, rc = whole r rc in RegExStar r, rc
	| RegExVariable s ->
			let pair = RegexContext.lookup_everything rc s in
			begin match Option.map ~f: fst pair, Option.map ~f: snd pair with
				| Some r, Some b ->
						let r, rc = whole r rc in
						let rc = RegexContext.insert_exn ?quotient: (Some true) rc s r b in
						RegExVariable s, rc
				| _ -> failwith "Huh?"
			end
	| RegExPermute (l, sep) ->
			let sep, rc = whole sep rc in
			let helper (l : regex list) : regex * RegexContext.t =
				let rec helperIn (l : regex list)
						((r, rc) : regex * RegexContext.t) : regex * RegexContext.t =
					match l with
					| [] -> r, rc
					| s :: tl ->
							let s, rc = whole s rc in
							helperIn tl (RegExConcat(r, RegExConcat(sep, s)), rc) in
				match List.hd l, List.tl l with
				| Some x, Some y -> let r, rc = whole x rc in helperIn y (r, rc)
				| _ -> RegExEmpty, rc
			in
			let bigOr (l : (regex * RegexContext.t) list) : regex * RegexContext.t =
				let rec bigOrHelper
						((r, rc) : (regex * RegexContext.t)) (l : (regex * RegexContext.t) list)
				: regex * RegexContext.t =
					match l with
					| [] -> r, rc
					| (x, _) :: xs -> bigOrHelper (RegExOr(r, x), rc) xs in
				match List.hd l, List.tl l with
				| Some x, Some y -> bigOrHelper x y
				| _ -> RegExEmpty, rc
			in bigOr (List.map ~f: helper (permutations l))
	| RegExMap (r, _) -> whole r rc
...
let rec kernel (r: regex) (rc: RegexContext.t) : regex * RegexContext.t =
	...
	| RegExVariable s ->
			...
			begin match Option.map ~f: fst pair, Option.map ~f: snd pair with
				| Some r, Some b ->
						let r, rc = kernel r rc in
						let rc = RegexContext.insert_exn ?quotient: (Some true) rc s r b in
						RegExVariable s, rc
				| _ -> failwith "Huh?"
			end
	| RegExPermute (l, sep) ->
			let sep, rc = kernel sep rc in
			let helper (l : regex list) : regex * RegexContext.t =
				let rec helperIn (l : regex list)
						((r, rc) : regex * RegexContext.t) : regex * RegexContext.t =
					match l with
					...
					| s :: tl ->
							...
							helperIn tl (RegExConcat(r, RegExConcat(sep, s)), rc) in
				match List.hd l, List.tl l with
				| Some x, Some y -> let r, rc = kernel x rc in helperIn y (r, rc)
				| _ -> RegExEmpty, rc
			in helper l
