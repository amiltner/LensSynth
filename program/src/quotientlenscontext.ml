...
(***** The main QuotientLensContext module {{{ *****)

module type QuotientLensContext_Sig = sig
    type t

    val empty                    : t
    val lookup_exn               : t -> id -> (lens * quotient_regex * quotient_regex)
    val lookup_type_exn          : t -> id -> quotient_regex * quotient_regex
    val lookup_impl_exn          : t -> id -> lens
    val insert_exn               : t -> id -> lens -> quotient_regex -> quotient_regex -> t
    val insert_list_exn          : t -> (id * lens * quotient_regex * quotient_regex) list -> t
    val create_from_list_exn     : (id * lens * quotient_regex * quotient_regex) list -> t
    ...
    val get_lens_context         : t -> LensContext.t 
end
...
module QuotientLensContext_Struct (Dict : Dictionary) : QuotientLensContext_Sig = struct
  type t = { defs     : (id, (lens*quotient_regex*quotient_regex)) Dict.t ;
  ...
  let lookup_exn (lc:t) (name:id) : lens*quotient_regex*quotient_regex =
    Dict.find_exn name lc.defs

  let lookup_type_exn (lc:t) (name:id) : quotient_regex*quotient_regex =
  ...
  let update_defs (defs:(id, (lens*quotient_regex*quotient_regex)) Dict.t)
      (name:id) (l:lens) (r1:quotient_regex) (r2:quotient_regex)
    : (id, (lens*quotient_regex*quotient_regex)) Dict.t =
  ...
  let insert_exn (lc:t) (name:id) (l:lens) (r1:quotient_regex) (r2:quotient_regex) : t =
    begin match (r1,r2) with
      | (QuotientRegExVariable id1, QuotientRegExVariable id2) ->
  ...
  let insert_list_exn (lc:t) (nirsl:(string * lens * quotient_regex * quotient_regex) list) : t =
    List.fold_left
  ...
  let create_from_list_exn (nirsl:(string * lens * quotient_regex * quotient_regex) list) : t =
  ...
  let get_lens_context (lc : t) : LensContext.t =
    LensContext.create_from_list_exn (List.map ~f:(fun (i, (l, r1, r2)) -> (i, l, kernel r1, kernel r2)) (Dict.as_kvp_list lc.defs)) 
end


module QuotientLensContext = QuotientLensContext_Struct(ListDictionary)
