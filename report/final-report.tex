...
%\addbibresource{local.bib,bcp.bib}
...
the Defense Advanced Research Project Agency's (DARPA) Building
Resource Adaptive Software Systems (BRASS) program---it is necessary to develop new, easier-to-use
...
comma with a vertical bar, or the name of one Hypertext Markup
Language (HTML) tag with another)
...
Lens-based languages are present in a variety of tools and have found mainstream
industrial use.
...
and
Extensible Markup Language (XML) transformation languages~\cite{DBLP:conf/pepm/LiuHT07}.
...
\end{lstlisting}
%
transforms a list of employees in the Management format to a list
...
from Augeas were tested after our synthesizer was
...
310 Abstract Syntax Tree (AST) nodes, and requires 1.1 input/output examples.
...
\begin{figure}
  ...
  \caption{Average number of random examples required to synthesize benchmark
    programs. %  {\bf Examples Required} is the average number of randomly
    % generated examples needed to correctly synthesize the lens.
    % {\bf
...
The average number of examples needed is much lower than the maximum number of
required examples because of correspondences in how we wrote the regular
expressions for the source and target data formats.  Specifically, when we had
corresponding disjunctions in both the source and the target, we ordered them
the same way.  The algorithm uses the supplied ordering to guide its search, and
so the system requires fewer examples.  We did not write the examples in this
style to facilitate synthesis, but rather because maintaining similar subparts
in similar orderings makes the types much easier to read. We expect that most
...
between XML, Resource Description Framework (RDF) and
Javascript Object Notation (JSON) formats using data from the data.gov
database; the data consisted of census statistics, demographic
statistics, wage comparison data, and crime index data).
...
In the second approach, ``Bijective Synthesis'' (\canonizeAndSpecSize{}), the
programmer uses the Optician bijective synthesis algorithm.
...
In the third approach, ``No Synthesis'' (\LensAndSpecSize{}), the
...
as proxies for the level of programmer effort when using each of the three
approaches: 
...
\begin{itemize}
  ...
  formats, (2) the number of AST nodes in $\canonize(q)$ for each QRE $q$
  requiring a  non-trivial canonizer (``canonizers'' as transformations that
  project a string to its equivalence class representative; they are written as
  programs in Boomerang), and (3) the number of AST nodes in the
  examples.  We use (1) to estimate the cost of describing
  the external source and target formats and (2) to estimate the
  cost of writing the requisite canonizers
...
\begin{figure}
  ...
  \label{fig:symm-times}
\end{figure}

For each benchmark in the suite and each mode, we ran the system with a timeout
of 60 seconds, averaging the result over 5 runs. Figure~\ref{fig:symm-times}
summarizes the results of these tests. We found that our algorithm is able to
...
To compare to the existing bijective synthesis algorithm, we ran our symmetric
...
the specification but was not bijective and hence was different from
the one generated by Optician.}
