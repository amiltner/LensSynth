...
module type Priority_Queue_Sig = sig
  ...
  val create_from_list : ('a * int) list -> 'a t

  val push : 'a t -> 'a -> int -> 'a t

  val push_all : 'a t -> ('a * int) list -> 'a t
  
  val pop : 'a t -> ('a * int * ('a t)) option
...
module Priority_Queue : Priority_Queue_Sig = struct
  ...
  let push (queue:'a t) (data:'a) (priority:int) : 'a t =
  ...
  let push_all (queue:'a t) (data_priority_list:('a * int) list) : 'a t =
    List.fold_left
    ~f:(fun acc (d,p) -> push acc d p)
      ~init:queue
      data_priority_list

  let create_from_list (data_priority_list:('a * int) list) : 'a t =
    push_all create data_priority_list

  let pop (queue:'a t) : ('a * int * ('a t)) option =
    begin match queue with
    | (d,p)::t -> Some (d,p,t)
    | [] -> None
    end
